...
# SPEC-GENIE-CLI-001 — genie CLI (cobra + Connect-gRPC + bubbletea TUI)
...
GENIE-AGENT의 **사용자 대면 CLI**를 정의한다. v0.1.0은 Phase 0에서 최소 cobra subcommand만 제공했으나, ROADMAP v2.0(2026-04-21)이 본 SPEC을 Phase 3으로 재배치하며 아래 3가지 축으로 확장한다:
...
3. **Slash Command 통합**: SPEC-GENIE-COMMAND-001의 Dispatcher를 클라이언트 측에서 **프리-디스패치**하여 `/clear`, `/exit` 같은 local command는 네트워크 왕복 없이 처리. prompt expansion은 daemon 전달.

본 SPEC 수락 시점에서:

- `genie` (사용자 CLI) + `genied` (데몬) 2개 실행파일.
- `genie` (인자 없음 또는 `genie chat`) → TUI REPL 진입.
- `genie ask "msg"` / `genie ask --stdin < file` → non-interactive, stdout에 응답 스트리밍.
- `genie session list/load/save/rm` → 세션 파일 관리 (`~/.genie/sessions/<name>.jsonl`).
- `genie config get/set/list` → 설정 조작 (CONFIG-001 RPC 위임).
- `genie tool list` → `Registry.ListNames()` 출력 (TOOLS-001 소비).
- `genie plugin list/install/remove` → PLUGIN-001 미구현 시 stub "not yet available".
...
> v1.0 SPEC-GENIE-CLI-001은 cobra 단일 구조였으나, v2.0에서:
...
1. **Phase 3 의존성 정렬**: MVP Milestone 1이 `TOOLS-001 → CLI-001`를 요구. CLI-001이 Phase 0에 있으면 `genie tool list` 같은 핵심 UX 명령이 누락.
...
- **v0.1.0 SPEC-GENIE-CLI-001 `spec.md` / `research.md`** (DEPRECATED, 본 repo 유지). exit code 표, cobra 명령 구조, `genied/main.go` 골격은 **원문 계승**. Transport 부분만 Connect-gRPC로 교체.
...
- **IN**: `cmd/genie/`, `cmd/genied/`, `internal/cli/` (tui/commands/session/config), Connect-gRPC 클라이언트, bubbletea TUI, slash command 프리디스패치, session 파일, keybindings.
- **OUT**: Auth(`genie login`), plugin 실제 설치(PLUGIN-001), web/mobile/desktop 클라이언트, Windows 정식 지원, 자동 update checker.
...
1. **2개 실행파일**:
   - `cmd/genied/main.go`: 데몬 진입 (CORE-001 bootstrap + TRANSPORT-001 gRPC wire-up + Registry initialization). v0.1.0과 거의 동일, ~40 LoC.
   - `cmd/genie/main.go`: 사용자 CLI. cobra root + subcommand 트리, ~15 LoC (로직은 `internal/cli/`).
...
3. **Subcommand 트리**:
   ...
   └── daemon
       ├── status
       └── shutdown              # GENIE_SHUTDOWN_TOKEN 필요
   ```
...
6. **Session 파일 관리**:
   - 경로: `~/.genie/sessions/<name>.jsonl` (JSON Lines).
...
8. **Keybindings (기본)**:
   ...
   - `Ctrl-R`: resume last session (`~/.genie/sessions/.last.jsonl`).
...
- **인증 명령**(`genie login`, OAuth 흐름): 후속 SPEC.
- **Plugin 실제 설치/업데이트**: 본 SPEC은 stub. 실제 구현은 PLUGIN-001.
- **Desktop/Mobile/Web 클라이언트** (genie-desktop Tauri, genie-mobile RN, genie-web Next.js): 별도 로드맵 (ROADMAP §10 OUT).
...
- **자동 update checker** (`genie update`): 후속.
...
**REQ-CLI-001 [Ubiquitous]** — Both `genie` and `genied` binaries **shall** expose `--version` or `version` subcommand that prints `genie version <semver> (commit <short>, built <iso>)` to stdout and exits 0. (v0.1.0 REQ-CLI-001 계승).

**REQ-CLI-002 [Ubiquitous]** — All error messages written to stderr **shall** be prefixed with `genie:` (for `genie`) or `genied:` (for `genied`). (v0.1.0 계승).
...
**REQ-CLI-004 [Ubiquitous]** — The `genie` client **shall** use Connect-gRPC (`connectrpc/connect-go`) as its transport library; the daemon's grpc-go server **shall** remain the server-side implementation (cross-library compatibility over HTTP/2).
...
**REQ-CLI-006 [Event-Driven]** — **When** `genie` is invoked with no subcommand (or `genie chat`), the CLI **shall** enter bubbletea TUI REPL, subscribing to `AgentService/ChatStream` and rendering streamed `SDKMessage` deltas progressively in the messages viewport.

**REQ-CLI-007 [Event-Driven]** — **When** `genie ask "<message>"` is invoked, the CLI **shall** open a `ChatStream`, send the message as the first stream request, and write the concatenated assistant text to stdout followed by a single newline; the stream **shall** be closed within 30 seconds default (override via `--timeout`).

**REQ-CLI-008 [Event-Driven]** — **When** the daemon connection cannot be established within 3 seconds, the CLI **shall** print `genie: daemon unreachable at <addr>` to stderr and exit with code 69. (v0.1.0 REQ-CLI-006 계승).
...
**REQ-CLI-011 [Event-Driven]** — **When** `genie session load <name>` is invoked, the CLI **shall** read `~/.genie/sessions/<name>.jsonl`, parse each line as an SDKMessage/Message, pass them as `ChatStream` initial messages, and enter TUI mode with viewport populated.

**REQ-CLI-012 [Event-Driven]** — **When** the user invokes `/save <name>` inside TUI, the client **shall** write the current messages array to `~/.genie/sessions/<name>.jsonl` (atomic via tmp+rename), display `[saved: <name>]`, and continue.

### 4.3 State-Driven (상태 기반)

**REQ-CLI-013 [State-Driven]** — **While** the daemon reports state `draining` via the Ping response, `genie ask` and `genie chat` **shall** return exit code 69 with message `daemon is draining`. (v0.1.0 REQ-CLI-008 계승).

**REQ-CLI-014 [State-Driven]** — **While** `GENIE_NO_COLOR=1` is set or stdout is not a TTY, the CLI **shall** disable ANSI color codes in all output (including TUI — fallback to minimal rendering). (v0.1.0 REQ-CLI-009 계승).
...
**REQ-CLI-016 [Unwanted]** — **If** the user supplies no arguments to `genie ask` and no `--stdin` flag, **then** cobra **shall** print usage and exit with code 2. (v0.1.0 REQ-CLI-010 계승).

**REQ-CLI-017 [Unwanted]** — **If** config file parsing fails (CONFIG-001 returns `ErrSyntax`), `genied` **shall** exit with code 78. (v0.1.0 REQ-CLI-011 계승).
...
**REQ-CLI-019 [Unwanted]** — The CLI **shall not** print daemon stack traces to stderr under normal operation; Go runtime errors from the daemon's `error` SDKMessage **shall** be formatted as `genie: daemon error: <message>` without stack content.

**REQ-CLI-020 [Unwanted]** — The TUI **shall not** save session files outside `~/.genie/sessions/`; `/save ../foo` attempts **shall** return error `[invalid session name]` without writing.
...
**REQ-CLI-023 [Optional]** — **Where** `GENIE_SHUTDOWN_TOKEN` is set and the user invokes `genie daemon shutdown`, the CLI **shall** call `DaemonService/Shutdown` with that token as `auth_token` metadata. (v0.1.0 REQ-CLI-015 계승).
...
**AC-CLI-001 — `genie version` 동작 (v0.1.0 AC-CLI-001 재수행)**
- **Given** 빌드 시 `-ldflags "-X main.version=0.2.0 -X main.commit=abc1234"`
- **When** `genie version`
- **Then** stdout에 `genie version 0.2.0 (commit abc1234, built <iso>)`, exit 0

**AC-CLI-002 — `genied` 부트스트랩 + Connect-gRPC listen**
- **Given** `t.TempDir()`으로 `GENIE_HOME` 설정
- **When** `genied` 실행 후 500ms 대기
- **Then** HTTP `/healthz` 200 응답 + gRPC `DaemonService/Ping` (Connect-Go client로 호출) state="serving"

**AC-CLI-003 — `genie ping` (Connect-Go client)**
- **Given** `genied` serving
- **When** `genie ping`
...
- **Given** `genied` 미실행
- **When** `genie ask "hi"`
- **Then** stderr에 `genie: daemon unreachable at 127.0.0.1:17891` + exit 69

**AC-CLI-005 — `genie ask` streaming E2E**
- **Given** `genied` serving + stub LLM이 `"Hi!"` 응답
- **When** `genie ask "Hello"`
- **Then** stdout에 `Hi!\n`, exit 0. 테스트는 `AgentService/ChatStream` 스트리밍 경로 사용 확인

**AC-CLI-006 — `genie ask --stdin`**
- **Given** `genied` serving, stdin pipe에 `"Hello"`
- **When** `echo "Hello" | genie ask --stdin`
- **Then** stdout에 응답 스트리밍 표시, exit 0

**AC-CLI-007 — TUI REPL 기본 진입**
- **Given** TTY 환경 (gomega-expect 기반 pty test), `genied` serving
- **When** `genie` (subcommand 없음) 또는 `genie chat`
...
**AC-CLI-010 — `genie session save/load` 왕복**
- **Given** TUI session에 user + assistant 1쌍 메시지 존재
- **When** `/save test01` 실행 후 TUI 종료. 이후 `genie session load test01`
- **Then** 새 TUI 시작 시 viewport에 이전 2개 메시지 복원, `~/.genie/sessions/test01.jsonl` 파일 2줄

**AC-CLI-011 — `genie tool list`**
- **Given** `genied` serving, TOOLS-001 Registry에 내장 6종 + MCP 1종(mock) 등록
- **When** `genie tool list`
- **Then** stdout에 정렬된 리스트 (`Bash, FileEdit, FileRead, FileWrite, Glob, Grep, mcp__foo__bar`), exit 0

**AC-CLI-012 — `genie config get/set/list`**
- **Given** `genied` serving, CONFIG-001 RPC 지원
- **When** `genie config set log.level debug` → `genie config get log.level`
- **Then** 두 번째 명령 stdout이 `debug`, exit 0

**AC-CLI-013 — `genie plugin install` stub**
- **Given** PLUGIN-001 미구현 상태
- **When** `genie plugin install ./some-plugin`
- **Then** stderr에 `genie: plugin system not yet available (SPEC-GENIE-PLUGIN-001 pending)`, exit 1

**AC-CLI-014 — Usage error exit 2**
- **Given** 실행 환경
- **When** `genie ask` (인자 없음, `--stdin` 없음)
- **Then** stderr에 cobra usage + exit 2

**AC-CLI-015 — `--format json` for streaming**
- **Given** `genied` serving
- **When** `genie ask "hi" --format json`
...
```
cmd/
├── genie/main.go                # ~15 LoC
└── genied/main.go               # ~40 LoC (v0.1.0 유지 + Registry init)
...
import (
    ...
    geniev1 "github.com/genieagent/genie/internal/transport/grpc/gen/geniev1"
    "github.com/genieagent/genie/internal/transport/grpc/gen/geniev1/geniev1connect"
)

type Client struct {
    daemon  geniev1connect.DaemonServiceClient
    agent   geniev1connect.AgentServiceClient
    tool    geniev1connect.ToolServiceClient
    config  geniev1connect.ConfigServiceClient
    baseURL string
}
...
func Dial(ctx context.Context, addr string, timeout time.Duration) (*Client, error) {
    ...
    c := &Client{
        baseURL: baseURL,
        daemon:  geniev1connect.NewDaemonServiceClient(httpClient, baseURL, connect.WithGRPC()),
        agent:   geniev1connect.NewAgentServiceClient(httpClient, baseURL, connect.WithGRPC()),
        tool:    geniev1connect.NewToolServiceClient(httpClient, baseURL, connect.WithGRPC()),
        config:  geniev1connect.NewConfigServiceClient(httpClient, baseURL, connect.WithGRPC()),
    }
    // Liveness check
    pingCtx, cancel := context.WithTimeout(ctx, timeout)
    defer cancel()
    _, err := c.daemon.Ping(pingCtx, connect.NewRequest(&geniev1.PingRequest{}))
...
func Save(path string, entries []Entry) error {
    ...
    tmp, err := os.CreateTemp(dir, ".genie-session-*.tmp")
...
| `genie version` | REQ-CLI-001, AC-CLI-001 계승 |
| `genie ping` (unary) | 그대로 유지 (v0.1.0 AgentService/Ping) |
| `genie ask` (unary) | v0.1.0 unary 유지 + **`--stream` 옵션(기본 true)으로 ChatStream 사용** |
| cobra 구조 | 유지, subcommand 확장 |
| v0.1.0 `agent.proto` `AgentService/Chat` | 유지 + `ChatStream` 추가 |
| `genied/main.go` ~40 LoC | 유지 |
| exit code 매핑 함수 | 그대로 이전 |
| `GENIE_NO_COLOR` | 유지 (REQ-CLI-014) |
| `GENIE_SHUTDOWN_TOKEN` | 유지 (REQ-CLI-023) |
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-CORE-001 | `genied` bootstrap, exit code |
| 선행 SPEC | SPEC-GENIE-CONFIG-001 | `config.Load`, CLI flag 매핑 |
| 선행 SPEC | SPEC-GENIE-TRANSPORT-001 | proto 기본 스키마, grpc-go 서버 |
| 선행 SPEC | SPEC-GENIE-QUERY-001 | SDKMessage 스트림 소비 |
| 선행 SPEC | SPEC-GENIE-COMMAND-001 | `Dispatcher.ProcessUserInput`, `SlashCommandContext` |
| 선행 SPEC | SPEC-GENIE-TOOLS-001 | `genie tool list` 데이터 소스 |
| 후속 SPEC | SPEC-GENIE-SKILLS-001 | Skill-backed command가 TUI에서 표시됨 (Provider 등록 경로) |
| 후속 SPEC | SPEC-GENIE-MCP-001 | `genie tool list`가 MCP tool도 포함 |
| 후속 SPEC | SPEC-GENIE-SUBAGENT-001 | `/agent <name>` 명령 (추후 추가) |
| 후속 SPEC | SPEC-GENIE-HOOK-001 | permission 인터랙션 UI 통합 |
| 후속 SPEC | SPEC-GENIE-PLUGIN-001 | `plugin install/remove` stub → 실제 구현 |
...
| R10 | `/save <name>`이 TUI에서만 가능한 제약이 사용자 직관에 반함 | 중 | 낮 | `genie session save <name>`은 명시적으로 "use /save in chat" 안내. 이유: stdin으로 현재 상태 주입 불가 |
...
- `.moai/project/structure.md` §53-54 `cmd/genie/`, §228 `internal/transport/`, §374 `genie-cli` Phase 5 TS 패키지 언급 (본 SPEC과 별개)
...
- `.moai/specs/SPEC-GENIE-CLI-001/DEPRECATED.md` (v0.1.0 재작성 지시)
- `.moai/specs/SPEC-GENIE-TRANSPORT-001/spec.md` §6.2 proto 스키마, REQ-TR-005 Ping 응답
- `.moai/specs/SPEC-GENIE-QUERY-001/spec.md` §5 SDKMessage 타입, REQ-QUERY-010 ctx cancel
- `.moai/specs/SPEC-GENIE-COMMAND-001/spec.md` (동시 작성 — `ProcessUserInput`, `SlashCommandContext`)
- `.moai/specs/SPEC-GENIE-TOOLS-001/spec.md` (동시 작성 — `genie tool list` 데이터 소스)
...
- `../SPEC-GENIE-COMMAND-001/spec.md` — slash command 시스템 (TUI에서 프리디스패치)
- `../SPEC-GENIE-TOOLS-001/spec.md` — tool registry (CLI `genie tool list` 소스)
...
- 본 SPEC은 **인증 명령**(`genie login`, OAuth device flow)을 구현하지 않는다. 후속 SPEC.
...
- 본 SPEC은 **자동 update checker를 포함하지 않는다** (`genie update`). 후속.
...
- 본 SPEC은 **MCP server를 CLI에서 직접 기동하지 않는다**. MCP-001이 daemon 내부에서 관리. CLI는 `genie tool list`로 관찰만.
- 본 SPEC은 **Remote daemon 접속 전용 모드를 보장하지 않는다**. 기본 loopback. 원격 접속은 TLS/인증 도입 후 (후속 SPEC).

---

**End of SPEC-GENIE-CLI-001 v0.2.0**
