# SPEC-GENIE-CLI-001 v0.2.0 — Research & Inheritance Analysis
...
```
✅ 유지: cmd/genied/main.go, cmd/genie/main.go 구조
✅ 유지: cobra root + subcommand tree
✅ 유지: exit code 0/1/2/69/78 매핑
✅ 유지: --format json, --config, --daemon-addr, --log-level flags
✅ 유지: genie version (ldflags + GetInfo)
✅ 유지: genie ping
✅ 유지: GENIE_NO_COLOR, GENIE_SHUTDOWN_TOKEN env
```

v0.1.0 §6.4 `genied/main.go` 40 LoC 골격은 거의 그대로. TRANSPORT-001 변경 없음.
...
이는 모순이 아님 — structure.md §374의 `packages/genie-cli/` (Ink v6 TypeScript)는 별도 패키지로, 본 SPEC의 `cmd/genie` (Go bubbletea)와 **공존**. bubbletea는 Go 단일 바이너리의 TUI를 제공.
...
| Claude Code (TS/Ink) | GENIE v0.2.0 (Go/bubbletea) |
|---------------------|------------------------------|
...
crush는 bubbletea 기반 TUI 채팅 인터페이스 + 멀티 LLM + LSP 통합. **GENIE의 목표와 거의 동형**. 구현 참고 우선순위 높음.
...
2. **미래 web client 준비**: structure.md §374 `packages/genie-web/` (Next.js) 가 gRPC-Web 또는 Connect 프로토콜로 동일 daemon 호출 가능. 프로토콜 옵션만 변경.
...
client := geniev1.NewDaemonServiceClient(conn)
resp, err := client.Ping(ctx, &geniev1.PingRequest{})
```
...
client := geniev1connect.NewDaemonServiceClient(httpClient, "http://"+addr, connect.WithGRPC())
resp, err := client.Ping(ctx, connect.NewRequest(&geniev1.PingRequest{}))
...
| 행동 | Claude Code | aider | gh | crush | **GENIE v0.2.0** |
|------|------------|-------|-----|-------|------------------|
...
`~/.genie/sessions/<name>.jsonl`

- `<name>`: `[a-zA-Z0-9_-]{1,64}` (REQ-CLI-020 경로 탈출 방지).
- 특수 파일: `.last.jsonl` (Ctrl-R 대상, auto-save on exit).
- migration: Phase 3은 version 필드 없음. 향후 `{"version":2,...}` 추가 시 `genie session migrate` 명령.

---

## 7. `genie chat` vs default subcommand

### 7.1 UX 결정

- `genie` (인자 없음) → `genie chat`로 alias.
- `genie chat` → TUI.
- `genie ask <msg>` → non-interactive.

이유:
- Claude Code, aider 모두 기본 진입 TUI (인자 없이 실행 시).
- 사용자 학습비용 최소.
- `genie` alone = 가장 빈번한 용도(대화)에 최단 입력 할당.

### 7.2 Non-TTY fallback

`genie` (stdin not TTY) → stderr에 "interactive mode requires TTY, use 'genie ask' for pipe mode" + exit 2. 또는 자동으로 `ask --stdin` 대체. Phase 3 결정: **명시적 에러** (사용자 의도 불명확).

---

## 8. `genie plugin` stub 처리

PLUGIN-001은 Phase 2 완료 후 의존 SPEC. 본 SPEC 작성 시점에 PLUGIN-001 미착수 가능.

**stub 전략**:
```
genie plugin list   → stdout: "no plugins available (PLUGIN-001 pending)"
genie plugin install <source> → stderr: "genie: plugin system not yet available", exit 1
genie plugin remove <name>    → 동일
```
...
func TestE2E_AskWithMockLLM(t *testing.T) {
    ...
    out, code := runGenieCLI(t, "ask", "Hello")
...
| `cmd/genie/main.go` | 제외 |
| `cmd/genied/main.go` | 제외 |
...
5. **Session lock**: 동일 세션 파일을 두 개의 `genie` 인스턴스가 동시 로드 시 충돌. 현재는 filesystem lock 없음. 향후 `.lock` 파일 + 프로세스 ID.
6. **`genie session save`를 CLI에서 직접 실행 불가 이유 명확화**: stdin으로 messages array 받지 않음. 이유는 "TUI에서만 현재 대화 상태를 안다". 향후 `genie session import <jsonl>` 추가 검토.
...
- **이식 자산**: v0.1.0의 60% (cobra 구조, exit code, version/ping, genied bootstrap). 40%는 신규(Connect-Go, bubbletea TUI, slash command, session, tool/config/plugin subcommands).
...
GREEN 완료 시점에서:
- `genie` (no arg) → TUI REPL.
- `genie ask "msg"` → streaming response.
- `genie session save test01` (TUI 내 `/save test01`) → `~/.genie/sessions/test01.jsonl` 생성.
- `genie session load test01` → 복원 후 TUI.
- `genie tool list`, `genie config get/set/list` 동작.
- `/help`, `/clear`, `/exit`, `/model`, `/compact`, `/status`, `/version` (COMMAND-001 연동).

Phase 3 MVP(`genie ask "hello"` + TUI chat + 6 slash command)가 CLI-001 + TOOLS-001 + COMMAND-001 3개 SPEC 모두 GREEN 시점에 성립.
