...
# SPEC-GOOSE-IDENTITY-001 — Identity Graph (POLE+O 스키마, Kuzu 임베디드, Temporal Context)
...
3. **엔티티 자동 추출** — 상호작용 트랜스크립트(SPEC-GOOSE-TRAJECTORY-001 결과물)에서 NER + 규칙 기반 추출.
4. **백엔드 어댑터** — `IdentityGraph` 인터페이스 + `KuzuBackend`(기본) + `Neo4jBackend`(옵션).

본 SPEC은 `internal/learning/identity/` 패키지의 인터페이스 계약과 **관찰 가능한 행동**을 규정한다. Vector embedding 연산(§4 sibling)은 SPEC-GOOSE-VECTOR-001이, 승격/안전장치는 SPEC-GOOSE-REFLECT-001/SAFETY-001이 담당한다.
...
When `ExportJSONLD(userID)` is called, the service shall emit a valid JSON-LD 1.1 document containing all entities and facts reachable from the user node; the emitted `@context` shall reference a project-local vocabulary URI (`urn:goose:identity:v1`).
...
Where the configuration value `identity.extractor.llm_assisted = true`, the `Extractor` implementation may additionally invoke an LLM (via SPEC-GOOSE-ROUTER-001) to resolve ambiguous entity mentions; the LLM response must still pass `SHACLValidator` before persistence.
...
type TemporalFact struct {
    ...
    EpisodeID  string            // Trajectory episode 참조 (SPEC-GOOSE-TRAJECTORY-001)
