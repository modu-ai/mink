...
```
Start(ctx, deps):
  1. config.Load() → yaml + OS keyring token (zalando/go-keyring)
  2. client.New(token, deps.Logger) → Telegram API client
  3. store.New(dbPath) → sqlite store (mapping/offset)
  4. audit.New(deps.Audit) → audit wrapper
  5. handler.New(client, store, audit, chatService) → inbound dispatcher
...
주요 변경 (P2/P3):
  - deps.Memory → 독립 sqlite store (modernc.org/sqlite Option B)
  - deps.Bridge.Query() → chatService (도메인 인터페이스, gRPC Chat 어댑팅)
```
...
- **unit**: client_test.go (mock httptest), markdown_test.go (18 special chars), config_test.go (yaml + token reject), keyring_test.go (OS keyring mocking).
- **handler integration**: mock client + mock chatService + 실 sqlite (테스트 임시 DB, t.TempDir() 격리).
- **bootstrap E2E**: full Start(ctx, deps) 시나리오, mock 모든 deps 통과 (except sqlite).
...
| 항목 | 위협 | 통제 |
|-----|------|-----|
| bot token 누출 | yaml 평문 저장 | REQ-MTGM-N01 (setup reject) + OS keyring (zalando/go-keyring) 강제 |
...
| PII leak via audit log | 다른 user 정보 outbound 본문 | REQ-MTGM-N06 + audit hash 만 기록, callback_data 도 동일 정책 |
| message replay | 같은 update 재전송 | offset 영속 (REQ-MTGM-U05, sqlite WAL) |
...
- [ ] BRIDGE-001 의 `ChatService` (gRPC `AgentService/Chat` 위 도메인 인터페이스) 가 outbound caller 친화 시그니처인지 (channel agnostic 입력) — research.md §2.1 검증.
- [ ] TOOLS-001 의 register API 가 in-process 등록 (network roundtrip 없음) 인지 — research.md §2.2.
- [ ] sqlite (modernc.org/sqlite v1.50.0) 가 WAL 모드 + concurrent read 지원하는지 — 별도 setup.md 또는 config 문서.
- [ ] zalando/go-keyring v0.2.8 이 cross-platform (macOS Keychain / Windows Credential Manager / Linux Secret Service) 동작 검증.
