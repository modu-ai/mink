# SPEC-GOOSE-CONFIG-001 — Research & Inheritance Analysis
...
- 계층 순서: managed → project → user → local. 본 SPEC의 3-layer(project < user < env)와 유사하지만 "managed policy" 레이어는 GOOSE에서는 생략.
...
var envMap = []envBinding{
    {Env: "GOOSE_LOG_LEVEL",    Path: "log.level",             Kind: KindString},
    {Env: "GOOSE_GRPC_PORT",    Path: "transport.grpc_port",   Kind: KindInt},
    {Env: "GOOSE_LEARNING_ENABLED", Path: "learning.enabled",  Kind: KindBool},
    // ...
}
```
...
1. **`.goose/config.yaml` vs `goose.yaml`**: 프로젝트 로컬 config 파일명. 본 SPEC은 `.goose/config.yaml`로 고정 (dotfile). 후속 논의 시 `goose.yaml` alias 추가 가능.
...
4. **XDG Base Directory**: Linux 사용자 중 `$XDG_CONFIG_HOME/goose/`를 선호. 본 SPEC은 `$GOOSE_HOME` 단일 override 허용. XDG는 차후 SPEC.
