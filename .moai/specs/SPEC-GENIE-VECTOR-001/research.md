# SPEC-GENIE-VECTOR-001 research.md — Preference Vector Space 구현 자료
...
```go
// $GENIE_HOME/.salt 에서 64-byte random salt 로드
...
```yaml
collection_name: genie_preferences
vectors:
  size: 768
  distance: Cosine
hnsw_config:
...
| salt 유실로 cross-session 불연속 | Medium | `genie vector regenerate-salt` 명령 + 사용자 고지 |
...
- 메트릭 (Prometheus naming):
  - `genie_vector_upsert_total{backend}`
  - `genie_vector_search_duration_seconds{backend}`
  - `genie_vector_user_update_total`
  - `genie_vector_similarity_opted_in_users`
