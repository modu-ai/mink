...
# SPEC-GENIE-COMPRESSOR-001 — Trajectory Compressor (Protected Head/Tail + LLM Middle Summary)
...
GENIE-AGENT **자기진화 파이프라인의 Layer 2**를 정의한다. SPEC-GENIE-TRAJECTORY-001이 기록한 긴 궤적을, **첫 N턴(head)과 마지막 M턴(tail)을 그대로 보존**하고 **중간 영역만 LLM에게 요약**시키는 방식으로 목표 토큰 예산(기본 15,250) 이하로 압축한다. 압축 결과와 과정 메트릭(`TrajectoryMetrics`)을 모두 기록하여 INSIGHTS-001의 토큰 절감률 통계와 REFLECT-001의 품질 회귀 분석에 사용한다.
...
또한 본 SPEC의 `Compressor` 타입은 SPEC-GENIE-CONTEXT-001이 정의한 **런타임 context compaction 인터페이스의 호환 구현체**로도 주입 가능하여, Phase 0의 in-session compaction(turn 실행 중)과 Phase 4의 offline trajectory compaction(디스크 압축)이 동일 코드 경로를 공유한다.
...
- **SPEC-GENIE-CONTEXT-001**: `Compactor { ShouldCompact(State) bool; Compact(State) (State, CompactBoundary, error) }` 인터페이스. 본 SPEC은 trajectory용 별도 API를 제공하고, 옵션으로 CONTEXT-001 호환 어댑터도 제공.
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-TRAJECTORY-001 | `Trajectory`, `TrajectoryEntry`, `Role` 타입 소비 |
| 선행 SPEC | SPEC-GENIE-ROUTER-001 | `Summarizer` 구현체가 내부에서 저렴한 모델 선택 시 사용 |
| 선행 SPEC | SPEC-GENIE-ADAPTER-001 | `Summarizer.Summarize`의 실제 LLM 호출 구현 |
| 선행 SPEC | SPEC-GENIE-CORE-001 | zap 로거, context 루트 |
| 후속 SPEC | SPEC-GENIE-INSIGHTS-001 | `TrajectoryMetrics` 집계 소비 |
| 후속 SPEC | SPEC-GENIE-CONTEXT-001 | `CompactorAdapter`가 CONTEXT-001 인터페이스 만족 |
...
- `.moai/specs/SPEC-GENIE-TRAJECTORY-001/spec.md` — 입력 공급자
- `.moai/specs/SPEC-GENIE-CONTEXT-001/spec.md` — Compactor 인터페이스 정의자
...
- `../SPEC-GENIE-INSIGHTS-001/spec.md` — 후속 메트릭 소비자
...
**End of SPEC-GENIE-COMPRESSOR-001**
