# SPEC-GENIE-COMPRESSOR-001 — Research & Porting Analysis
...
- 선행 SPEC-GENIE-TRAJECTORY-001이 생성할 `Trajectory` / `TrajectoryEntry` 타입을 import 전제.
- SPEC-GENIE-ROUTER-001이 제공할 저렴한 요약 모델(Gemini 3 Flash 등) 선택 로직은 `Summarizer` 구현체 내부.
