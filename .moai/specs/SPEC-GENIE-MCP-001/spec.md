...
# SPEC-GENIE-MCP-001 — MCP Client/Server (stdio · WebSocket · SSE, OAuth 2.1, Deferred Loading)
...
GENIE-AGENT의 **Model Context Protocol (MCP) 통합**을 정의한다. Claude Code와 동일하게 genied는 **이중 역할**을 수행한다:
...
본 SPEC은 **JSON-RPC 2.0 wire 포맷의 직접 구현을 포함하지 않으며**, `modelcontextprotocol/go-sdk`(공식 Go SDK) 위에 GENIE의 인터페이스를 올리는 래퍼 층을 규정한다.
...
- **IN**: `MCPClient`, `MCPServer` 인터페이스, `Transport` 추상화 + stdio/WebSocket/SSE 3구현, 연결 풀 + memoize, Deferred tool loading, OAuth 2.1 + PKCE 플로우(브라우저 콜백 포함), 이름 충돌 해결, `MCPTool`/`MCPResource`/`MCPPrompt` 타입, credential 저장(`~/.genie/mcp-credentials/`), `mcp.json` 사용자 설정 파일 스키마, 세션 재연결 정책.
...
7. Credential 저장: `~/.genie/mcp-credentials/{server-id}.json` (file mode 0600).
8. `mcp.json` 스키마 — 사용자 설정 파일(global `~/.genie/mcp.json` + project `.genie/mcp.json`).
...
**REQ-MCP-001 [Ubiquitous]** — Every tool exposed by an external MCP server **shall** appear in GENIE's tool inventory with the namespaced name `mcp__{serverName}__{toolName}`; collisions within the same server **shall** cause `ConnectToServer` to return `ErrDuplicateMCPToolName` for that server only.
...
**REQ-MCP-003 [Ubiquitous]** — MCP credentials (OAuth tokens) **shall** be stored under `~/.genie/mcp-credentials/{server-id}.json` with file mode `0600`; the process **shall** refuse to read a credential file whose mode exceeds `0600` and **shall** log a security warning.
...
- **Then** `~/.genie/mcp-credentials/fx.json`에 `access_token`+`refresh_token` 저장(file mode 0600), 세션은 Connected
...
└── mcp/
    ...
    ├── credentials.go       # ~/.genie/mcp-credentials/ I/O
...
8. 저장: `~/.genie/mcp-credentials/{server_id}.json` file mode 0600.
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-TRANSPORT-001 | gRPC transport 기반 인터셉터 패턴(logging/recovery) 공유 |
| 선행 SPEC | SPEC-GENIE-CONFIG-001 | `mcp.json` loader 통합, feature gate |
| 선행 SPEC | SPEC-GENIE-CORE-001 | zap 로거, context 루트, graceful shutdown |
| 후속 SPEC | SPEC-GENIE-TOOLS-001 | MCP tool을 tool registry에 편입하는 consumer |
| 후속 SPEC | SPEC-GENIE-SKILLS-001 | `PromptToSkill` 결과 등록 |
| 후속 SPEC | SPEC-GENIE-SUBAGENT-001 | agent-scoped MCP 서버 초기화 |
| 후속 SPEC | SPEC-GENIE-PLUGIN-001 | plugin manifest `mcpServers:` 로딩 |
...
- `.moai/specs/SPEC-GENIE-TRANSPORT-001/spec.md` — gRPC 패턴 (인터셉터 참고)
- `.moai/specs/SPEC-GENIE-CONFIG-001/spec.md` — 설정 로더
...
- `../SPEC-GENIE-TRANSPORT-001/spec.md`
- `../SPEC-GENIE-TOOLS-001/spec.md` (Phase 3)
