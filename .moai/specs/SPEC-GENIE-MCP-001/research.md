# SPEC-GENIE-MCP-001 — Research & Porting Analysis

> **목적**: Claude Code의 MCP(Model Context Protocol) 이중 역할 시스템 → GENIE Go 포팅 계약. `.moai/project/research/claude-primitives.md` §3의 분석을 본 SPEC REQ와 1:1 매핑한다.
> **작성일**: 2026-04-21
...
**결론**: GREEN 단계는 `internal/mcp/` 9개 파일 **zero-to-one** 신규 작성 + `modelcontextprotocol/go-sdk` 위에 GENIE 인터페이스 레이어.
...
- 경로 `~/.claude/` → `~/.genie/` (프로젝트 convention).
...
- GENIE는 이 SDK 위에 wrapper 층만 유지 → GENIE 자체 인터페이스 변경 없이 MCP 버전 이동 가능.
...
- `go.mod`에 정확한 버전 pin.
- GENIE 인터페이스(`MCPClient`, `MCPServer`, `Transport`)는 SDK 타입에 종속되지 않도록 추상화.
...
- GENIE 위임 영역: wire format, message marshaling, schema check.
- GENIE 자체 구현: connection lifecycle, OAuth, credentials, mcp.json loader, namespace 충돌 해결, deferred loading 정책.
...
7. **Multi-process credential race**: `~/.genie/mcp-credentials/` file lock은 단일 플랫폼. Windows 포팅 시 `golang.org/x/sys/windows`.
