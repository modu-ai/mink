# SPEC-GOOSE-COMMAND-001 — Research & Inheritance Analysis
...
이 파일들은 Claude Code `commands/` 표준을 따르며, 본 SPEC의 custom command loader가 동일 스키마를 지원해야 GOOSE에서 재활용 가능하다.
...
외부 레포 미러 없음. tech.md/structure.md가 `cobra`를 CLI 프레임워크로 가정하나, cobra는 **OS-level** CLI 명령(`goose version`, `goose ask`)에 사용. 본 SPEC의 slash command는 **대화 내 in-band command**로 별도 계층.
...
| `$GOOSE_HOME` | env `GOOSE_HOME` | 본 SPEC 신규 |
...
- **project > user**: 현재 프로젝트의 `.goose/commands/`가 `~/.goose/commands/` 보다 우선. 프로젝트별 워크플로우 커스터마이징.
...
2. **`$GOOSE_HOME` 노출**: env 변수 치환을 환영할지 보안 위험으로 볼지. 현재는 명시 화이트리스트만(`$CWD`, `$GOOSE_HOME`). 일반 env는 거절.
...
GREEN 완료 시점에서:
- `goose` CLI에서 `/help`, `/clear`, `/exit`, `/model`, `/compact`, `/status`, `/version` 동작
- `.goose/commands/moai.md` 로드 → `/moai plan SPEC-XXX` → prompt expansion → QUERY-001로 전달 → LLM 호출
- QUERY-001 AC-QUERY-001 (user_ack 흐름) 이전에 slash command 판정 수행

Phase 3 MVP(`goose ask "hello"` → LLM 응답 + `/help` 로컬 응답)가 CLI-001과 함께 성립.
