...
version: 0.1.0
spec: SPEC-GOOSE-CROSSPLAT-001
---

# Acceptance Criteria — SPEC-GOOSE-CROSSPLAT-001
...
- [ ] `curl -fsSL https://goose.ai/install | sh`가 macOS, Linux, Windows (Git Bash)에서 정상 동작
- [ ] `irm https://goose.ai/install.ps1 | iex`가 Windows PowerShell에서 정상 동작
- [ ] `winget install ai-goose.goose`가 Windows에서 정상 동작
...
- [ ] Homebrew tap (`brew install ai-goose/tap/goose`) 동작
...
2. AI.GOOSE 바이너리 다운로드 (SHA256 검증 통과)
3. `~/.local/bin/goose` 설치 + PATH 추가
4. "Ollama not found. Installing..." → `brew install ollama` 실행
5. Ollama 서비스 시작 + 헬스체크 통과
6. "System RAM: 16 GB → Model: ai-goose/gemma4-e4b-rl-v1:q5_k_m (~4 GB)" 출력
7. `ollama pull` 진행률 표시 → 완료
8. "Detected CLI tools: claude" 출력
9. `~/.goose/config.yaml` 생성 (모델 + CLI 도구 정보)
10. "AI.GOOSE installed successfully! Run 'goose init' to get started." 출력
...
**Then**:
1. AI.GOOSE 바이너리 정상 설치
2. Ollama 설치 시도 → 실패
3. "Warning: Ollama installation failed. Install manually: https://ollama.com" 출력
4. 모델 다운로드 스킵
5. "AI.GOOSE binary installed. Configure Ollama and run 'goose init'." 출력
6. exit code 0
...
2. goose.exe 다운로드 + SHA256 검증
3. `%USERPROFILE%\bin\goose.exe` 설치
4. "Ollama already installed" 감지 → 설치 스킵
5. "System RAM: 32 GB → Model: ai-goose/gemma4-e4b-rl-v1:q8_0 (~5 GB)" 출력
6. `ollama pull` 진행률 표시 → 완료
7. "No external CLI tools detected" 출력
8. `.goose\config.yaml` 생성
...
**Then**:
1. AI.GOOSE 바이너리 이미 설치됨 감지 → 재설치 확인
...
2. "Selected model: ai-goose/gemma4-e2b-rl-v1 (2B, Q4_K_M, ~1.5 GB)" 출력
3. `ollama pull ai-goose/gemma4-e2b-rl-v1` 실행
4. 다운로드 완료 후 `ollama list`에 `ai-goose/gemma4-e2b-rl-v1` 표시
...
**Then**:
1. goose 바이너리 다운로드 + 설치
2. `goose --version` 정상 응답
3. `which goose` → Homebrew 셀러 경로
...
3. `~/.local/bin/goose` 존재 + 실행 권한
4. `~/.goose/config.yaml` 존재
