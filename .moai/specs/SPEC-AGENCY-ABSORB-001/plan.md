...
- **SPEC-AGENCY-CLEANUP-002** (가칭, 선행 스펙 6건 — spec.md Open Items "SPEC-AGENCY-CLEANUP-002 선행 스펙" 표 참조):
  - CL-1: REQ-DEPRECATE-003 deprecation window 만료 시점 확인 및 물리적 삭제 대상 목록 확정.
  - CL-2: `.agency/` legacy 디렉터리 8 파일 `moai migrate agency` 실행 또는 archive.
  - CL-3: `.claude/skills/agency-*` 5개 디렉터리 삭제(회귀 테스트 통과 후).
  - CL-4: `.claude/agents/agency/` 6 파일 삭제 결정(planner/builder/evaluator/learner 삭제, copywriter/designer는 REQ-ABSORB-009 fallback 유지 여부 별도 결정).
  - CL-5: `.claude/commands/agency/` 8 redirect stub 및 `.claude/rules/agency/constitution.md` redirect stub 삭제.
  - CL-6: CLAUDE.md §4의 "removed" 어휘를 filesystem 실제 상태와 동기화.
...
## v1.0.1 Audit Patch History (2026-04-25)

plan-audit mass-20260425 피드백(FAIL, score 0.62)에 따라 본 plan.md를 다음과 같이 갱신:

- Post-Completion Recommendations의 SPEC-AGENCY-CLEANUP-002에 6개 선행 스펙(CL-1~CL-6)을 명시하여 실행 정합성 결함(D3/D4/D5)의 해소 경로를 구체화.
- spec.md Open Items 확장·acceptance.md REQ 총 개수 정정과 일관.
- Milestone 본문·Risks·Verification Strategy는 이미 감사 PASS 항목이므로 변경 없음.
