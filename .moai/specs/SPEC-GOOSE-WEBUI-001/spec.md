...
labels: [phase-6, milestone-m6, area/webui, area/frontend, type/feature]
target_milestone: v0.2.0
mvp_status: deferred
deferred_reason: "0.1.0 MVP 범위 외 — v0.2.0 이월 (2026-05-17 사용자 확정). 0.1.0의 비개발자 채널은 ONBOARDING-001 설치 위저드 + Telegram/Slack/Discord로 충당. Web UI 전반은 v0.2.0. (단, ONBOARDING Phase 5 amendment 의 Web 설치 위저드는 0.1.0 유지). MINK-WEB-CONFIG-001 (설치 후 설정)도 0.1.0 유지."
---

# SPEC-GOOSE-WEBUI-001 — Localhost Web UI (비개발자 채널)

> v0.2 신규. 비개발자 사용자가 CLI 학습 없이 AI.MINK를 설치·운영·대화할 수 있는 localhost 기반 Web UI.
...
AI.MINK의 v0.1 Alpha 채널 3종(`mink CLI/TUI` + Telegram + 본 Web UI) 중 비개발자 진입점이 되는 **localhost 전용 브라우저 GUI**를 정의한다. 사용자는 `mink web` 서브커맨드로 daemon에 동봉된 정적 번들을 띄우고, 기본 브라우저로 `http://127.0.0.1:8787`에 접속해서 다음 흐름을 수행한다:

1. **Install wizard** — 첫 접속 시 provider key 입력 → `~/.goose/secrets/`(키링 참조)·`./.goose/config/`(설정)에 저장 → 첫 대화 smoke test.
2. **Chat 페이지** — SSE 스트림으로 LLM 응답을 토큰 단위로 받는다. AI.MINK persona·growth 단계가 voice에 반영된다.
...
- **비개발자 진입 장벽 해소**: `mink CLI`만 노출하면 친구·가족·동료가 AI.MINK를 깔아볼 길이 사실상 없다. 브라우저는 모든 OS의 공통 분모.
...
| 차원 | v0.1.0 | v0.2.0 |
|---|---|---|
| Launch | "mink web" 1줄 | port conflict / browser open 실패 / re-launch idempotency 명시 |
...
2. `cmd/goose/web.go` 서브커맨드 — `mink web [--port N] [--no-browser] [--bind 127.0.0.1]`. daemon이 안 떠 있으면 `goosed` 자동 spawn (별도 프로세스).
...
9. Brand consistency: 표기 `AI.MINK` (사용자 가시), `goose` (코드 식별자 = 백틱), 색상은 `.moai/project/brand/design-tokens.json`의 primary `#FFB800`, neutral scale, fontFamily Inter+Pretendard.
...
**REQ-WEBUI-201 [Event-Driven]** — **When** `mink web` is invoked, the CLI **shall** (a) check whether `goosed` is reachable on `cfg.Transport.HealthPort`, (b) if not, fork-spawn `goosed` and wait up to 5 s for `state=serving`, (c) resolve the configured `webui.bind_port` (default 8787), (d) open the user's default browser to `http://127.0.0.1:{port}/` unless `--no-browser` is set, and (e) print the URL to stdout regardless of `--no-browser`. (v0.1.0 REQ-WEBUI-001 launch behavior 흡수.)
...
**REQ-WEBUI-401 [Optional]** — **Where** the user supplies `--port N` to `mink web`, the server **shall** bind to `127.0.0.1:N` and `[::1]:N`; if the port is occupied the server **shall** fail with `webui.port_in_use` error (no auto-increment, explicit failure).
...
- 표기: `<h1>AI.MINK</h1>` (산문 위치), `<code>mink CLI</code>` (백틱). brand-lint 검증은 spec.md / research.md 자체에 적용 — frontend 정적 자원에는 적용되지 않으나 사용자 가시 문자열은 동일 컨벤션을 따른다 (i18n ko.json/en.json).

### 6.7 5-Phase 구현 계획 (run phase에서 수행)

| Phase | 범위 | 주요 산출물 | 의존 |
|-------|-----|----------|------|
| Phase 1 — Server skeleton + embed | `internal/webui/server.go` + embed.FS + SPA shell + `mink web` 서브커맨드 | REQ-WEBUI-101/104/201/501/502 그린 | DAEMON-WIRE-001 |
...
| **R**eadable | Korean-first UI copy (ko.json), AI.MINK notation enforcement, page-per-route 컴포넌트 단일 책임, shadcn/ui 표준 컴포넌트 |
...
**AC-WEBUI-01 — `mink web` launch end-to-end**
- **Given** `mink web` 명령 실행, daemon 미실행, port 8787 free, default browser 설정
- **When** 명령 실행 후 5 s 대기
...
- **Then** (a) 두 파일에서 `AI.MINK` 표기 사용, 부적합 패턴 (`Goose project`, `mink project`, `GOOSE-AGENT` 백틱 외부) 매치 0건, (b) 코드 식별자(`mink CLI`, `mink web`, `goosed daemon`) 는 `<code>` 태그 안에 위치, (c) `localStorage.locale=en` 설정 후 새로고침 시 모든 문자열 영문, (d) `localStorage.locale=ko` (default) 시 한국어
...
| REQ-WEBUI-201      | AC-WEBUI-01                       | mink web launch |
...
- 본 SPEC은 **AI.MINK persona의 voice/tone tuning UI를 포함하지 않는다**. persona/voice.md 편집은 후속 SPEC (CONTEXT-001 후속 amendment 후보).
