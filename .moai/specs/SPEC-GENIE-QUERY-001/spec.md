...
# SPEC-GENIE-QUERY-001 — QueryEngine 및 Query Loop (Async Streaming Agentic Core)
...
GENIE-AGENT의 **agentic 코어 런타임**을 정의한다. Claude Code의 async generator 기반 query loop를 Go로 포팅하여, 하나의 대화 세션 = 하나의 `QueryEngine` 인스턴스라는 생명주기 계약 위에서 streaming 응답, tool 실행, permission gating, budget tracking, compaction trigger yield를 통합한다.
...
본 SPEC은 코드 본체(`internal/query/`, `internal/query/loop/`, `internal/message/`, `internal/permissions/`)의 **인터페이스 계약과 관찰 가능 행동**을 규정한다. Compaction 전략의 세부 알고리즘은 SPEC-GENIE-CONTEXT-001에, Tool 레지스트리 자체 구현은 SPEC-GENIE-TOOLS-001에 위임한다.
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-CORE-001 | `genied` 데몬 + zap 로거 + context 루트 |
| 후속 SPEC | SPEC-GENIE-CONTEXT-001 | `Compactor`, `ToolUseContext`, `AutoCompactTrackingState` 실 구현 |
| 후속 SPEC | SPEC-GENIE-TOOLS-001 | `tools.Registry`, `tools.Executor` 실 구현 |
| 후속 SPEC | SPEC-GENIE-ADAPTER-001 | `LLMCall` 함수 타입의 실제 Anthropic/OpenAI 구현 |
| 후속 SPEC | SPEC-GENIE-SUBAGENT-001 | `TeammateIdentity`, `CanUseTool` teammate 정책 |
| 후속 SPEC | SPEC-GENIE-HOOK-001 | `PostSamplingHooks`, `StopFailureHooks` 이벤트 dispatcher |
| 후속 SPEC | SPEC-GENIE-COMMAND-001 | `processUserInput`의 slash command 분기 |
| 후속 SPEC | SPEC-GENIE-TRANSPORT-001 | `<-chan SDKMessage` → gRPC stream 변환 |
...
- `.moai/project/research/claude-core.md` §1 Core Loop, §3 TS 원문 인터페이스, §4 Go 매핑, §5 재사용 평가, §6 설계 원칙 10가지, §7 SPEC-GENIE-QUERY-001 초안, §8 고리스크 영역
...
- `.moai/specs/SPEC-GENIE-CORE-001/spec.md` — 선행 SPEC (데몬 생명주기)
...
- `../SPEC-GENIE-CONTEXT-001/spec.md` — 후속 Compactor 구현
