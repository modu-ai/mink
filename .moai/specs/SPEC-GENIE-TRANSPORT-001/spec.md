...
# SPEC-GENIE-TRANSPORT-001 — gRPC 서버/proto 스키마 기본 계약
...
`genied` 데몬이 외부 클라이언트(CLI, Desktop, Web)와 대화할 **gRPC 서버의 기본 계약과 proto 스키마 최소 집합**을 정의한다. CORE-001이 확보한 HTTP `/healthz`를 그대로 두되, 본 SPEC은 별도 포트에서 **gRPC listener**를 추가 바인딩한다.

수락 조건 통과 시점에서 grpcurl 또는 generated Go client가:

- `genie.v1.DaemonService/Ping` 호출 → `PingResponse{version, uptime_ms, state}` 회신
- `genie.v1.DaemonService/GetInfo` 호출 → 빌드 메타(commit, goVersion, buildTime) 회신
- `genie.v1.DaemonService/Shutdown` 호출 (with auth token) → daemon이 graceful shutdown 개시
...
- CLI-001(genie CLI)은 gRPC로 `genied`에 접속. LLM-001 역시 daemon 내부에서 동작하므로 전송을 직접 쓰지 않지만, 향후 MCP 서버 노출 또는 외부 세션 시 gRPC 경유 필요.
- proto 스키마가 없으면 CLI와 daemon이 ad-hoc JSON으로 붙게 되어 tech.md §3.3(proto 표준)의 원칙 위배.

### 2.2 범위 경계

- **IN**: `proto/genie/v1/daemon.proto`의 최소 `DaemonService`, `grpc-go` 서버 초기화, reflection 지원(개발자 편의), interceptor 체인(logging + recovery), insecure TCP(localhost 전제), graceful stop 시 CORE-001 shutdown hook과 연동.
...
- gRPC listener는 **별도 포트** (기본 `:17891`, ENV `GENIE_GRPC_PORT`).
- CORE-001의 HTTP health server는 그대로 유지. gRPC server 내부에서도 health checking protocol(`grpc.health.v1`)을 구현하여 gRPC 클라이언트가 `Check("genie.v1.DaemonService")`로 상태 조회 가능.
...
1. `proto/genie/v1/daemon.proto` 파일 생성 (package `genie.v1`).
...
6. `grpc.reflection.v1alpha` reflection 서비스 등록 (DEBUG/개발 빌드에서만; `GENIE_GRPC_REFLECTION=true` env로 제어).
7. Shutdown RPC 인증: 초기 단순 static token(ENV `GENIE_SHUTDOWN_TOKEN`). 없으면 Shutdown RPC 비활성.
...
**REQ-TR-001 [Ubiquitous]** — The gRPC server **shall** bind to `127.0.0.1` by default and only allow loopback connections unless `GENIE_GRPC_BIND` is explicitly set to a non-loopback interface.
...
**REQ-TR-003 [Ubiquitous]** — The proto package name **shall** be `genie.v1` and the Go package path **shall** be `github.com/genieagent/genie/internal/transport/grpc/gen/geniev1`.
...
**REQ-TR-006 [Event-Driven]** — **When** a client invokes `DaemonService/Shutdown` with a valid `auth_token` header matching `GENIE_SHUTDOWN_TOKEN`, the server **shall** reply `ShutdownResponse{accepted: true}` and then trigger the CORE-001 root context cancellation within 100ms after response is flushed.
...
**REQ-TR-009 [State-Driven]** — **While** `GENIE_GRPC_REFLECTION` is unset or `false`, the reflection service **shall not** be registered on the server.
...
**REQ-TR-011 [Unwanted]** — **If** `GENIE_SHUTDOWN_TOKEN` is empty or unset, **then** the server **shall** register `Shutdown` RPC as disabled (returning `codes.Unimplemented`) — never accept shutdown via RPC in that mode.

**REQ-TR-012 [Unwanted]** — The server **shall not** accept connections over plaintext HTTP/2 from non-loopback addresses; any connection attempt from a non-loopback peer while `GENIE_GRPC_BIND=127.0.0.1` **shall** be rejected by the listener.
...
**REQ-TR-014 [Optional]** — **Where** `GENIE_GRPC_MAX_RECV_MSG_BYTES` is set to a positive integer, the server **shall** apply that value as `grpc.MaxRecvMsgSize` instead of the default `4 MiB`.
...
- **Given** `genied` state=`serving`, gRPC server bound to random port (test harness)
...
- **When** `Check(ctx, &HealthCheckRequest{Service: "genie.v1.DaemonService"})`
- **Then** `Status == SERVING`

**AC-TR-003 — Shutdown 토큰 없이 거부**
- **Given** `GENIE_SHUTDOWN_TOKEN=secret`, client metadata에 `auth_token` 헤더 미포함
...
- **Given** `GENIE_SHUTDOWN_TOKEN=secret`, client metadata `auth_token=secret`
- **When** `Shutdown(ctx, &ShutdownRequest{})`
...
- **Given** `GENIE_GRPC_REFLECTION` 미설정
...
- **Given** `GENIE_GRPC_BIND=0.0.0.0`은 유효(명시적 opt-in), 그러나 기본값 `127.0.0.1`인 상태
...
├── gen/geniev1/                # buf-generated *.pb.go, *_grpc.pb.go (do not edit)
└── *_test.go
...
syntax = "proto3";
package genie.v1;
option go_package = "github.com/genieagent/genie/internal/transport/grpc/gen/geniev1;geniev1";
...
$ buf generate      # writes internal/transport/grpc/gen/geniev1/
```
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | **SPEC-GENIE-CORE-001** | shutdown hook, state accessor, zap logger |
| 선행 SPEC | **SPEC-GENIE-CONFIG-001** | `TransportConfig.GRPCPort` 소비 |
| 후속 SPEC | SPEC-GENIE-LLM-001 | LLM RPC 추가 정의 |
| 후속 SPEC | SPEC-GENIE-AGENT-001 | Agent RPC 추가 정의 |
| 후속 SPEC | SPEC-GENIE-CLI-001 | generated Go client 소비 |
...
| # | 리스크 | 가능성 | 영향 | 완화 |
|---|------|------|-----|------|
| R1 | proto 변경이 파괴적(breaking)으로 발전 | 중 | 높 | `buf breaking` CI gate, `genie.v1` 패키지 불변, v2 필요 시 신규 디렉토리 |
...
- `.moai/specs/SPEC-GENIE-CORE-001/spec.md` §3.2 OUT OF SCOPE (gRPC는 본 SPEC)
...
**End of SPEC-GENIE-TRANSPORT-001**
