...
# SPEC-GENIE-CONTEXT-001 — Context Window 관리 및 Compaction 전략
...
`QueryEngine`(SPEC-GENIE-QUERY-001)의 매 iteration이 API 호출 전에 의존하는 **context 계층**을 정의한다. 본 SPEC은 다음 세 요소를 묶어 하나의 일관된 계약으로 제공한다:
...
본 SPEC은 코드 본체(`internal/context/`)의 **인터페이스 계약과 관찰 가능 행동**을 규정하고, `QueryEngine`의 iteration 루프가 호출하는 3개 함수(`ShouldCompact`, `Compact`, `Get*Context`)의 서명을 확정한다. LLM-기반 요약의 실제 호출은 SPEC-GENIE-COMPRESSOR-001이 구현하고, 본 SPEC은 `Summarizer` 인터페이스의 consumer로만 참여한다.
...
- SPEC-GENIE-QUERY-001의 continue site `after_compact` 경로는 `Compactor.ShouldCompact/Compact`를 호출하도록 설계되었다. 이 인터페이스가 구현되지 않으면 QUERY-001의 AC-QUERY-011, AC-QUERY-006은 통과할 수 없다.
...
- CORE-001이 `genied` 데몬을 띄우고 QUERY-001이 loop 골격을 만든 직후 즉시 필요. Phase 0의 마지막 퍼즐 조각 중 하나.
...
**REQ-CTX-016 [Optional]** — **Where** environment variable `GENIE_HISTORY_SNIP=1` is set, the `Snip` strategy **shall** be preferred over `AutoCompact`/`ReactiveCompact` even when `Summarizer` is available; this feature gate allows deterministic snip-only mode for debugging.
...
SPEC-GENIE-QUERY-001 §6.2의 `QueryEngineConfig.Compactor`는 다음 메서드 셋을 요구:
...
| 선행 SPEC | SPEC-GENIE-QUERY-001 | `Compactor` 인터페이스 서명, `loop.State`, `message.Message` |
| 후속 SPEC | SPEC-GENIE-COMPRESSOR-001 | `Summarizer.Summarize` 실 구현 (LLM 요약) |
| 후속 SPEC | SPEC-GENIE-ADAPTER-001 | provider `usage.input_tokens`로 token count 보정 |
| 후속 SPEC | SPEC-GENIE-CONFIG-001 | `GENIE_HISTORY_SNIP`, `TokenLimit`, `ProtectedHead/Tail` 설정값 |
...
- `.moai/project/research/claude-core.md` §3 (TS 인터페이스: getSystemContext/getUserContext memoize), §4 (포팅 매핑), §6 원칙 5·10, §7 SPEC-GENIE-CONTEXT-001 초안, §8 리스크(budget undercount)
...
- `.moai/specs/SPEC-GENIE-QUERY-001/spec.md` — Compactor 인터페이스 요구 원천
...
- `../SPEC-GENIE-QUERY-001/spec.md` — 인터페이스 원천
- `../SPEC-GENIE-QUERY-001/research.md` — State.TaskBudget 계약 원천
