...
# SPEC-GENIE-ADAPTER-001 — 6 Provider 어댑터 (Anthropic/OpenAI/Google/xAI/DeepSeek/Ollama)
...
GENIE-AGENT Phase 1의 **LLM HTTP 호출 경계**를 정의한다. QUERY-001의 `LLMCallFunc` 인터페이스에 대한 6개 provider 구현(Anthropic, OpenAI, Google Gemini, xAI Grok, DeepSeek, Ollama)을 공통 `Provider` interface 아래 통합하는 `internal/llm/provider` 패키지를 구현한다.
...
- QUERY-001 `LLMCall` interface 수신측 구현 계약이 본 SPEC에서 확정되어야 Phase 1 MVP Milestone 1(`genie ask "hello"` → Claude/GPT 응답)이 동작.
...
**REQ-ADAPTER-016 [Unwanted]** — The adapter **shall not** perform any disk write outside of `~/.genie/credentials/` (CREDPOOL) and `~/.claude/.credentials.json` (Anthropic token sync, write-back only after successful refresh).
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-QUERY-001 | `LLMCallFunc` 시그니처 수신측 구현, `message.Message` / `StreamEvent` 타입 |
| 선행 SPEC | SPEC-GENIE-CREDPOOL-001 | `CredentialPool`, `Refresher` interface |
| 선행 SPEC | SPEC-GENIE-ROUTER-001 | `Route` 소비, `ProviderRegistry` 메타 참조 |
| 선행 SPEC | SPEC-GENIE-RATELIMIT-001 | 응답 헤더를 `Tracker.Parse`로 전달 |
| 선행 SPEC | SPEC-GENIE-PROMPT-CACHE-001 | Anthropic 어댑터가 `Plan` 소비 |
| 선행 SPEC | SPEC-GENIE-CORE-001 | zap logger, context 루트 |
| 선행 SPEC | SPEC-GENIE-CONFIG-001 | `LLMConfig.Providers[*]` |
| 후속 SPEC | SPEC-GENIE-TOOLS-001 | `tool.Definition` 소비, tool_use StreamEvent 전달 |
| 후속 SPEC | SPEC-GENIE-ERROR-CLASS-001 | HTTP 에러 분류(14 FailoverReason) 연계 |
...
- `.moai/specs/SPEC-GENIE-QUERY-001/spec.md` §6.2 `LLMCall` interface
- `.moai/specs/SPEC-GENIE-CREDPOOL-001/spec.md` §6.2 `Refresher` interface
- `.moai/specs/SPEC-GENIE-ROUTER-001/spec.md` §6.2 `Route`
- `.moai/specs/SPEC-GENIE-RATELIMIT-001/spec.md` §6.2 `Tracker.Parse`
- `.moai/specs/SPEC-GENIE-PROMPT-CACHE-001/spec.md` §6.2 `Plan`
