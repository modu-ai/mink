...
MINK 런타임은 **로컬 우선(local-first) 하이브리드 검색 엔진**을 통해 Plan Phase의 context retrieval, PAI identity 검색, skill 의미 검색, Task/Ritual 결과 조회를 수행한다. 본 SPEC은 Tobias Lütke가 공개한 [tobi/qmd](https://github.com/tobi/qmd)의 Rust 포트인 [qntx-labs/qmd](https://github.com/qntx-labs/qmd)를 **CGO staticlib로 링크**하여 `goosed` 단일 바이너리 안에 임베드한다.
...
MINK는 두 종류의 지식 표현을 병행한다. 혼동 방지를 위해 명시적으로 분리한다.
...
- **IN (본 SPEC의 전체 구현 범위)**: `./.goose/`의 마크다운 인덱싱, Go 측 공개 API (`qmd.Index`, `qmd.Query`, `qmd.Reindex`, `qmd.Watch`), MCP stdio 서버, GGUF 모델 자동 다운로드, fsnotify 기반 자동 재인덱스, CLI 서브커맨드(`mink qmd ...`), CGO staticlib 빌드 파이프라인. **각 기능의 런타임 활성화 마일스톤(M1/M3/M4)은 §3.1 및 §8 Rollout을 정규 레퍼런스로 따른다**(예: fsnotify watcher는 M1 코드 포함, M3 런타임 활성; MCP stdio 서버는 M1 코드 포함, M4 공식 공개).
...
7. **증분 재인덱스 + fsnotify watcher**: 파일 content hash 비교로 변경 감지. `mink qmd reindex`를 통한 **수동 증분 재인덱스는 M1**에서 동작. **fsnotify watcher 자동 트리거(debounce 500ms) 런타임 활성화는 M3**. watcher 구현 코드와 단위 테스트는 M1에 포함되지만, `qmd.Watch()`는 M1 빌드에서 `stop`만 반환하고 이벤트 루프는 M3에서 활성화된다. [Code M1, Runtime M3]
8. **MCP stdio 서버**: JSON-RPC 2.0 (LSP-style). 메서드: `qmd/query`, `qmd/reindex`, `qmd/stats`. `mink qmd mcp` 서브커맨드와 framing/methods 구현 및 단위 테스트는 M1에 포함되지만, **외부 MCP 클라이언트 공개(문서화된 stdio 인터페이스로서의 공식 노출)는 M4**에서 이뤄진다. M1 빌드에서는 내부 통합 테스트 용도로만 사용한다. [Code M1, Runtime M4]
9. **CLI 서브커맨드**: `mink qmd reindex`, `mink qmd query <text>`, `mink qmd stats`는 M1. `mink qmd mcp`(stdio 서버 실행)은 코드 기준 M1, 공식 런타임 공개 M4. `mink qmd models verify` / `models download`는 M1. [Code M1, Runtime: reindex/query/stats M1, mcp M4]
...
**REQ-QMD-005 [Event-Driven]** — **When** `mink qmd reindex [path]` is invoked, the system **shall** perform a full reindex of the given path (or all configured roots if omitted) and emit a progress log every 1000 documents processed.
...
- **Given** `mink qmd mcp` 를 별도 프로세스로 실행 (M4 런타임; M1~M3에서는 테스트 harness가 내부 API로 동등 경로 호출)
...
- **Given** `mink qmd reindex` 실행 중 (진행률 50%)
...
- **When** `mink qmd reindex ./.goose/test-link` 실행
...
- **When** `./.goose/data/models/bge-small-en-v1.5.gguf` 부재 상태로 `mink qmd reindex` 실행
...
```
/ (repo root)
├── cmd/mink/main.go              # CLI 엔트리, `qmd` 서브커맨드 라우팅
...
```
mink qmd reindex [path]       # 전체 또는 특정 경로 재인덱스
mink qmd query <text> [-k N]  # 검색 실행 (debug용)
mink qmd stats                # 인덱스 통계 출력
mink qmd mcp                  # stdio MCP 서버 실행 (외부 agent 연결용)
mink qmd models verify        # 모델 SHA256 검증
mink qmd models download      # 모델 강제 재다운로드
```
...
- `mink qmd reindex` 수동 명령만 지원. `mink qmd query`, `mink qmd stats`, `mink qmd models verify`, `mink qmd models download` 활성.
- Agent Core는 Plan Phase에서 `qmd.Query` 호출.
- fsnotify watcher **코드 존재, 런타임 비활성** (`qmd.watcher.enabled: false`).
- MCP 서버 **코드 존재, 런타임 비활성** (`qmd.mcp.enabled: false`). `mink qmd mcp` 명령은 M1에서 "M4에서 활성화됩니다" 메시지 출력 후 exit code 0으로 종료. (단위 테스트용 내부 호출은 허용.)
...
### 8.3 Phase 3 (M4)
- MCP stdio 서버 외부 공개 (`mink qmd mcp`).
...
- `test/integration/qmd_mcp_test.go`: 자식 프로세스로 `mink qmd mcp` 실행 → stdin/stdout 왕복
