---
id: SPEC-GENIE-PROMPT-CACHE-001
version: 0.1.0
status: Planned
created: 2026-04-21
updated: 2026-04-21
...
# SPEC-GENIE-PROMPT-CACHE-001 — Prompt Caching (system_and_3 Strategy, 4 Breakpoint, TTL)
...
- GENIE의 multi-turn agentic loop(QUERY-001의 queryLoop)는 동일 system prompt + tool schema를 매 turn 반복 전송. Caching 없으면 턴마다 동일 내용을 재청구.
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-QUERY-001 | `message.Message` 타입 정의 |
| 선행 SPEC | SPEC-GENIE-ROUTER-001 | ADAPTER-001이 Route에 따라 caching 적용 여부 결정 |
| 후속 SPEC | SPEC-GENIE-ADAPTER-001 | Anthropic 어댑터가 CachePlan을 소비 |
| 외부 | Go 1.22+ | |
...
- `.moai/specs/SPEC-GENIE-ROUTER-001/spec.md` — adapter가 provider 분기
- `.moai/specs/SPEC-GENIE-QUERY-001/spec.md` — `message.Message` 타입
...
**End of SPEC-GENIE-PROMPT-CACHE-001**
