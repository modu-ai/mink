# SPEC-GOOSE-CORE-001 — Research & Inheritance Analysis

> **목적**: `goosed` 데몬 부트스트랩 구현에 재활용 가능한 자산과 재작성이 필요한 영역을 식별한다. Explore 에이전트를 사용하지 않고 본 작성자가 직접 grep/glob/ls로 레포를 스캔한 결과이다.
> **작성일**: 2026-04-21
...
- `cobra` — CLI subcommand 없음 (goosed는 데몬 단일 진입점). CLI-001(goose CLI)에서 도입.
...
- `TestGoosed_BootstrapsAndServesHealthz` — AC-CORE-001.
- `TestGoosed_SigtermTriggersGracefulExit` — AC-CORE-002.
- `TestGoosed_MalformedConfigExits78` — AC-CORE-003.
- `TestGoosed_PortInUseExits78` — AC-CORE-004 (pre-bound listener 주입).
- `TestGoosed_DrainingState_Returns503` — AC-CORE-006.
...
2. **헬스 endpoint 포트 기본값**: 제안 `17890` (goose의 모바일 키패드 "GOOS" 매핑). 다른 GOOSE 프로세스와 충돌 시 `GOOSE_HEALTH_PORT` env로 override.
