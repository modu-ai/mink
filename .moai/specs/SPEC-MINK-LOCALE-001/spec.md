...
| 0.4.0 | 2026-05-16 | **Phase 2 구현 완료**. amendment-v0.2 의 6 신규 AC (AC-LC-020~025) 가 모두 GREEN. 3 PR 머지: (1) PR #223 backend probe endpoint + IP geolocation + Accuracy 필드 — `internal/server/install/handler.go` `localeProbe` 핸들러 + `internal/locale/iplookup.go` ipapi.co 어댑터, (2) PR #225 CLI auto-detect — `--no-auto-detect` cobra flag + `internal/locale/detect.go` `DetectOptions`/`DetectWithOptions` entry point + `CLINoticeText` 상수 (AC-LC-022 regex strict 매칭), (3) PR #224 frontend + reverse geocoding — `Step1Locale.tsx` 자동 감지 흐름 (GPS → IP fallback → manual) + `useGeolocation` hook + `InstallApi.probeLocale` + `internal/locale/reverse.go` Nominatim 어댑터 (handler.go GPS branch wiring). Privacy notice 표시 + `accuracy` 배지 (high/medium/manual). Test 결과: Go race-detector PASS, Vitest 6/6 PASS, Playwright Web speedrun PASS. 새 npm/Go 모듈 도입 0 (amendment-v0.2 약속 준수). Phase 1 종결물 (REQ-LC-001~016 / AC-LC-001~018) 회귀 0. frontmatter version 0.3.0 → 0.4.0, status in-progress → implemented. Phase 2 follow-up Known Limitations (China GFW 실 device 검증 / 다국적 secondary language 자동 감지 / Web reverse geocoding 정확도 성/시 단위 검증) 는 amendment-v0.3 후보로 progress.md 에 기록. | expert-frontend + expert-backend |
