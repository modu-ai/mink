...
- **Status**: 🟡 draft — Phase 1A + 1B + 1C (paths + progress) 완료, Phase 1C-keyring / 1D / 1E / 1F / 2 / 3 / 4 잔여
- **Last update**: 2026-05-15
- **Spec version**: v0.3.1 (amendment-v0.3 본문 병합 후)
- **Doc commits**: SPEC 생성 (`a2b3551`) → MINK rebrand (`81d9fa4`) → amendment-v0.3 본문 병합 (본 PR)
- **Implementation commits**: 3 (Phase 1A PR #200 / Phase 1B PR #201 / Phase 1C PR #202)
...
| **Phase 1C-keyring**: keyring.go + zalando/go-keyring | ⏸️ | 150-250 LOC | OS keyring abstraction (KeyringClient interface) + SetProviderKey/GetProviderKey/DeleteProviderKey. prefix `mink.provider.{name}.api_key`. 신규 의존성 zalando/go-keyring v0.2.6. |
...
---
Last Updated: 2026-05-16 (Phase 1C paths + draft persistence + spec §6.0 Canonical Path Policy — internal/onboarding/{paths,progress,paths_test,progress_test}.go 989 LOC + spec.md v0.3.2 + 56 sub-tests PASS, 누적 114 tests)
