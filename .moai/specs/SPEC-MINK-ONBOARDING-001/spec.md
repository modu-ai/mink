...
labels: [onboarding, cli, web-ui, installer, wizard, localization, mink-rebrand]
---

# SPEC-MINK-ONBOARDING-001 — CLI + Web UI Install Wizard

> **REBRAND FROM SPEC-GOOSE-ONBOARDING-001 (2026-05-14)**: 본 SPEC 은 SPEC-MINK-BRAND-RENAME-001 (commit f0f02e4, 2026-05-13) + SPEC-MINK-ENV-MIGRATE-001 (v0.2.0 completed) + SPEC-MINK-USERDATA-MIGRATE-001 (v0.2.0 completed) 의 정책에 따라 선행 SPEC-GOOSE-ONBOARDING-001 (v0.2.1, draft) 의 본문을 MINK 로 rebrand 한 후속 SPEC 이다. CLI 명령 `mink init` → `mink init`, 디렉토리 `./.goose/` → `./.mink/` (구현 시 USERDATA-MIGRATE-001 의 alias loader 패턴 위임). 선행 SPEC body 는 immutable 로 유지되며, 선행 SPEC frontmatter status 는 별도 commit 에서 `superseded` 로 전환된다.

> **v0.2 Amendment (2026-04-24, propagated 2026-04-25, 선행 SPEC 기록)**: SPEC-GOOSE-ARCH-REDESIGN-v0.2 반영.
...
> **초기 설정 범위**: `./.mink/` 디렉터리 생성 → `persona/soul.md` 입력 → provider key 저장 (OS keyring) → 첫 messenger 채널 활성화.
...
| 버전 | 날짜 | 변경 사유 | 담당 |
|-----|------|---------|------|
| 0.1.0 | 2026-04-22 | 초안 작성 (선행 SPEC-GOOSE-ONBOARDING-001). v5.0 ROADMAP Phase 6 Localization 시리즈 4번째(최종). 사용자 지시: "설치시 사용자의 국가와 정보를 수집해서 사용자에 맞게". Desktop App 첫 실행 시 8단계 UX로 locale + identity + daily pattern + interests + ritual + LLM provider + privacy 수집. | manager-spec |
| 0.2.0 | 2026-04-25 | (선행 SPEC) SPEC-GOOSE-ARCH-REDESIGN-v0.2 Amendment 전파. 스코프를 Desktop Tauri 8-step → CLI(`mink init`) + Web UI 설치 마법사로 축소. REQ-OB-013/REQ-OB-018 [DEPRECATED v0.2] 표시. 5개 미커버 REQ 에 대응하는 AC 신설(AC-OB-017~020). | manager-spec |
| 0.2.1 | 2026-05-14 | (선행 SPEC) POST-BRAND-RENAME marker 추가. labels 에 `post-brand-rename` 추가. | manager-spec |
| 0.3.0 | 2026-05-14 | SPEC-MINK-BRAND-RENAME-001 + ENV-MIGRATE-001 + USERDATA-MIGRATE-001 rebrand 정책 적용. 본문 MINK/Goose/goose 명칭을 MINK/Mink/mink 로 치환 + CLI `mink init` → `mink init` + workspace path `./.goose/` → `./.mink/`. id `SPEC-GOOSE-ONBOARDING-001` → `SPEC-MINK-ONBOARDING-001` 신설, 선행 SPEC supersede. 다른 SPEC 의 cross-reference (SPEC-GOOSE-CONFIG-001 / SPEC-GOOSE-LOCALE-001 / SPEC-GOOSE-I18N-001 / SPEC-GOOSE-REGION-SKILLS-001 / SPEC-GOOSE-ARCH-REDESIGN-v0.2) 는 immutable 보존. labels 에 `mink-rebrand` 추가. | manager-spec |
...
| 0.3.2 | 2026-05-16 | Phase 1C canonical path policy 명문화 + 충돌 ACs 정정. §6.0 신설 — `~/.mink/` vs `./.mink/` 경계 명문화. AC-OB-007/012/018 의 provider 메타데이터 config 대상을 `~/.mink/config.yaml` 로 정정. AC-OB-027 의 7-Step 통합 config 분기 (model+delegation → `~/.mink`, persona+messenger+consent → `./.mink`). REQ-OB-009 persist 대상 분기 명시. REQ-OB-027 전역/프로젝트 분리 참조 추가. | manager-spec |
...
- `./.mink/config.yaml` (providers, locale, consent flags)
- `./.mink/persona/soul.md`
- OS keyring 항목: `mink.provider.{name}.api_key`
- `./.mink/onboarding-completed` 타임스탬프 파일
...
Amendment가 설정한 scope("`./.mink/` 생성 → persona → provider key → 첫 messenger 채널")를 충족하는 최소 단계:
...
한 단계 스킵 시 후속 기능이 저하되지만, **전 단계 모두 스킵해도 `./.mink/` 초기화는 완료**되어야 한다 (MINK를 일단 띄우는 것이 최우선).

### 2.3 branding.md §5.1 First Day with MINK

> "Hatching 🐣 — 알에서 깨어나는 최초의 만남. Mink가 사용자를 발견하고 각인(imprinting)하는 순간."
...
12. **완료 처리**:
    ...
    - `~/.mink/config.yaml`에 전역 데이터 통합 (model, delegation, providers) + `./.mink/config.yaml`에 프로젝트 스코프 데이터 분리 저장 (persona, messenger, consent) — §6.0 Canonical Path Policy
13. **4분 목표 수행 측정**:
...
**REQ-OB-005 [Event-Driven]** — **When** the user invokes `mink init` in a fresh environment (no `./.mink/config.yaml` exists or `onboarding_completed` is absent) **or** opens the Web UI install wizard (`mink serve --install-wizard`), the system **shall** start the onboarding flow at Step 1 rather than the main application shell.
...
**REQ-OB-007 [Event-Driven]** — **When** Step 3 (Provider) is submitted with an API key, the backend **shall** store the secret in the OS keyring via `zalando/go-keyring` under entry `mink.provider.{name}.api_key` and **shall not** write plaintext secrets to `./.mink/config.yaml`.
...
**REQ-OB-009 [Event-Driven]** — **When** the user completes Step 7, the backend **shall** (a) persist project-scoped data (persona, messenger, consent) to `./.mink/config.yaml` and global data (model, delegation, provider key source) to `~/.mink/config.yaml` per §6.0 Canonical Path Policy (CONFIG-001), (b) call REGION-SKILLS-001 to activate country skills, (c) seed Identity Graph initial nodes, (d) write `./.mink/onboarding-completed` timestamp file, and (e) exit the wizard (CLI returns to shell; Web UI signals server shutdown and displays "return to terminal").
...
**REQ-OB-011 [State-Driven]** — **While** the user is mid-onboarding and quits (CLI: Ctrl+C; Web UI: tab closed), the collected data (partial) **shall** be persisted to `./.mink/onboarding-draft.yaml`, and re-running `mink init --resume` or reopening the Web UI **shall** resume from the last completed step.
...
**REQ-OB-017 [Unwanted]** — **If** Step 2 (Persona) is submitted with a `name` field containing 500+ characters or shell/HTML injection patterns (per regex `[<>&{};|$]` or length > 500), **then** the backend **shall** reject the submission and log a security event to `./.mink/security-events.log`.
...
**When** the onboarding completes (Step 7 Submit), the backend **shall** persist all collected settings split across two files per §6.0 Canonical Path Policy: `~/.mink/config.yaml` (global half) receives model selection (from Step 2), CLI tool availability (from Step 3), and provider configuration (from Step 5); `./.mink/config.yaml` (project-scoped fields are written to `./.mink/config.yaml` per §6.0) receives persona (from Step 4), messenger channel (from Step 6), and consent flags (from Step 7).
...
- **Given** fresh install, `./.mink/config.yaml` 없음
...
- **When** Next 실행
- **Then** OS keyring에 `mink.provider.anthropic.api_key` 저장, 전역 config(`~/.mink/config.yaml`)에는 `providers.anthropic.api_key_source: keyring` 만 저장(평문 없음). provider keyring 메타데이터는 전역 설정에 해당하므로 `~/.mink/config.yaml`에 기록한다(§6.0 Canonical Path Policy).
...
- **When** Step 5 Submit
- **Then** REGION-SKILLS-001이 `korean-holidays`, `kakao-talk`, `jondaetmal-etiquette` 자동 활성. CONFIG-001 `skills.region.active` 배열에 3개 ID 등록. `./.mink/onboarding-completed` 파일 생성

**AC-OB-010 — 완료 메시지 + Imprinting 이벤트 (verifies REQ-OB-009, REQ-OB-019)**
- **Given** 온보딩 완료
- **When** Step 5 Submit 후
- **Then** CLI는 ASCII 아트 "🐣 → 🐥" 전환과 "Mink is ready. Run `mink start`." 메시지(prefers reduced motion 시 ASCII 전환 생략 텍스트만). Web UI는 egg→mink transition과 "터미널로 돌아가세요" 메시지(reduced motion 시 fade only). 한국어 사용자에게는 "안녕하세요, [이름]님!" 인사 적용
...
- **Then** `./.mink/onboarding-draft.yaml`에 Step 1~2 데이터 존재, UI는 Step 3부터 재시작

**AC-OB-012 — LLM provider 스킵 (verifies REQ-OB-010)**
- **Given** Step 3에서 Skip
- **When** Step 4로 진행
- **Then** 전역 config `~/.mink/config.yaml`에 `llm.default_provider: "unset"` 저장(provider 설정은 전역 설정에 해당, §6.0 Canonical Path Policy). 온보딩 완료 후 첫 `mink start` 실행 시 "LLM 공급자를 설정해주세요" 비차단 배너/알림 표시
...
- **When** Next 실행
- **Then** 폼 검증 에러 "Invalid API key format for Anthropic (expected prefix `sk-ant-`)" 표시, OS keyring에 미기록(keyring 항목 조회 시 `ErrNotFound`), `~/.mink/config.yaml`에도 미기록(provider keyring 메타데이터는 전역 설정, §6.0)
...
- **Given** 전체 7-Step 완료
- **When** Step 7 Submit
- **Then** §6.0 Canonical Path Policy에 따라 두 파일에 분리 저장:
  - model + delegation 데이터는 `~/.mink/config.yaml` 에, persona + messenger + consent 데이터는 `./.mink/config.yaml` 에 분리 저장 (§6.0 Canonical Path Policy)
  - `~/.mink/config.yaml`에는 다음 항목 포함(전역 절반 — install-time 탐지 결과 + provider 메타데이터):
  ```yaml
  model:
  ...
  ```
  - `./.mink/config.yaml`에는 다음 항목 포함(프로젝트 스코프 절반):
  ```yaml
  persona:
    name: "User"
...
### 6.0 Canonical Storage Path Policy **[CLARIFIED in Phase 1C — 2026-05-16]**

본 절은 MINK가 파일을 저장하는 두 루트 디렉터리(`~/.mink/`와 `./.mink/`)의 경계를 명문화한다. 이전 버전의 AC/REQ에서 두 경로가 혼용된 경우가 있었으며, Phase 1C에서 아래 정책으로 통일한다.

#### 전역 설정 디렉터리 (`~/.mink/`)

사용자 단위로 단 하나 존재하며, 재설치·다중 프로젝트에서도 공유되는 데이터를 보관한다.

| 항목 | 설명 |
|------|------|
| `config.yaml` | 설치 시 탐지 결과(model, delegation.available_tools), provider keyring 메타데이터(`providers.{name}.api_key_source`), 사용자 수준 기본 messenger |
| `audit.log` | 향후 감사 로그 (선택적) |

`~/.mink/config.yaml`은 `install.sh`(SPEC-MINK-CROSSPLAT-001 M2/M5)가 초기 기록하며, 온보딩 완료(Step 7 Submit) 시 model + delegation 항목이 갱신된다.

#### 프로젝트 작업 디렉터리 (`./.mink/`)

각 프로젝트 루트에 하나씩 존재하며, 해당 프로젝트에 국한된 데이터를 보관한다. `ProjectConfigDir()`는 `os.Getwd()` 에서 상위 디렉터리로 올라가며 `.mink/` 디렉터리를 탐색한다(upward traversal).

| 항목 | 설명 |
|------|------|
| `config.yaml` | persona, messenger, consent 등 프로젝트 스코프 설정 |
| `persona/soul.md` | MINK 페르소나 정의 파일 |
| `memory/`, `skills/`, `tasks/` | 프로젝트 런타임 데이터 |
| `onboarding-draft.yaml` | 중단된 온보딩 마법사 상태 (진행 중만 존재) |
| `onboarding-completed` | 타임스탬프 파일 (온보딩 완료 마커) |
| `security-events.log` | REQ-OB-017 위반 이벤트 기록 (0640 권한) |

#### 설정 병합 규칙

- **읽기**: 프로젝트 설정(`./.mink/config.yaml`)이 전역 설정(`~/.mink/config.yaml`)보다 우선 적용된다.
- **온보딩 완료 시 쓰기(Step 7 Submit)**:
  - `model`, `delegation` (설치 시 탐지 결과) → `~/.mink/config.yaml`
  - `persona`, `messenger`, `consent` (프로젝트 스코프) → `./.mink/config.yaml`
- **AC-OB-027 "통합 config"**의 `~/.mink/config.yaml`은 전역 절반(model, delegation, providers)을 가리키며, persona/messenger/consent는 `./.mink/config.yaml`에 분리 저장된다.
- **draft 파일**은 프로젝트 스코프(`./.mink/onboarding-draft.yaml`)이므로, 여러 프로젝트가 각자의 중단된 마법사를 독립적으로 보유할 수 있다.
...
- 검증 통과 시에만 `keyring.Set("mink.provider."+provider+".api_key", key)`
...
| **T**rackable | 각 step 소요 시간 익명 로그(opt-in 후에만), 완료율 추적, `./.mink/onboarding-draft.yaml` atomic write로 상태 복원 |
