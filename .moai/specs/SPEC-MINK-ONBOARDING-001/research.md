# SPEC-MINK-ONBOARDING-001 — Research Notes
...
- **기본값 품질**: Duolingo는 기본값이 너무 공격적이라 조정률 낮음 → MINK는 보수적 기본값
- **각인 효과**: 이름을 묻는 단계는 "나를 기억한다"는 느낌을 줘 engagement 상승

### 1.3 MINK의 차별화

- **부화 애니메이션**: 경쟁사 대부분은 "완료!" 토스트만. MINK는 감성적 연결.
- **LOCALE 강조**: 대부분 영어 위주. MINK는 Step 2에서 사용자 언어로 전환.
...
```
MINK processes your data to provide the Daily Companion service.
...
2. 수집 목적: MINK Daily Companion 서비스 제공, 개인화
3. 보유 기간: 사용자가 탈퇴 또는 삭제 요청 시까지
4. 동의 거부 권리: 거부할 경우 일부 기능 제한됨

※ MINK는 주민등록번호, 금융정보, 생체정보 등 민감정보를 수집하지 않습니다.
...
**필수 요소**:
- 수집 카테고리
- "Do not sell my personal information" 옵션 (MINK는 판매 안 함)
...
MINK does not sell your personal information.

Your rights:
- Right to know
- Right to delete
- Right to opt-out (N/A for MINK)

☐ I acknowledge the privacy practices of MINK.
```

### 3.5 LGPD (브라질), FZ-152 (러시아), APPI (일본)

- LGPD: GDPR 유사, 포르투갈어
- FZ-152: 데이터 현지화 조항, 러시아 영토 내 저장 권고 (MINK는 로컬 저장이므로 자연 준수)
- APPI: 처리 목적 + 제3자 제공 여부 (MINK는 제3자 제공 없음)
...
- 임시 파일 `~/.mink/.onboarding-draft.yaml.tmp` → rename
...
**End of research.md for SPEC-MINK-ONBOARDING-001**
