...
# SPEC-GENIE-HOOK-001 — Lifecycle Hook System (24 Events + useCanUseTool 권한 플로우)
...
**REQ-HK-019 [Optional]** — **Where** `ENV GENIE_HOOK_TRACE=1` is set, the dispatcher **shall** emit DEBUG-level trace logs including full `HookInput` + `HookOutput` JSON for every handler invocation.
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-QUERY-001 | `DispatchPreToolUse`/`DispatchPostSampling` consumer |
| 선행 SPEC | SPEC-GENIE-CORE-001 | zap 로거, context 루트, fsnotify watcher |
| 후속 SPEC | SPEC-GENIE-SKILLS-001 | `FileChangedConsumer` 등록 |
| 후속 SPEC | SPEC-GENIE-SUBAGENT-001 | `SubagentStart/Stop/TeammateIdle` dispatcher, CoordinatorHandler |
| 후속 SPEC | SPEC-GENIE-MCP-001 | `PostToolUse.updatedMCPToolOutput` 수신 |
| 후속 SPEC | SPEC-GENIE-PLUGIN-001 | `PluginHookLoader` 구현 |
| 후속 SPEC | SPEC-GENIE-CLI-001 | `InteractiveHandler` 구현 |
...
| R7 | 비-TTY 환경 탐지가 CI 환경에서 false positive | 중 | 낮 | `GENIE_HOOK_NON_INTERACTIVE=1` env override 제공. CI 설정 권장 |
...
- `.moai/specs/SPEC-GENIE-QUERY-001/spec.md` — `PostSamplingHooks`, `StopFailureHooks` entrypoint
- `.moai/specs/SPEC-GENIE-SKILLS-001/spec.md` — `FileChangedConsumer` 계약
...
- 본 SPEC은 **Hook handler cross-process dispatch를 지원하지 않는다**. 단일 genied 프로세스 내.

---

**End of SPEC-GENIE-HOOK-001**
