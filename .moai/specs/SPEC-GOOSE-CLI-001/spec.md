...
# SPEC-GOOSE-CLI-001 — mink CLI (cobra + Connect-gRPC + bubbletea TUI)
...
AI.MINK의 **사용자 대면 CLI**를 정의한다. v0.1.0은 Phase 0에서 최소 cobra subcommand만 제공했으나, ROADMAP v2.0(2026-04-21)이 본 SPEC을 Phase 3으로 재배치하며 아래 3가지 축으로 확장한다:
...
- `goose` (인자 없음 또는 `mink chat`) → TUI REPL 진입.
- `mink ask "msg"` / `mink ask --stdin < file` → non-interactive, stdout에 응답 스트리밍.
- `mink session list/load/save/rm` → 세션 파일 관리 (`~/.goose/sessions/<name>.jsonl`).
- `mink config get/set/list` → 설정 조작 (CONFIG-001 RPC 위임).
- `mink tool list` → `Registry.ListNames()` 출력 (TOOLS-001 소비).
- `mink plugin list/install/remove` → PLUGIN-001 미구현 시 stub "not yet available".
...
1. **Phase 3 의존성 정렬**: MVP Milestone 1이 `TOOLS-001 → CLI-001`를 요구. CLI-001이 Phase 0에 있으면 `mink tool list` 같은 핵심 UX 명령이 누락.
...
- **OUT**: Auth(`mink login`), plugin 실제 설치(PLUGIN-001), web/mobile/desktop 클라이언트, Windows 정식 지원, 자동 update checker.
...
1. **2개 실행파일**:
   ...
   - `cmd/mink/main.go`: 사용자 CLI. cobra root + subcommand 트리, ~15 LoC (로직은 `internal/cli/`).
...
3. **Subcommand 트리**:
   ...
   └── daemon
       ├── status
       └── shutdown              # MINK_SHUTDOWN_TOKEN 필요
   ```
...
- **인증 명령**(`mink login`, OAuth 흐름): 후속 SPEC.
...
- **자동 update checker** (`mink update`): 후속.
...
**REQ-CLI-001 [Ubiquitous]** — Both `goose` and `goosed` binaries **shall** expose `--version` or `version` subcommand that prints `mink version <semver> (commit <short>, built <iso>)` to stdout and exits 0. (v0.1.0 REQ-CLI-001 계승).
...
**REQ-CLI-006 [Event-Driven]** — **When** `goose` is invoked with no subcommand (or `mink chat`), the CLI **shall** enter bubbletea TUI REPL, subscribing to `AgentService/ChatStream` and rendering streamed `SDKMessage` deltas progressively in the messages viewport.

**REQ-CLI-007 [Event-Driven]** — **When** `mink ask "<message>"` is invoked, the CLI **shall** open a `ChatStream`, send the message as the first stream request, and write the concatenated assistant text to stdout followed by a single newline; the stream **shall** be closed within 30 seconds default (override via `--timeout`).
...
**REQ-CLI-011 [Event-Driven]** — **When** `mink session load <name>` is invoked, the CLI **shall** read `~/.goose/sessions/<name>.jsonl`, parse each line as an SDKMessage/Message, pass them as `ChatStream` initial messages, and enter TUI mode with viewport populated.
...
**REQ-CLI-013 [State-Driven]** — **While** the daemon reports state `draining` via the Ping response, `mink ask` and `mink chat` **shall** return exit code 69 with message `daemon is draining`. (v0.1.0 REQ-CLI-008 계승).

**REQ-CLI-014 [State-Driven]** — **While** `MINK_NO_COLOR=1` is set or stdout is not a TTY, the CLI **shall** disable ANSI color codes in all output (including TUI — fallback to minimal rendering). (v0.1.0 REQ-CLI-009 계승).
...
**REQ-CLI-016 [Unwanted]** — **If** the user supplies no arguments to `mink ask` and no `--stdin` flag, **then** cobra **shall** print usage and exit with code 2. (v0.1.0 REQ-CLI-010 계승).
...
**REQ-CLI-023 [Optional]** — **Where** `MINK_SHUTDOWN_TOKEN` is set and the user invokes `mink daemon shutdown`, the CLI **shall** call `DaemonService/Shutdown` with that token as `auth_token` metadata. (v0.1.0 REQ-CLI-015 계승).
...
**AC-CLI-010 — `mink session save/load` 왕복**
- **Given** TUI session에 user + assistant 1쌍 메시지 존재
- **When** `/save test01` 실행 후 TUI 종료. 이후 `mink session load test01`
- **Then** 새 TUI 시작 시 viewport에 이전 2개 메시지 복원, `~/.goose/sessions/test01.jsonl` 파일 2줄

**AC-CLI-011 — `mink tool list`**
...
| `mink version` | REQ-CLI-001, AC-CLI-001 계승 |
| `mink ping` (unary) | 그대로 유지 (v0.1.0 AgentService/Ping) |
| `mink ask` (unary) | v0.1.0 unary 유지 + **`--stream` 옵션(기본 true)으로 ChatStream 사용** |
...
| `MINK_NO_COLOR` | 유지 (REQ-CLI-014) |
| `MINK_SHUTDOWN_TOKEN` | 유지 (REQ-CLI-023) |
...
| 선행 SPEC | SPEC-GOOSE-TOOLS-001 | `mink tool list` 데이터 소스 |
| 후속 SPEC | SPEC-GOOSE-SKILLS-001 | Skill-backed command가 TUI에서 표시됨 (Provider 등록 경로) |
| 후속 SPEC | SPEC-GOOSE-MCP-001 | `mink tool list`가 MCP tool도 포함 |
...
| R10 | `/save <name>`이 TUI에서만 가능한 제약이 사용자 직관에 반함 | 중 | 낮 | `mink session save <name>`은 명시적으로 "use /save in chat" 안내. 이유: stdin으로 현재 상태 주입 불가 |
...
- `.moai/specs/SPEC-GOOSE-TOOLS-001/spec.md` (동시 작성 — `mink tool list` 데이터 소스)
...
- `../SPEC-GOOSE-TOOLS-001/spec.md` — tool registry (CLI `mink tool list` 소스)
...
- 본 SPEC은 **인증 명령**(`mink login`, OAuth device flow)을 구현하지 않는다. 후속 SPEC.
...
- 본 SPEC은 **자동 update checker를 포함하지 않는다** (`mink update`). 후속.
...
- 본 SPEC은 **MCP server를 CLI에서 직접 기동하지 않는다**. MCP-001이 daemon 내부에서 관리. CLI는 `mink tool list`로 관찰만.
