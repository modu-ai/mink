...
- **Test scenario**: Given the parent environment populated with `ANTHROPIC_API_KEY=xyz`, `OPENAI_API_KEY=abc`, `GOOSE_AUTH_TOKEN=zzz`, `MY_TOKEN=t`, `PASSWORD=p`, `HARMLESS=keep`; and an `InlineCommandHandler{Command: "env"}`; When the subprocess is started; Then the captured stdout includes `HARMLESS=keep` and excludes every variable whose name matches the §6.11 deny-list.
...
| 명시적 이름 | `ANTHROPIC_API_KEY`, `OPENAI_API_KEY`, `GOOSE_AUTH_*` (glob; `GOOSE_AUTH_TOKEN`, `GOOSE_AUTH_REFRESH` 등 모두 포함) |
