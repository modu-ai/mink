# SPEC-GOOSE-QUERY-001 — Research & Porting Analysis
...
- CORE-001이 수립할 `internal/core/`, `cmd/goosed/`만 선행 존재 가정.
...
| Claude Code (TS) | GOOSE (Go) 패키지 | 결정 근거 |
|---|---|---|
...
- **다음 단계 선행 요건**: SPEC-GOOSE-CONTEXT-001의 `Compactor` 인터페이스 서명이 본 SPEC §6.2와 동일해야 함. 두 SPEC이 동시 작성되므로 인터페이스 교차 검증이 GREEN 직전 작업.
