...
- **성공 정의**: AC-QUERY-001 ~ 012(12개) 전부 GREEN + `go test -race` 무경보 + `golangci-lint` 무경고 + 가중 평균 coverage ≥ 85% (`internal/query` 90%+, `internal/query/loop` 92%+, research.md §8.5 기준).
...
|---------|-----|----------|----------|------|
| T0.1 | 타입 골격 | `internal/message/message.go`, `internal/message/sdk_message.go`, `internal/message/stream_event.go` | — | `Message`, `ContentBlock`, `ToolResult`, `SDKMessage{Type,Payload}`, `StreamEvent` 타입 선언만 (메서드 미구현). Type enum 9개 (spec.md §6.2). |
...
| T1.2 | `TestSDKMessage_TypeSwitchExhaustive` | `SDKMessage` payload 구조체 9종 + exhaustive type-switch helper | T0.1 | REQ-QUERY-002, REQ-QUERY-007 매핑 |
...
| T3.3 | `TestQueryEngine_SubmitMessage_Returns_Within_10ms` (stub: `LLMCall` 이 100ms sleep 후 응답) | goroutine spawn, 연결 dial 을 goroutine 내부로 이동 | T3.2 | REQ-QUERY-016, R7 검증 |
...
### 단계 S8 — Middleware / Teammate / Permission inbox 완성 (REQ-QUERY-013/018/020)

| Task ID | Test (RED) | 구현 (GREEN) | 의존 task | 매핑 REQ |
|---------|-----------|-------------|----------|---------|
| T8.1 | `TestQueryLoop_PostSamplingHooks_FIFO_MutateMessage` | `PostSamplingHooks` FIFO 적용 훅 체인 | T4.1 | REQ-QUERY-018 |
| T8.2 | `TestQueryEngine_ResolvePermission_ResumesLoop` | `ResolvePermission(toolUseID, Behavior)` inbox 전달 + loop 재개 | T4.2 | REQ-QUERY-013 |
| T8.3 | `TestQueryLoop_TeammateIdentity_InjectedIntoSystemPromptAndMetadata` | `TeammateIdentity` 전파 2경로 | T3.3 | REQ-QUERY-020 |

**S8 완료 기준**: optional/suspend 계열 REQ 전부 GREEN.
...
| `internal/message/sdk_message.go` | `SDKMessage`, 9개 Type enum, payload 구조체 | T0.1, T1.2 |
...
- [ ] Integration test 12개(AC-QUERY-001~012) 전부 GREEN (build tag `integration`)
...
| Integration | `go test -tags=integration ./internal/query/...` | AC-QUERY-001 ~ 012 GREEN | T9.3 |
...
1. spec.md §5 의 AC-QUERY-001 ~ 012 전부 integration test GREEN.
