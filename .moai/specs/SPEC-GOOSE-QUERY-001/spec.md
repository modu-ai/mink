...
AI.MINK의 **agentic 코어 런타임**을 정의한다. Claude Code의 async generator 기반 query loop를 Go로 포팅하여, 하나의 대화 세션 = 하나의 `QueryEngine` 인스턴스라는 생명주기 계약 위에서 streaming 응답, tool 실행, permission gating, budget tracking, compaction trigger yield를 통합한다.
...
**REQ-QUERY-006 [Event-Driven]** — **When** the LLM response contains a `tool_use` content block, the `queryLoop` **shall** invoke `CanUseTool(ctx, toolName, input, ToolPermissionContext)` and dispatch based on the returned `PermissionBehavior`: `Allow` → yield a `permission_check{tool_use_id, behavior: "allow"}` `SDKMessage` then execute via `tools.Executor`, `Deny` → yield a `permission_check{tool_use_id, behavior: "deny", reason}` `SDKMessage` then synthesize a `ToolResult{is_error: true, content: "denied: <reason>"}`, `Ask` → yield a `permission_request{tool_use_id, tool_name, input}` `SDKMessage` and suspend the loop until a resolution arrives on the engine's permission inbox (REQ-QUERY-013). *(감사 review-1 D6 채택 A: Allow/Deny 경로도 `permission_check` SDKMessage 를 yield 하여 AC-002/AC-003 관찰 기대와 정합. `SDKMsgPermissionCheck` 타입은 §6.2 enum 에 추가.)*
...
**REQ-QUERY-008 [Event-Driven]** — **When** the API returns a `max_output_tokens` terminal event, the `queryLoop` **shall** (a) increment `State.maxOutputTokensRecoveryCount`, (b) if the counter is ≤ 3, re-issue the API call with the same messages array and no modifications, (c) if > 3, transition to `Terminal{success: false, error: "max_output_tokens_exhausted"}`. Additionally, at the `after_compact` continue site (REQ-QUERY-009), the `queryLoop` **shall** reset `State.maxOutputTokensRecoveryCount` to 0 because the post-compaction context is materially different and deserves a fresh retry budget. *(감사 review-1 M2: R5 완화 행동을 normative REQ 로 승격.)*
...
**REQ-QUERY-010 [Event-Driven]** — **When** `ctx.Done()` fires (caller aborts via `context.Context` cancellation), the `queryLoop` **shall** (a) stop consuming LLM chunks, (b) release any pending tool permissions, (c) yield a `Terminal{success: false, error: "aborted"}` `SDKMessage` on the output channel, and (d) close the output channel, all within 500ms. *(단계 순서 교정 (감사 review-1 D7): 닫힌 채널에 Terminal 을 yield 할 수 없으므로 yield → close 순서를 강제. AC-QUERY-008 의 "마지막 SDKMessage 가 Terminal" 기대와 정합.)*
...
### 4.4 Unwanted Behavior / Ubiquitous Prohibition (방지 및 상시 금지)

> **라벨 체계 주기**: 감사 review-1 (D4) 에 따라 REQ-015, REQ-016 은 본래 EARS Unwanted 패턴(`If ... then ...`)이 아닌 **Ubiquitous prohibition (상시 `shall not` 불변식)** 이다. REQ 번호를 안정적으로 유지(acceptance.md/plan.md/compact 참조 보존)하기 위해 §4.4 섹션에 두되 라벨을 `[Ubiquitous]`로 교정한다. REQ-014, REQ-017 은 진정한 Unwanted 패턴이며 `[Unwanted]` 유지.
...
**REQ-QUERY-015 [Ubiquitous]** — The `QueryEngine` **shall not** write to `State.messages` from goroutines other than the `queryLoop` goroutine owning the current iteration; all state transitions **shall** occur in the single loop goroutine to eliminate lock-based synchronization. *(라벨 교정: 원래 Ubiquitous prohibition. §4.1 의 4개 불변식과 동일한 상시 금지 성격이나 `shall not` 문장을 한 곳에 모으기 위해 §4.4 에 배치.)*

**REQ-QUERY-016 [Ubiquitous]** — The `QueryEngine.SubmitMessage` **shall not** block the caller for longer than 10ms before returning the output channel, even if LLM connection setup is slow (connection dial happens inside the `queryLoop` goroutine). *(라벨 교정: Ubiquitous 성능 불변식. 트리거 조건 없는 상시 계약이므로 Unwanted 가 아닌 Ubiquitous.)*
...
- **When** `SubmitMessage`
- **Then** 채널 순서에 `permission_check{tool_use_id, behavior:"deny", reason:"destructive"}` SDKMessage가 yield된 후, tool은 실제 실행되지 않고, `ToolResult{is_error:true, content:"denied: destructive"}`가 다음 API call의 messages에 포함되며, 대화는 계속 진행됨(terminal은 두 번째 assistant turn 후 success)
...
- **Given** `QueryEngineConfig.TaskBudget = {total: 50}`, 스텁 LLM이 1턴차에 60 units를 소비 (총 예산 초과 소비 유발)
- **When** `SubmitMessage` drain
- **Then** 1턴 완료 시점에 `remaining` 이 `-10` (음수) 으로 차감된 후, 2턴차 iteration 시작 시 REQ-QUERY-011 의 `remaining <= 0` gate 가 발동하여 `Terminal{success:false, error:"budget_exceeded"}`
...
**AC-QUERY-013 — Ask permission suspend / resume (REQ-QUERY-013)**
- **Given** 스텁 LLM 이 첫 응답에서 `tool_use{name:"deleteFile", input:{"path":"/tmp/x"}}` 블록을 반환하고, `StubCanUseTool` 이 `Decision{Behavior:Ask, Reason:"destructive op"}` 반환. `StubLLMCall` 은 payload recorder 로 모든 호출 기록. 테스트는 별도 goroutine 에서 `ResolvePermission(toolUseID, Allow)` 를 호출할 수 있도록 준비.
- **When** 테스트가 `SubmitMessage(ctx, "del file")` 을 호출하여 채널을 drain 시작. 이후 `permission_request{tool_use_id, tool_name, input}` `SDKMessage` 를 수신한 다음, 외부 goroutine 에서 `engine.ResolvePermission(toolUseID, Allow)` 호출.
- **Then** (a) `ResolvePermission` 호출 전까지 loop 는 tool 을 실행하지 않고 추가 LLM call 도 시도하지 않는다 (payload recorder 의 LLM call 건수가 suspend 기간 동안 1건으로 고정됨을 확증), (b) resolve 후 loop 가 재개되어 tool 실행 → `tool_result` 합성 → 두 번째 LLM call 수행, (c) 두 번째 LLM call payload 의 messages[] 에 `tool_result` 포함, (d) 최종 `Terminal{success:true}`. **Edge case**: `ResolvePermission(toolUseID, Deny)` 로 해결하면 `ToolResult{is_error:true, content:"denied: destructive op"}` 가 합성되고 대화는 계속된다.

**AC-QUERY-014 — SubmitMessage 10ms 마감 (REQ-QUERY-016)**
- **Given** 스텁 LLM 이 goroutine 내부 초기화(connection dial 시뮬레이션)에 100ms 를 소요하도록 구성. `QueryEngine.New(cfg)` 로 엔진 생성은 이미 완료된 상태.
- **When** 테스트가 `t0 := time.Now(); ch, _ := engine.SubmitMessage(ctx, "hi"); t1 := time.Now()` 로 호출 경계 시각을 측정. N=1000 반복.
- **Then** 각 호출에서 `t1 - t0 ≤ 10ms`. p99 기준으로도 ≤ 10ms 보장. 반환된 채널 `ch` 는 이후 stub 의 100ms 지연이 끝난 시점부터 정상적으로 메시지를 수신하여 최종 `Terminal{success:true}` 로 종료. 성능 회귀 방지를 위해 go testing 의 `testing.B` 또는 `time.Now` 기반 직접 측정 허용.

**AC-QUERY-015 — PostSamplingHooks FIFO chain (REQ-QUERY-018)**
- **Given** `QueryEngineConfig.PostSamplingHooks = []MessageHook{h1, h2}` 등록. `h1` 은 샘플된 `Message.content` 의 말미에 `" [h1]"` 문자열을 append 하고, `h2` 는 `" [h2]"` 를 append. 스텁 LLM 이 `Message{role:"assistant", content:"ok"}` 를 yield.
- **When** `SubmitMessage(ctx, "hi")` drain.
- **Then** queryLoop 가 sampled `Message` 를 tool 파싱 단계로 넘기기 전 hook 체인을 FIFO 순서로 적용하여 내부 Message.content 가 `"ok [h1] [h2]"` 로 변형된다 (관찰 방법: `PostSamplingHooks` 내부 spy 또는 다음 LLM call 의 messages[] 에 누적 반영되는 content 확인). **관찰 범위 명시**: 외부로 yield 되는 `SDKMessage{type:"message"}` 의 원본 보존 여부는 본 SPEC 범위 밖 — 테스트는 "hook 체인 적용 결과가 tool 파싱과 State.messages 에 반영된다" 는 최소 계약만 검증.

**AC-QUERY-016 — TeammateIdentity 주입 (REQ-QUERY-020)**
- **Given** `QueryEngineConfig.TeammateIdentity = &TeammateIdentity{AgentID:"spec-ga-01", TeamName:"alpha"}` 구성. `StubLLMCall` 에 payload recorder 활성.
- **When** `SubmitMessage(ctx, "hi")` 호출 후 채널 drain.
- **Then** (a) outbound LLM call payload 의 system 파트에 `{"agent_id":"spec-ga-01","team_name":"alpha"}` 구조화 헤더가 포함됨 (payload recorder 가 기록한 첫 API call 의 system content 검증), (b) 관찰된 모든 `SDKMessage` 의 `meta` 필드(또는 동등한 metadata 경로)에 동일한 `{agent_id:"spec-ga-01", team_name:"alpha"}` 포함. **Edge case**: `TeammateIdentity == nil` 인 기본 경로에서는 system header 주입 0, metadata 의 teammate 필드 부재 (네거티브 검증).
...
const (
    ...
    SDKMsgPermissionRequest  SDKMessageType = "permission_request"  // Ask 분기 (REQ-QUERY-006/013)
    SDKMsgPermissionCheck    SDKMessageType = "permission_check"    // Allow/Deny 분기 (REQ-QUERY-006, 감사 D6)
...
**turnCount 증분 모델 (감사 review-1 D5 정의)**:

`State.TurnCount` 는 "완료된 iteration 의 수" 를 의미한다. 하나의 iteration 은 다음 중 하나의 경로로 완료된다:
- **(A) Pure assistant turn**: LLM 이 tool_use 없이 `stop` 으로 응답 완료 (assistant terminal)
- **(B) Tool roundtrip turn**: LLM 이 tool_use 블록을 반환하여 `CanUseTool` 게이트 + tool 실행 + tool_result 합성까지 완료

두 경로 모두 iteration 완료 시점에 `turnCount++` 가 발생한다. 따라서 AC-QUERY-001(tool 없는 1회 응답, 경로 A) 과 AC-QUERY-002(tool 1회 roundtrip + 후속 assistant 응답, 경로 B 1회 + 경로 A 1회) 의 turnCount 값이 각각 1, 2 로 성립한다.

| Continue Site | 트리거 | 새 State 구성 |
|---|---|---|
| `after_compact` | `Compactor.ShouldCompact == true` | messages 치환, `autoCompactTracking` 갱신, `taskBudget.Remaining` 누적 보존, `maxOutputTokensRecoveryCount` → 0 (REQ-QUERY-008 reset 조항) |
| `after_retry` | `max_output_tokens` 수신, counter ≤ 3 | `maxOutputTokensRecoveryCount++`, messages 불변, **`turnCount` 증분 없음** (동일 iteration 재시도) |
| `after_tool_results` | tool 실행 완료 (경로 B iteration 종료) | messages 에 `user` role tool_result block append, `turnCount++`, `taskBudget.Remaining -= tokens_used` |
| `after_assistant_terminal` | LLM 이 tool_use 없이 `stop` 으로 응답 완료 (경로 A iteration 종료) | messages 에 assistant message append, `turnCount++`, `taskBudget.Remaining -= tokens_used`. 이 continue site 는 별도의 next-iteration 분기를 요구하지 않는 **terminal 경로** 이므로 실제로는 iteration 루프 종료 직전에 counter 만 갱신하는 지점으로 동작 (Continue.Reason 별도 상수는 생성하지 않고 loop body 내부 inline 처리; REQ-QUERY-003 의 "continue site" 집합은 State 재할당 3곳으로 제한 유지) |

**요약**: `State` 재할당 경로(continue site)는 3곳으로 불변이나, `turnCount` 증분은 iteration 완료 시점(경로 A의 assistant terminal 또는 경로 B의 after_tool_results)에 발생한다. `after_retry` 는 iteration 내부 재시도이므로 증분 없음.
...
5. **RED #5**: `TestQueryLoop_MaxOutputTokens_Retries3ThenFails` — AC-QUERY-005.
...
13. **RED #13**: `TestQueryLoop_AskPermission_SuspendResume` — AC-QUERY-013 (REQ-QUERY-013).
14. **RED #14**: `TestQueryEngine_SubmitMessage_Returns10ms` — AC-QUERY-014 (REQ-QUERY-016).
15. **RED #15**: `TestQueryLoop_PostSamplingHooks_FifoChain` — AC-QUERY-015 (REQ-QUERY-018).
16. **RED #16**: `TestQueryEngine_TeammateIdentity_InjectedEverywhere` — AC-QUERY-016 (REQ-QUERY-020).
17. ... (REQ-QUERY-015 같은 방어 계약은 race detector + state mutation 경로 테스트로).
18. **GREEN**: 모든 RED 확인 후 `internal/query/*`, `internal/message/*`, `internal/permissions/*` 최소 구현.
19. **REFACTOR**: continue site를 별도 파일(`continue_site.go`)로 추출, LLMCall 어댑터 seam 정리, middleware chain 일반화.
