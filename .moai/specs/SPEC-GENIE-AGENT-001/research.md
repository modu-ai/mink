# SPEC-GENIE-AGENT-001 — Research & Inheritance Analysis
...
- **YAML 선택 이유**: structure.md §1 `agents/*.yaml`로 이미 명문화. MoAI의 Markdown agent는 Claude Code 전용 개념이므로 GENIE의 런타임 agent와 용어 구분.
- 향후 MoAI agents를 GENIE가 읽을 필요가 생기면 adapter를 별도 SPEC에서 추가.
...
`default.yaml`은 embed → 유저가 `$GENIE_HOME/agents/default.yaml`을 override해도 작동.
