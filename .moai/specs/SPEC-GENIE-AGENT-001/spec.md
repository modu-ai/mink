...
# SPEC-GENIE-AGENT-001 — Agent Runtime 최소 생애주기 + Persona
...
GENIE-AGENT 대화 루프의 **최소 Agent Runtime과 Persona 모델**을 정의한다. 본 SPEC은 Tool Registry(Tool-001), Memory(MEMORY-001), Learning Engine(REFLECT-001) 등 후속 SPEC의 consumer가 접속할 **인터페이스 계약**과 **단일 LLM 호출 루프**를 확정한다.

수락 조건 통과 시점에서:

- YAML로 정의된 `Agent` persona를 로드한다 (`agents/*.yaml`, CORE-001 `GENIE_HOME` 하위 또는 프로젝트 로컬).
...
agents/                   # $GENIE_HOME/agents/ (런타임 탐색)
...
system_prompt: |
  You are GENIE, a helpful personal assistant.
...
- `$GENIE_HOME/agents/*.yaml` + 레포 builtin `embed.FS`를 모두 스캔, 중복 name 시 $GENIE_HOME 우선.
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | **SPEC-GENIE-CORE-001** | zap logger, context 전파 |
| 선행 SPEC | **SPEC-GENIE-CONFIG-001** | `Config` 일부 — agents dir 경로 등 |
| 선행 SPEC | **SPEC-GENIE-LLM-001** | `LLMProvider`, `StreamReader`, `Usage` 소비 |
| 후속 SPEC | SPEC-GENIE-TOOL-001 | `ToolInvoker` 구현체 |
| 후속 SPEC | SPEC-GENIE-MEMORY-001 | Conversation persistence |
| 후속 SPEC | SPEC-GENIE-TELEM-001 | `InteractionObserver` 구현체 |
| 후속 SPEC | SPEC-GENIE-FEEDBACK-001 | observer 체인 연결 |
| 후속 SPEC | SPEC-GENIE-CLI-001 | `AskStream` 소비자 |
...
- `.moai/specs/SPEC-GENIE-LLM-001/spec.md` §3.1 IN SCOPE 항목 1-6
