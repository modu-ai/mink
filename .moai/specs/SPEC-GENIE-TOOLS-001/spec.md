...
# SPEC-GENIE-TOOLS-001 — Tool Registry 및 ToolSearch (Deferred Loading, Inventory)
...
GENIE-AGENT의 **Tool 실행 인프라 계층**을 정의한다. `SPEC-GENIE-QUERY-001` §3.2(OUT)에서 인터페이스만 선언한 `tools.Registry` 및 `tools.Executor`를 본 SPEC이 완성하며, Claude Code의 deferred-loading `ToolSearch` 메커니즘(`.moai/project/research/claude-primitives.md` §3.4)과 Hermes `cli.py` / `model_tools.py`의 **auto-registry inventory 패턴**을 Go로 포팅한다.
...
- **MVP Milestone 1 블로커**: ROADMAP §7 "MVP Milestone 1 — 동작하는 에이전트"의 필수 경로는 `TOOLS-001 → CLI-001`. 본 SPEC이 완료되어야 `genie ask "list files in current dir"` → `Glob tool 실행` → 결과 반환이 end-to-end로 동작.
...
- **기존 SPEC-GENIE-AGENT-001 (v1.0, DEPRECATED)**: 일부 tool 시그니처 아이디어만 참고.
...
- **MCP transport**(stdio/WS/SSE) / OAuth / connection lifecycle: SPEC-GENIE-MCP-001.
...
**REQ-TOOLS-016 [Unwanted]** — The `Bash` tool **shall not** inherit environment variables matching secret-name heuristics (`*_TOKEN`, `*_KEY`, `*_SECRET`, `GENIE_SHUTDOWN_TOKEN`) into subprocess env unless `input.inherit_secrets == true` is explicitly set AND the invocation is pre-approved via `PermissionMatcher`.
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-CORE-001 | zap logger, context root, graceful shutdown |
| 선행 SPEC | SPEC-GENIE-CONFIG-001 | `ToolsConfig`, `PermissionsConfig` 로딩 |
| 선행 SPEC | SPEC-GENIE-QUERY-001 | `permissions.CanUseTool`, `ToolPermissionContext` 타입 소유 (본 SPEC이 구현을 **호출**) |
| 선행 SPEC | SPEC-GENIE-MCP-001 | `mcp.Connection`, `mcp.Manager`, `FetchToolManifest`, `ConnectionClosed` 이벤트 |
| 후속 SPEC | SPEC-GENIE-CLI-001 | `genie tool list` 서브커맨드가 `Registry.ListNames` + `Inventory.ForModel` 호출 |
| 후속 SPEC | SPEC-GENIE-COMMAND-001 | slash command가 tool을 참조 (`/bash`, `/read` 등 custom command) |
| 후속 SPEC | SPEC-GENIE-SUBAGENT-001 | teammate별 tool visibility 제어 (`useExactTools`, `tools: ["*"]`) |
| 후속 SPEC | SPEC-GENIE-HOOK-001 | PreToolUse/PostToolUse hook이 `Executor.Run` 감싸는 middleware로 진입 |
...
- `.moai/specs/SPEC-GENIE-QUERY-001/spec.md` §3.2 OUT(tools.Executor 위임), REQ-QUERY-006/007/012/017, AC-QUERY-002/003/009/010
- `.moai/specs/SPEC-GENIE-MCP-001/spec.md` (동시 작성 중 — `mcp.Connection`, `mcp.Manager`, `FetchToolManifest` API)
...
- `../SPEC-GENIE-COMMAND-001/spec.md` — Slash command가 tool을 소비하는 경로
- `../SPEC-GENIE-CLI-001/spec.md` — CLI에서 `genie tool list`로 Inventory 노출
