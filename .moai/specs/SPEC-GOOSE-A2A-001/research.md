# SPEC-GOOSE-A2A-001 research.md — A2A Protocol 구현 자료
...
- 두 개의 goosed 인스턴스를 port 9001/9002 로 spawn, A → B delegation E2E.
...
| 리스크 | 영향 | 완화 |
|----|----|----|
| Agent Card 서명키 유실 | High | `goose a2a rotate-key` + 새 키 배포, 기존 card 는 grace 30일 |
...
Prometheus metrics:
- `goose_a2a_task_total{direction=in|out, result=success|failure}`
- `goose_a2a_task_duration_seconds`
- `goose_a2a_auth_failures_total{reason}`
- `goose_a2a_escrow_holds_open`
- `goose_a2a_reputation_updates_total`
...
```bash
$ goose a2a publish-card ./agent-card.yaml
[sign] Ed25519 signature ok
[serve] /.well-known/agent-card available at https://example.com

$ goose a2a call urn:goose:agent:legal-review --task code_review --budget 2000
