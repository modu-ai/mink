...
GOOSE 에이전트가 **외부 에이전트(다른 GOOSE, Claude, Gemini, 사용자 정의 A2A 에이전트)** 와 통신하고 태스크를 위임하며, **IDE(VS Code / Zed / JetBrains)** 와 프로토콜 수준에서 연동되도록 한다. 본 SPEC 은 다음 다섯 요소를 하나의 일관된 계약으로 제공한다:
...
5. **ACP Adapter (IDE 통합)** — Hermes ACP 패턴을 기반으로 VS Code / Zed / JetBrains 의 IDE 에이전트 프로토콜을 GOOSE 의 MCP 인터페이스로 브리지.
...
3. `/.well-known/agent-card` HTTP 엔드포인트 (GOOSE 가 자신을 외부에 노출할 때).
...
12. `ACPAdapter` — IDE 측 JSON-RPC ↔ GOOSE MCP-001 tool 변환. VS Code / Zed / JetBrains 벤더별 differ 를 작은 `Flavor` enum 으로 흡수.
...
When `ACPAdapter.Start(flavor)` is called with flavor in `{vscode, zed, jetbrains}`, the adapter shall expose the MCP tool catalog as ACP skills and bridge each tool invocation into a corresponding MCP call at the local GOOSE daemon.

### REQ-A2A-011 [Event-Driven]
When an ACP tool invocation completes, the adapter shall send the result back to the IDE in the flavor-specific JSON-RPC schema; the adapter shall not leak internal GOOSE session identifiers into the IDE response.
...
All persistent artifacts (tokens cache, escrow ledger, reputation store) shall be encrypted at rest using the `$GOOSE_HOME/.secret` device-local key (AES-256-GCM).
...
Agent Card 가 사용자의 identity-linked 크리덴셜을 보관 가능. 키는 장치별(`$GOOSE_HOME/.secret` 64byte 랜덤). 메인 DB(MEMORY-001) 와 같은 원칙. Key 유실 == 재가입(고지).
...
Given 두 개의 GOOSE 인스턴스(A, B)가 로컬에서 실행될 때, When `A` 가 `B` 에게 `DelegateTask` 를 호출하면, Then OAuth 2.0 PKCE 토큰이 발행되고 재사용되며, `auth.token_ttl` 만료 후에는 refresh 흐름이 자동 실행된다.
...
- **CLI 영향**: `goose a2a publish-card <path>`, `goose a2a call <target> <task>`, `goose a2a reputation <agentID>`, `goose acp start --flavor vscode`.
...
6. **Escrow 암호화 키 회전**: `$GOOSE_HOME/.secret` 회전 시 기존 ledger 재암호화 절차. 후속 CLI `goose a2a rotate-key`.
