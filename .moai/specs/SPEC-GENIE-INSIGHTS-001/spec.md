...
# SPEC-GENIE-INSIGHTS-001 — 다차원 Insights 추출 (Pattern/Preference/Error/Opportunity)
...
GENIE-AGENT **자기진화 파이프라인의 Layer 3**를 정의한다. TRAJECTORY-001이 기록한 `.jsonl` 궤적 파일 + MEMORY-001의 `facts` 테이블을 입력으로, **4개 다차원 집계(overview / models / tools / activity)**와 **4개 질적 분류(Pattern / Preference / Error / Opportunity)**를 생성한다. 각 분류 항목은 **신뢰도 점수**(관찰 횟수 + 분산 기반)를 달며, REFLECT-001(Phase 5)의 5단계 승격 파이프라인 입력이자 사용자 주간/월간 리포트의 데이터 소스로 사용된다.
...
- `/genie insights` CLI 명령(CLI-001)의 백엔드. 주간 리포트 자동 생성 + 사용자 확인 워크플로우.
...
8. `internal/learning/insights/scanner.go`: `${GENIE_HOME}/trajectories/**/*.jsonl` 스캔 (`TrajectoryReader`).
...
**REQ-INSIGHTS-005 [Event-Driven]** — **When** `Extract(period=Last(7))` is invoked, the engine **shall** scan `${GENIE_HOME}/trajectories/{success,failed}/YYYY-MM-DD.jsonl` for dates in the range `[now-7d, now]` and ignore files outside range.
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-TRAJECTORY-001 | `.jsonl` 파일 스키마, `Trajectory` 타입, `TrajectoryMetadata.FailureReason` |
| 선행 SPEC | SPEC-GENIE-COMPRESSOR-001 | (선택) `Summarizer` 인터페이스 재사용 — `UseLLMSummary=true` 시 |
| 선행 SPEC | SPEC-GENIE-MEMORY-001 | (선택) `MemoryManager` 주입 — Opportunity category 보강 |
| 선행 SPEC | SPEC-GENIE-ERROR-CLASS-001 | `FailureReason` 문자열 ↔ `FailoverReason` enum 매핑 |
| 선행 SPEC | SPEC-GENIE-CORE-001 | `GENIE_HOME`, zap 로거 |
| 후속 SPEC | SPEC-GENIE-REFLECT-001 (Phase 5) | `Insight` 목록을 5단계 승격 입력으로 소비 |
| 후속 SPEC | SPEC-GENIE-CLI-001 | `genie insights [--last 7]` 명령이 본 SPEC의 `Report.RenderTable()` 호출 |
...
- `.moai/specs/SPEC-GENIE-TRAJECTORY-001/spec.md` — `.jsonl` 공급자
- `.moai/specs/SPEC-GENIE-ERROR-CLASS-001/spec.md` — FailureReason 매핑
...
- `../SPEC-GENIE-TRAJECTORY-001/spec.md` — 선행
- `../SPEC-GENIE-MEMORY-001/spec.md` — Opportunity 보강
- `../SPEC-GENIE-ERROR-CLASS-001/spec.md` — Error 카테고리 데이터
- `../SPEC-GENIE-COMPRESSOR-001/spec.md` — (선택) Summarizer 재사용
...
**End of SPEC-GENIE-INSIGHTS-001**
