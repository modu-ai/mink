...
# SPEC-GENIE-SKILLS-001 — Progressive Disclosure Skill System (L0~L3, YAML, 4 Trigger)
...
GENIE-AGENT의 **Skill 시스템**을 정의한다. Claude Code의 Progressive Disclosure(L0~L3 effort 티어) + YAML frontmatter + 4-trigger 활성화 모델을 Go로 포팅하여, `QueryEngine`(SPEC-GENIE-QUERY-001)이 매 iteration 직전 적절한 Skill을 로드·치환·권한 검증한 후 system/user prompt에 반영하게 한다.
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-QUERY-001 | Skill consumer(QueryEngine prompt 주입자)의 인터페이스 계약 |
| 선행 SPEC | SPEC-GENIE-CORE-001 | zap 로거, context 루트 |
| 후속 SPEC | SPEC-GENIE-HOOK-001 | `FileChanged` 이벤트 → `FileChangedConsumer` 라우팅 |
| 후속 SPEC | SPEC-GENIE-SUBAGENT-001 | `context: fork` skill의 agent spawn |
| 후속 SPEC | SPEC-GENIE-PLUGIN-001 | plugin manifest `skills:` 로딩 |
| 후속 SPEC | SPEC-GENIE-COMMAND-001 | `argument-hint:` 소비 |
...
- `.moai/specs/SPEC-GENIE-QUERY-001/spec.md` — consumer 인터페이스
