...
> **본 문서는 SPEC-GOOSE-ONBOARDING-001 v0.2.0에 대한 Amendment이다.**
...
**REQ-OB-021 [Event-Driven]** — **When** the user reaches the "Model Setup" step (new Step 2), the onboarding flow **shall** detect whether Ollama is installed and whether a GOOSE model is available, and:
- (a) **If** Ollama is installed and a model is available, display the detected model name and size, and allow the user to proceed or change the model. "Already available" **shall** be determined by querying `ollama list` (or `GET /api/tags`) and checking for any model whose name starts with `ai-goose/`.
...
**REQ-OB-023 [Event-Driven]** — **When** the user reaches the "CLI Tools" step (new Step 3), the onboarding flow **shall** scan the system PATH for `claude`, `gemini`, and `codex` CLI tools, display detected tools with their version numbers, and configure delegation routing rules in `~/.goose/config.yaml`.
...
**REQ-OB-027 [Event-Driven]** — **When** the onboarding completes (Step 7 Submit), the backend **shall** auto-generate `~/.goose/config.yaml` incorporating all collected settings including: model selection (from Step 2), CLI tool availability (from Step 3), persona (from Step 4), provider configuration (from Step 5), messenger channel (from Step 6), and consent flags (from Step 7).
...
type ModelSetup struct {
    ...
    DetectedModel   string // e.g., "ai-goose/gemma4-e4b-rl-v1:q5_k_m"
...
- **Given** Ollama 설치됨, `ai-goose/gemma4-e4b-rl-v1:q5_k_m` 모델 존재
...
- **Given** 전체 7-Step 완료
- **When** Step 7 Submit
- **Then** `~/.goose/config.yaml`에 다음 항목 모두 포함:
  ```yaml
  model:
    selected: "ai-goose/gemma4-e4b-rl-v1:q5_k_m"
    provider: "ollama"
  delegation:
    available_tools:
      - name: claude
...
|---------------------------|-----------------|
| `~/.goose/config.yaml`의 모델 정보 | Step 2에서 "detected"로 표시, 재다운로드 불필요 |
| `~/.goose/config.yaml`의 CLI 도구 | Step 3에서 "detected"로 표시, 재스캔 불필요 |
...
**End of SPEC-GOOSE-ONBOARDING-001 Amendment v0.3.0**
