...
> **유지/추가**: **`goose init` CLI 마법사** + **Web UI 설치·설정 마법사** (비개발자 대응).
...
> "Apple Native 초기 설정 · 모바일 디바이스 페어링 · 이메일 가입을 제거하고, **`goose init` CLI 마법사**와 **Web UI 설치 마법사**로 비개발자도 3분 내 설정을 마칠 수 있게 한다."

본 SPEC은 **GOOSE 첫 설치 시 사용자를 맞이하는 CLI + Web UI 온보딩 마법사**를 정의한다. 두 경로는 동일한 backend 상태 머신(`internal/onboarding`)을 공유하며, 동일한 최종 산출물(`./.goose/` 디렉터리 트리)을 생성한다.
...
| 측면 | `goose init` CLI | Web UI Wizard |
|-----|-----------------|---------------|
| 진입점 | 쉘에서 `goose init` 실행 | `goose serve --install-wizard` 실행 후 브라우저 열림 (`http://localhost:5173/install`) |
...
| 비정상 종료 재개 | `goose init --resume` | `/install?resume=1` |
...
- 이메일/모바일 페어링은 GOOSE 핵심가치(로컬 우선)와 불일치.
- **가장 단순한 설치 경로**: 터미널 한 줄 또는 브라우저 한 탭.

따라서:
- **CLI(`goose init`)** — 개발자가 `curl | sh` 설치 직후 즉시 실행 가능. 서버 환경에서도 동작.
- **Web UI** — `goose serve --install-wizard`로 로컬 서버 기동 후 자동 브라우저 오픈. 터미널에 익숙지 않은 가족 · 지인도 사용 가능.
...
한 단계 스킵 시 후속 기능이 저하되지만, **전 단계 모두 스킵해도 `./.goose/` 초기화는 완료**되어야 한다 (GOOSE를 일단 띄우는 것이 최우선).

### 2.3 branding.md §5.1 First Day with GOOSE

> "Hatching 🐣 — 알에서 깨어나는 최초의 만남. Goose가 사용자를 발견하고 각인(imprinting)하는 순간."

각인(imprinting) 메타포는 유지: Step 2(Persona) 완료 시 GOOSE mood가 `calm → imprinting → curious`로 전환. CLI에서는 ASCII 아트 전환, Web UI에서는 간단한 CSS transition (prefers-reduced-motion 시 fade only).
...
1. **CLI 경로 (`goose init`)** — `cmd/goose/cmd/init.go`:
...
10. **완료 처리**:
    - CLI: ASCII "🐣 → 🐥" 아트 전환 + "Goose is ready. Run `goose start`." 메시지
    - Web UI: 간단한 egg→goose transition (prefers-reduced-motion 존중), "터미널로 돌아가세요" 메시지
...
**REQ-OB-005 [Event-Driven]** — **When** the user invokes `goose init` in a fresh environment (no `./.goose/config.yaml` exists or `onboarding_completed` is absent) **or** opens the Web UI install wizard (`goose serve --install-wizard`), the system **shall** start the onboarding flow at Step 1 rather than the main application shell.
...
**REQ-OB-011 [State-Driven]** — **While** the user is mid-onboarding and quits (CLI: Ctrl+C; Web UI: tab closed), the collected data (partial) **shall** be persisted to `./.goose/onboarding-draft.yaml`, and re-running `goose init --resume` or reopening the Web UI **shall** resume from the last completed step.
...
- **When** 사용자가 `goose init` 실행 또는 `goose serve --install-wizard` 후 `http://localhost:5173/install` 접근
...
- **Then** CLI는 ASCII 아트 "🐣 → 🐥" 전환과 "Goose is ready. Run `goose start`." 메시지(prefers reduced motion 시 ASCII 전환 생략 텍스트만). Web UI는 egg→goose transition과 "터미널로 돌아가세요" 메시지(reduced motion 시 fade only). 한국어 사용자에게는 "안녕하세요, [이름]님!" 인사 적용

**AC-OB-011 — 중도 이탈 후 재개 (verifies REQ-OB-011)**
- **Given** Step 2 (Persona) 완료 후 사용자가 Ctrl+C(CLI) 또는 탭 닫기(Web UI)
- **When** `goose init --resume` 또는 `/install?resume=1`
- **Then** `./.goose/onboarding-draft.yaml`에 Step 1~2 데이터 존재, UI는 Step 3부터 재시작

**AC-OB-012 — LLM provider 스킵 (verifies REQ-OB-010)**
- **Given** Step 3에서 Skip
- **When** Step 4로 진행
- **Then** CONFIG-001 `llm.default_provider: "unset"` 저장. 온보딩 완료 후 첫 `goose start` 실행 시 "LLM 공급자를 설정해주세요" 비차단 배너/알림 표시
...
- **When** Playwright 스크립트 `e2e/install-wizard-speedrun.spec.ts`가 Web UI Step 1 → Step 5 전체를 자동 진행하고, 별도 `scripts/cli-install-speedrun.sh`가 `goose init --yes` 경로를 실행
...
- **Given-2 (CLI)** `NO_COLOR=1 goose init` 실행
- **When** 온보딩 각 단계 렌더
...
```
cmd/goose/cmd/
├── init.go                         # `goose init` CLI entry
└── serve.go                        # `goose serve --install-wizard` mode
...
| R3 | LLM provider 스킵 후 영영 미설정 | 중 | 중 | 메인 앱 첫 실행 시 persistent notice + `goose config provider` 단축 명령 |
...
| R9 | Web UI 서버 포트 충돌 (5173 사용 중) | 중 | 낮 | 포트 auto-increment, 환경변수 `GOOSE_INSTALL_PORT` 지원 |
| R10 | 온보딩 반복 버그 (완료 후에도 다시 실행) | 낮 | 고 | `./.goose/onboarding-completed` 타임스탬프 + 파일 존재 검증, Force-reset은 `goose init --force`에서만 |
...
- v0.2 Amendment: **CLI 온보딩은 본 SPEC의 IN SCOPE이다**. (v0.1에서는 "CLI 환경에서 온보딩을 강제하지 않는다" 조항이 있었으나, v0.2 Amendment에서 `goose init` CLI 마법사가 핵심 경로로 지정되었으므로 해당 조항은 철회한다.)
