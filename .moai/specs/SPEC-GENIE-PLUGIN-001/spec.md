...
# SPEC-GENIE-PLUGIN-001 — Plugin Host (manifest.json + MCPB + 4 Primitive 패키징)
...
GENIE-AGENT의 **Plugin Host**를 정의한다. Claude Code의 `manifest.json` 중심 플러그인 아키텍처를 Go로 포팅하여, 하나의 plugin 디렉토리(또는 MCPB 번들 파일)에 Skills + Agents + MCP Servers + Slash Commands + Hooks **4 primitive를 한꺼번에 패키징**하고, 3단계 discovery(user / project / marketplace)로 로드하며, `clearThenRegister` atomic swap으로 hot-reload를 지원한다.

본 SPEC이 통과한 시점에서 `internal/plugin` 패키지는:

- `PluginManifest`(name, version, skills, agents, mcpServers, commands, hooks) JSON 스키마 파싱,
- 3-level discovery: `$HOME/.genie/plugins/` (user), `./.genie/plugins/` (project), 마켓플레이스 URI(Phase 5+ 스텁),
...
- Plugin 활성화/비활성화 + `.genie/plugins.yaml` user config 연동.
...
4. Discovery:
   - User level: `$HOME/.genie/plugins/<name>/manifest.json`,
   - Project level: `./.genie/plugins/<name>/manifest.json`,
   - Marketplace URI: Phase 5+ stub (`LoadPluginFromMarketplace(uri) → ErrNotImplemented`).
5. Validator:
   - name이 reserved list(`genie`, `claude`, `mcp`, `plugin`, `_*`)에 없음,
...
9. 활성화 상태: `./.genie/plugins.yaml` — enabled plugins + user-provided variables.
...
**REQ-PL-016 [Unwanted]** — Plugin names reserved by the host **shall not** be loaded: `genie`, `claude`, `mcp`, `plugin`, `_*` (underscore prefix); such names cause `ErrReservedPluginName`.
...
const (
    SourceUser PluginSource = iota  // ~/.genie/plugins/
    SourceProject                   // ./.genie/plugins/
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-SKILLS-001 | `skill.LoadPluginSkills(manifest, dir) (staged, commit, rollback)` entry point |
| 선행 SPEC | SPEC-GENIE-MCP-001 | `mcp.LoadPluginServers` entry point |
| 선행 SPEC | SPEC-GENIE-SUBAGENT-001 | `subagent.LoadPluginAgents` entry point |
| 선행 SPEC | SPEC-GENIE-HOOK-001 | `hook.LoadPluginHooks` + `HookEventNames()` |
| 선행 SPEC | SPEC-GENIE-CONFIG-001 | `plugins.yaml` loader, feature gate |
| 선행 SPEC | SPEC-GENIE-CORE-001 | zap 로거, context 루트 |
| 후속 SPEC | SPEC-GENIE-COMMAND-001 | `command.LoadPluginCommands` entry point (본 SPEC은 stub 호출) |
...
- `.moai/specs/SPEC-GENIE-SKILLS-001/spec.md` — LoadPluginSkills 계약
- `.moai/specs/SPEC-GENIE-MCP-001/spec.md` — LoadPluginServers
- `.moai/specs/SPEC-GENIE-SUBAGENT-001/spec.md` — LoadPluginAgents
- `.moai/specs/SPEC-GENIE-HOOK-001/spec.md` — LoadPluginHooks + HookEventNames
...
- `../SPEC-GENIE-SKILLS-001/spec.md`
- `../SPEC-GENIE-MCP-001/spec.md`
- `../SPEC-GENIE-SUBAGENT-001/spec.md`
