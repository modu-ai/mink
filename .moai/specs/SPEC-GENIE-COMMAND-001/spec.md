...
# SPEC-GENIE-COMMAND-001 — Slash Command System (내장 + Custom, Skill 연계)
...
GENIE-AGENT의 **Slash Command 파서 및 디스패처**를 정의한다. `SPEC-GENIE-QUERY-001` §3.2는 `processUserInput(prompt)`를 "본 SPEC에서 noop (원문 그대로 반환). COMMAND-001이 확장"으로 위임한다. 본 SPEC이 그 훅 포인트를 채우며, Claude Code `commands/` 디렉토리의 slash command 시스템을 Go로 포팅한다.

본 SPEC 수락 시점에서:

- 사용자가 입력한 `/help`, `/clear`, `/model <alias>`, `/compact [target]`, `/exit` 등 **내장 command 최소 세트**가 LLM 호출 없이 local로 처리되고,
- `/moai`, `/agency`, `/custom` 등 **사용자 정의 slash command**가 YAML frontmatter 기반으로 `~/.genie/commands/` + `.genie/commands/` 에서 로드되며,
...
- **MoAI/Agency 명령 호환성**: ROADMAP의 목표는 Claude Code `.claude/commands/*.md`와 동형의 slash command를 GENIE가 실행하는 것. `/moai run SPEC-XXX` 같은 기존 MoAI 워크플로우를 그대로 재활용하려면 동일 frontmatter 스키마를 지원해야 함.
...
4. `internal/command/custom/` — Custom command loader.
   - `~/.genie/commands/*.md` (user scope), `.genie/commands/*.md` (project scope) 스캔.
...
- **Skill 본체 로딩 / L0-L3 progressive disclosure / conditional skill / remote skill**: SPEC-GENIE-SKILLS-001.
...
**REQ-CMD-017 [Optional]** — **Where** `CommandConfig.CustomCommandRoots` contains additional paths beyond `~/.genie/commands/` and `.genie/commands/`, the loader **shall** scan each additional root with the same precedence rules (later-declared roots have higher precedence).
...
- **Given** 테스트 fixture `t.TempDir()/commands/greet.md`:
  ...
  argument-hint: "<name>"
  ---
  Hello $ARGUMENTS, welcome to GENIE.
  ```
- **When** Registry를 `WithCustomRoots(tmpDir)`로 구성 후 `ProcessUserInput(ctx, "/greet Alice", sctx)`
- **Then** `ProceedWithPrompt{prompt: "Hello Alice, welcome to GENIE."}` 반환
...
- **Given** custom command `name: help` (`.genie/commands/help.md`)
...
type Context struct {
    Args Args
    Env  map[string]string // $CWD, $GENIE_HOME 등
}
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-CORE-001 | zap logger, context root |
| 선행 SPEC | SPEC-GENIE-CONFIG-001 | `CommandConfig` (custom roots, max size) |
| 선행 SPEC | SPEC-GENIE-QUERY-001 | `submitMessage` 훅 포인트, `SlashCommandContext` 구현측 |
| 후속 SPEC | SPEC-GENIE-SKILLS-001 | `user-invocable` skill을 command로 노출 (Provider 인터페이스 구현) |
| 후속 SPEC | SPEC-GENIE-CLI-001 | CLI readline / history / `genie ask "/help"` 경로 |
| 후속 SPEC | SPEC-GENIE-ROUTER-001 | `ResolveModelAlias` 구현 (/model 명령) |
| 후속 SPEC | SPEC-GENIE-CONTEXT-001 | `OnCompactRequest` 처리 (/compact 명령) |
| 후속 SPEC | SPEC-GENIE-SUBAGENT-001 | `PlanModeActive` 힌트 (REQ-CMD-011) |
...
- `.moai/specs/SPEC-GENIE-QUERY-001/spec.md` §3.2 OUT (processUserInput passthrough), §5 AC-QUERY-001 (user_ack 흐름)
- `.moai/specs/SPEC-GENIE-SKILLS-001/spec.md` (동시 작성 중 — `user-invocable` skill 노출 경로)
...
- `../SPEC-GENIE-TOOLS-001/spec.md` — Permission matcher 구문 (glob style 공유 디자인 결정)
- `../SPEC-GENIE-CLI-001/spec.md` — CLI readline이 command line을 submitMessage로 전달
...
- 본 SPEC은 **Skill 본체 로딩 / progressive disclosure / conditional skill / remote skill을 구현하지 않는다**. SPEC-GENIE-SKILLS-001.
