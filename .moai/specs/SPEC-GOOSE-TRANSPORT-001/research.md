# SPEC-GOOSE-TRANSPORT-001 — Research & Inheritance Analysis
...
- `cmd/goosed/main.go` 부재(CORE-001 GREEN 이후 생김).
...
TypeScript 클라이언트(Phase 5+ goose-web)는 Connect-ES를 사용해 **동일 proto**를 재활용 가능. `connect-go`는 Phase 5+에서 재평가.
...
생성물 위치 논쟁:
- 옵션 A: `internal/transport/grpc/gen/goosev1/`
- 옵션 B: `pkg/proto/goosev1/` (public)
...
3. **Proto 패키지 고정 규칙**: `goose.v1`으로 확정. 향후 breaking 필요 시 `goose.v2` 신규 디렉토리. 본 SPEC은 v2 준비 로직 없음.
...
GREEN 단계 완료 시점에서 CLI-001은 generated `goosev1.DaemonServiceClient`를 소비할 수 있고, LLM/AGENT 후속 SPEC은 `proto/goose/v1/`에 서비스를 추가하는 표준 경로를 확보한다.
