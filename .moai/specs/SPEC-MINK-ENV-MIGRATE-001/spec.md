...
| 3 | 분산 production read site 10 곳 의 alias loader 채택 (15 runtime-read keys 전체 처리) |
...
- **REQ-MINK-EM-009**: alias loader 는 strict mode option (`Options.StrictUnknownAliasError`) 을 제공하여, 등록되지 않은 키에 대한 read 호출을 error 로 반환할 수 있도록 한다 (default = false; future SPEC-MINK-ENV-CLEANUP-001 의 GOOSE_* 완전 제거 시 strict=true 로 flip).
...
### Phase 3 — GOOSE_* fallback + sync.Once warning 채택 (분산 read site 10 곳)

산출물 (모두 단일 commit):
- `internal/audit/dual.go:140` (GOOSE_HOME)
- `internal/config/config.go:274` (GOOSE_HOME)
- `internal/command/adapter/aliasconfig/loader.go:32` (const homeEnv)
- `internal/transport/grpc/server.go:169,277,297` (GRPC_REFLECTION, GRPC_MAX_RECV_MSG_BYTES, SHUTDOWN_TOKEN)
- `internal/hook/handlers.go:270` (GOOSE_HOOK_TRACE)
- `internal/hook/permission.go:251` (GOOSE_HOOK_NON_INTERACTIVE)
- `cmd/minkd/main.go:89` (GOOSE_ALIAS_STRICT)
- `internal/llm/provider/qwen/client.go:38` (envQwenRegion const)
- `internal/llm/provider/kimi/client.go:40` (envKimiRegion const)
...
- in-tree test 의 `t.Setenv("GOOSE_*", ...)` 50+ 곳 전수 → `t.Setenv("MINK_*", ...)` 로 변경
  - `cmd/minkd/integration_test.go` (10 occurrence)
  - `internal/audit/dual_test.go` (4 occurrence)
  - `internal/command/adapter/aliasconfig/*_test.go` (8 occurrence)
  - `internal/config/config_test.go` (10+ occurrence)
  - `internal/llm/provider/qwen/client_test.go`, `kimi/client_test.go` (4 occurrence)
  - `internal/transport/grpc/server_test.go` (3 occurrence)
  - `internal/tools/builtin/terminal/bash_test.go` (1 occurrence)
...
| R4 | Phase 4 의 50+ test migration 에서 누락 발생 시 backward compat regression | Medium | grep 자동화 (AC-MINK-EM-007) + 전체 `go test ./...` |
...
| 8 | 시간 추정 / deadline 명시 | MoAI 운영 규칙 (CLAUDE.md §6 — Time Estimation 금지) | 우선순위 라벨로 대체 (priority: High) |
