...
# SPEC-MINK-ENV-MIGRATE-001 — `MINK_*` env vars → `MINK_*` deprecation alias loader
...
`MINK_*` 환경변수 22개에 대응하는 `MINK_*` 키를 도입하고, 두 키 공존 기간 동안 alias loader 를 단일 read 경로로 채택한다. 사용자가 dotfile / CI / Docker manifest 의 `MINK_*` 표기를 즉시 갱신하지 않아도 동작이 깨지지 않도록 backward-compat 를 보장하면서, **per-key per-process 1회** 의 deprecation warning 을 통해 마이그레이션을 유도한다.

### §1.2 Non-Goals

- `MINK_*` env var 의 완전 제거 → 별도 후속 SPEC (SPEC-MINK-ENV-CLEANUP-001), post-1.0 release + 1+ minor cycle 후
- user-data path migration (`~/.goose → ~/.mink`) → SPEC-MINK-USERDATA-MIGRATE-001 (별도)
- 외부 운영 자산 (`docker-compose.yaml`, k8s manifests) 의 MINK_* update → 후속 docs SPEC
...
| related_specs | SPEC-MINK-ENV-CLEANUP-001 (예정) | post-1.0 MINK_* 완전 제거 |
...
| REQ-MINK-EM-005 | REQ-MINK-BR-027 | "MINK_* still works with deprecation" 정책 구현체 (OLD only → backward compat 동작 보장) |
...
본 SPEC = 위 결정 (2) 의 이행. CHANGELOG.md line 23 ("`SPEC-MINK-ENV-MIGRATE-001`: `MINK_*` 21개 env vars → `MINK_*` deprecation alias loader") 가 이 약속을 명시 기록.

### §2.2 현재 상태

- BRAND-RENAME 머지 후, 코드 식별자 / 산문 / 모듈 path 는 `mink` 로 정리 완료 (commit `f0f02e4`)
- 그러나 **env var 키 자체는 `MINK_*` 그대로 잔존** — 22개 unique key, 174 .go reference (research.md §1)
- 사용자가 brand 가 바뀐 빌드를 실행할 때 env 표기는 여전히 `MINK_*` 만 인식 — 의도된 일시적 상태 (BRAND-RENAME footnote 명시)

### §2.3 본 SPEC 의 가치

- backward-compat 100% (기존 MINK_* 사용자 환경 그대로 동작)
- forward-compat 진입점 (MINK_* 신규 표기 작동)
- 마이그레이션 신호 (deprecation warning) 를 **noise-free** 로 emit (per-key per-process 1회)
- 단일 코드 경로 (alias loader API) 로 이후 SPEC-MINK-ENV-CLEANUP-001 의 MINK_* 완전 제거가 1줄 수정 (loader option flip) 으로 가능
...
| 5 | `MINK_AUTH_*` prefix deny-list 에 `MINK_AUTH_*` 추가 (`internal/hook/isolation_unix.go`, `isolation_other.go`) |
| 6 | in-tree test 의 `t.Setenv("MINK_*", ...)` → `t.Setenv("MINK_*", ...)` migration (alias 검증 test 1~2 곳 제외) |
| 7 | 산문/주석/error message 의 MINK_* 표기 → MINK_* update (alias 검증 test 의 코멘트 제외, 본 SPEC 의 documents 제외) |
| 8 | 단위 test (envalias 패키지 자체 + 각 read site 의 alias 동작) |
| 9 | integration test (실제 env set + main wire-up + 1회 warning emit 검증) |

### §3.2 OUT scope

research.md §6.2 표 참조. 핵심: MINK_* 완전 제거 / user-data path / 외부 운영 자산 / viper 도입 / 신규 runtime read logic 신설.
...
- **REQ-MINK-EM-001**: MINK 시스템은 모든 MINK_* / MINK_* env var 접근을 alias loader (`internal/envalias`) 의 단일 API 를 통해 수행한다.
- **REQ-MINK-EM-002**: alias loader 는 22개 MINK_* key 각각에 대응하는 MINK_* key 를 1:1 매핑으로 등록한다 (예: `MINK_HOME ↔ MINK_HOME`, `MINK_AUTH_REFRESH ↔ MINK_AUTH_REFRESH`).
...
- **REQ-MINK-EM-004**: 사용자가 `MINK_X` 만 설정하고 `MINK_X` 를 미설정한 상태에서 alias loader 가 X 키 read 호출을 받으면, alias loader 는 `MINK_X` 의 값을 반환하고 deprecation warning ("`MINK_X` is deprecated; please rename to `MINK_X`") 을 1회 emit 한다.
...
- **REQ-MINK-EM-006**: `MINK_X` 와 `MINK_X` 가 동시에 설정된 상태에서 alias loader 가 X 키 read 호출을 받으면, alias loader 는 `MINK_X` 값을 반환하고 (NEW > OLD priority, research.md §3.4) `MINK_X` 는 무시하며 conflict warning ("both `MINK_X` and `MINK_X` set; using `MINK_X`") 을 1회 emit 한다.
...
### AC-MINK-EM-002 — MINK_X only 시나리오 (REQ-MINK-EM-004)

- **Given**: `MINK_LOG_LEVEL` 미설정, `MINK_LOG_LEVEL=debug` 설정
- **When**: `minkd` 프로세스 기동 + envOverlay 호출
- **Then**:
  - `cfg.Log.Level == "debug"` (값 반환 정상)
  - stderr (zap JSON line) 에 `"msg":"deprecated env var, please rename"` + `"old":"MINK_LOG_LEVEL"` + `"new":"MINK_LOG_LEVEL"` 출력 1회
  - 검증: `MINK_LOG_LEVEL= MINK_LOG_LEVEL=debug ./minkd 2>&1 | grep -c "MINK_LOG_LEVEL"` → 1

### AC-MINK-EM-003 — MINK_X only 시나리오 (REQ-MINK-EM-005)

- **Given**: `MINK_LOG_LEVEL=debug` 설정, `MINK_LOG_LEVEL` 미설정
- **When**: `minkd` 프로세스 기동
- **Then**:
  - `cfg.Log.Level == "debug"` (값 반환 정상)
  - stderr 에 MINK_LOG_LEVEL 관련 warning 0회
  - 검증: `MINK_LOG_LEVEL=debug MINK_LOG_LEVEL= ./minkd 2>&1 | grep -c "MINK_LOG_LEVEL"` → 0

### AC-MINK-EM-004 — 동시 설정 시나리오 (REQ-MINK-EM-006, NEW > OLD)

- **Given**: `MINK_LOG_LEVEL=info`, `MINK_LOG_LEVEL=debug` 동시 설정
- **When**: `minkd` 프로세스 기동
- **Then**:
  - `cfg.Log.Level == "info"` (MINK 우선)
  - stderr 에 conflict warning ("both `MINK_LOG_LEVEL` and `MINK_LOG_LEVEL` set; using `MINK_LOG_LEVEL`") 1회 emit
  - 검증: stderr grep 으로 `"both"` + `"MINK_LOG_LEVEL"` 동일 라인 1회

### AC-MINK-EM-005 — sync.Once 검증 (REQ-MINK-EM-003, REQ-MINK-EM-007)

- **Given**: `MINK_LOG_LEVEL=debug` 설정
...
- **When**: `Get("LOG_LEVEL")` 호출 (`MINK_LOG_LEVEL=debug` 설정)
- **Then**:
  - panic 발생 안 함
  - 값 반환 정상 ("debug")
...
- **When**: `grep -rn 't\.Setenv("MINK_' --include="*.go" . | grep -v "/vendor/" | grep -v "envalias/"`
- **Then**: 결과 0건 (alias 검증 전용 test 인 `internal/envalias/loader_test.go` 만 MINK_* 사용 허용)
- 검증: 위 grep 명령 + `wc -l` → 0
...
- **Then**:
  - 최소 6 sub-test PASS (MINK only / MINK only / both / sync.Once / nil logger / strict mode)
...
- **When**: `grep -n "MINK_AUTH_\|MINK_AUTH_" internal/hook/isolation_*.go`
- **Then**:
  - 두 파일 모두 `MINK_AUTH_*` prefix 도 deny-list 에 포함
  - 기존 `MINK_AUTH_*` prefix 유지 (backward compat)
...
- **Given**: feature branch
- **When**: `grep -rn "MINK_" --include="*.go" . | grep -v "/vendor/" | grep -v "envalias/" | grep -v "_test.go"`
- **Then**:
...
- 검증: 위 grep + 수동 review (alias mapping 만 MINK_* 표기 허용)
...
- `internal/envalias/keys.go` — 22 key mapping table (MINK_X ↔ MINK_X) const map
...
### Phase 3 — MINK_* fallback + sync.Once warning 채택 (분산 read site 11 곳)

> **Frame anchor**: 모든 file:line 참조는 base commit `f0f02e4` (= origin/main, BRAND-RENAME-001 squash merge 직후) 기준이다. Phase 3 commit 직전 line 번호 drift 가능 — Phase 3 atomic commit 시 grep 으로 키워드 (`os.Getenv("MINK_X"`, `envQwenRegion`, `envKimiRegion`, `homeEnv`) 재확인 후 적용.

산출물 (모두 단일 commit, 8 `os.Getenv` direct callsite + 3 const-based callsite = 11 read sites):

`os.Getenv` direct callsite (8):
- `internal/audit/dual.go:140` — `os.Getenv("MINK_HOME")`
- `internal/config/config.go:274` — `os.Getenv("MINK_HOME")` (resolveGooseHome 함수 안)
- `internal/transport/grpc/server.go:169` — `os.Getenv("MINK_GRPC_REFLECTION")`
- `internal/transport/grpc/server.go:277` — `os.Getenv("MINK_GRPC_MAX_RECV_MSG_BYTES")`
- `internal/transport/grpc/server.go:297` — `os.Getenv("MINK_SHUTDOWN_TOKEN")`
- `internal/hook/handlers.go:270` — `os.Getenv("MINK_HOOK_TRACE")`
- `internal/hook/permission.go:251` — `os.Getenv("MINK_HOOK_NON_INTERACTIVE")`
- `cmd/minkd/main.go:89` — `os.Getenv("MINK_ALIAS_STRICT")`

const-based callsite (3, const 정의 + read 함수 묶음):
- `internal/command/adapter/aliasconfig/loader.go:32` (const `homeEnv = "MINK_HOME"`) + `loader.go:92` (callsite `os.Getenv(homeEnv)`)
- `internal/llm/provider/qwen/client.go:38` (const `envQwenRegion = "MINK_QWEN_REGION"`) + `client.go:99` (callsite)
- `internal/llm/provider/kimi/client.go:40` (const `envKimiRegion = "MINK_KIMI_REGION"`) + `client.go:135` (callsite)

각 호출부의 변경 패턴:
```go
// before
v := os.Getenv("MINK_HOME")
...
in-tree test 의 `t.Setenv("MINK_*", ...)` 28 곳 전수 → `t.Setenv("MINK_*", ...)` 로 변경 (검증 명령: `grep -rEn 't\.Setenv\("MINK_' --include='*.go' .` → 28):
...
`os.Setenv("MINK_*", ...)` 6 곳 전수 → `t.Setenv("MINK_*", ...)` 로 변경 (research.md §2.4 일관성, R10 mitigation: process-wide env 오염 회피):
...
exception: `internal/envalias/loader_test.go` 의 alias 검증 test 만 MINK_* 직접 setenv 유지 (의도된 backward compat 검증)

검증: AC-MINK-EM-007 의 grep + 전체 `go test ./...` PASS

### Phase 5 — 산문/주석/error message + flag help text update

> **Boundary clarification (D9)**: 본 Phase 는 alias 등록 + 산문 정리만 처리. runtime read logic 신설 (현재 미구현 4 keys: `MINK_TELEGRAM_BOT_TOKEN`, `MINK_HISTORY_SNIP`, `MINK_METRICS_ENABLED`, `MINK_GRPC_BIND`) 은 OUT scope (§11 Exclusion #5/#6).
>
> **Frame anchor**: file:line 참조는 base commit `f0f02e4` 기준. Phase 5 commit 직전 line drift 가능 — keyword grep 으로 재확인.

산출물:
- error message: `internal/cli/commands/messaging_telegram.go:68` (`MINK_TELEGRAM_BOT_TOKEN` → `MINK_TELEGRAM_BOT_TOKEN` + alias hint 추가 "(legacy `MINK_TELEGRAM_BOT_TOKEN` also accepted)")
- flag help: 동 파일 line 138 동일 패턴
- error message: `internal/messaging/telegram/keyring_nokeyring.go:20,25`
- 주석 (한국어): research.md §1.1 의 모든 production read site 의 한글 주석 (`// MINK_X 환경변수 키` → `// MINK_X 환경변수 키 (legacy alias: MINK_X)`)
...
- READ-ONLY: `internal/envalias/keys.go` 의 alias mapping 정의 (MINK_* literal 등장 의도된 자료) 는 변경 금지
- READ-ONLY: 본 SPEC 의 산출 documents (`spec.md`, `research.md`) 의 MINK_* 표기는 의도된 인용
...
const (
    ...
    SourceGoose                    // MINK_X 사용 (deprecated)
)
...
//   - returns (MINK_X value, SourceGoose, true) when only MINK_X set; emits deprecation warning per-key once
...
// keyMappings maps the short newKey ("LOG_LEVEL") to the MINK_/MINK_ pair.
// All 22 keys from research.md §1.1 inventory are registered here.
var keyMappings = map[string]struct {
    Mink  string
    Goose string
}{
    "HOME":                       {Mink: "MINK_HOME",                       Goose: "MINK_HOME"},
    "LOG_LEVEL":                  {Mink: "MINK_LOG_LEVEL",                  Goose: "MINK_LOG_LEVEL"},
    "HEALTH_PORT":                {Mink: "MINK_HEALTH_PORT",                Goose: "MINK_HEALTH_PORT"},
    "GRPC_PORT":                  {Mink: "MINK_GRPC_PORT",                  Goose: "MINK_GRPC_PORT"},
    "LOCALE":                     {Mink: "MINK_LOCALE",                     Goose: "MINK_LOCALE"},
    "LEARNING_ENABLED":           {Mink: "MINK_LEARNING_ENABLED",           Goose: "MINK_LEARNING_ENABLED"},
    "CONFIG_STRICT":              {Mink: "MINK_CONFIG_STRICT",              Goose: "MINK_CONFIG_STRICT"},
    "GRPC_REFLECTION":            {Mink: "MINK_GRPC_REFLECTION",            Goose: "MINK_GRPC_REFLECTION"},
    "GRPC_MAX_RECV_MSG_BYTES":    {Mink: "MINK_GRPC_MAX_RECV_MSG_BYTES",    Goose: "MINK_GRPC_MAX_RECV_MSG_BYTES"},
    "SHUTDOWN_TOKEN":             {Mink: "MINK_SHUTDOWN_TOKEN",             Goose: "MINK_SHUTDOWN_TOKEN"},
    "HOOK_TRACE":                 {Mink: "MINK_HOOK_TRACE",                 Goose: "MINK_HOOK_TRACE"},
    "HOOK_NON_INTERACTIVE":       {Mink: "MINK_HOOK_NON_INTERACTIVE",       Goose: "MINK_HOOK_NON_INTERACTIVE"},
    "ALIAS_STRICT":               {Mink: "MINK_ALIAS_STRICT",               Goose: "MINK_ALIAS_STRICT"},
    "QWEN_REGION":                {Mink: "MINK_QWEN_REGION",                Goose: "MINK_QWEN_REGION"},
    "KIMI_REGION":                {Mink: "MINK_KIMI_REGION",                Goose: "MINK_KIMI_REGION"},
    "TELEGRAM_BOT_TOKEN":         {Mink: "MINK_TELEGRAM_BOT_TOKEN",         Goose: "MINK_TELEGRAM_BOT_TOKEN"},
    "AUTH_TOKEN":                 {Mink: "MINK_AUTH_TOKEN",                 Goose: "MINK_AUTH_TOKEN"},
    "AUTH_REFRESH":               {Mink: "MINK_AUTH_REFRESH",               Goose: "MINK_AUTH_REFRESH"},
    "HISTORY_SNIP":               {Mink: "MINK_HISTORY_SNIP",               Goose: "MINK_HISTORY_SNIP"},
    "METRICS_ENABLED":            {Mink: "MINK_METRICS_ENABLED",            Goose: "MINK_METRICS_ENABLED"},
    "GRPC_BIND":                  {Mink: "MINK_GRPC_BIND",                  Goose: "MINK_GRPC_BIND"},
    // Note: MINK_AUTH_ prefix is handled separately in internal/hook/isolation_*.go
...
```
{"level":"warn","ts":"2026-05-13T12:00:00.000+0900","msg":"deprecated env var, please rename","old":"MINK_LOG_LEVEL","new":"MINK_LOG_LEVEL","spec":"SPEC-MINK-ENV-MIGRATE-001"}
```
conflict format:
```
{"level":"warn","ts":"...","msg":"both legacy and new env var set; using new key","new":"MINK_LOG_LEVEL","old":"MINK_LOG_LEVEL","value_source":"MINK_LOG_LEVEL"}
```
...
| R5 | `MINK_TELEGRAM_BOT_TOKEN` 의 runtime read 미구현 — alias 등록만 하면 사용자 혼란 가능 ("env 설정했는데 동작 안 함") | Low | error message 의 hint 만 update (Phase 5), runtime read 신설은 OUT scope 명시 |
| R6 | `MINK_AUTH_*` prefix glob 변경 시 기존 보안 테스트 회귀 | High | AC-MINK-EM-009 + 명시 sub-test 추가 + race detector |
...
| R11 | 사용자 dotfile (`.bashrc`, `.zshrc`) 에 `export MINK_HOME=...` 만 있을 때 첫 alias warning 출력 | Low | release notes 에 dotfile rewrite guidance 명시. AC-MINK-EM-005 의 warning 메시지 format 에 user-actionable 한 hint ("Replace `MINK_HOME` with `MINK_HOME` in your shell rc files") 포함 |
| R12 | Docker / k8s container env spec 에 `MINK_*` 만 정의된 경우 alias 동작은 OK 이나 manifest update 가 외부 작업 영역 | Low | 본 SPEC IN-scope 외 명시 (§11 #3). 후속 docs SPEC trigger 또는 release notes link 로 처리. alias loader 가 자동 흡수하므로 즉시 깨지지 않음 |
...
|---|-----------|-----------|----------|
| 1 | `MINK_*` env var 의 alias 등록 삭제 (완전 제거) | 사용자 마이그레이션 시간 보장 (최소 1+ minor release) | SPEC-MINK-ENV-CLEANUP-001 (후속) |
| 2 | user-data path migration (`~/.goose → ~/.mink`) | 데이터 손실 위험 + 별도 마이그레이션 도구 필요 | SPEC-MINK-USERDATA-MIGRATE-001 (별도) |
| 3 | `.env.example`, `Dockerfile`, `docker-compose.yaml`, k8s manifests 의 `MINK_*` 표기 update | 외부 운영 자원 (alias loader 가 자동 흡수) | 후속 docs SPEC + release notes |
| 4 | viper 도입 (현재 `os.Getenv` 직접 호출 → viper 추상화로 전환) | 본 SPEC 은 alias 도입만 — refactor 범위 분리 | 별도 refactor SPEC (도입 미정) |
| 5 | `MINK_TELEGRAM_BOT_TOKEN` 의 runtime read logic 신설 (현재는 error message hint 만) | behavior change scope minimization | 별도 SPEC (필요 시) |
| 6 | `MINK_HISTORY_SNIP`, `MINK_METRICS_ENABLED`, `MINK_GRPC_BIND` 의 runtime read logic 신설 | 동일 (현재 미구현 feature gate) | 각 feature 의 별도 SPEC |
| 7 | cobra flag 의 default value alias (e.g., `--token` flag default 가 MINK_TELEGRAM_BOT_TOKEN 참조 시) | 현재 코드에 해당 패턴 없음 (research.md §1.1 line 16 확인) | 본 SPEC 외 |
