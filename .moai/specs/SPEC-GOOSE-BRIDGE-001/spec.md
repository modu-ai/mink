...
Bridge는 단일 머신 내부에서 `goosed` daemon과 **로컬 브라우저의 Web UI**를 연결하는 **localhost 전용 통신 bridge**이다. 비개발자가 CLI 없이 GOOSE를 사용하기 위한 Web UI(별도 Phase 6 산출물, Skill: MOAI-WEBUI-*)가 WebSocket 또는 SSE로 daemon에 접속할 때, 그 통신 계약과 구현을 규정한다.
...
CLI 경험이 없는 사용자가 GOOSE를 쓰려면 브라우저 UI가 필요하다. Web UI는 정적 번들(HTML/JS/CSS)이지만, LLM 응답은 스트리밍이므로 **양방향 영구 연결**이 필요하다. 해당 연결 요구를 충족하는 가장 단순한 경로:
...
| ------------------- | ----------------------------------------- | -------------------------------------------- |
| 클라이언트 유형     | Native (goose CLI, goose-desktop)         | Browser (Web UI)                             |
