...
# SPEC-MINK-BRAND-RENAME-001 — MINK → MINK 전역 rename
...
`AI.MINK` 프로젝트의 brand 와 code identifier 를 모두 `MINK` (Made IN Korea, 산문) / `mink` (식별자·slug)로 통일한다. Go module path 변경 (`github.com/modu-ai/goose` → `github.com/modu-ai/mink`), GitHub repo rename (`modu-ai/goose` → `modu-ai/mink`), proto package rename (`goose.v1` → `mink.v1`), CLI binary rename (`goose`/`goosed`/`goose-proxy` → `mink`/`minkd`/`mink-proxy`)을 단일 SPEC PR squash로 합쳐 실행한다. immutable 이력 (88개 SPEC-GOOSE-* 디렉토리, 모든 SPEC HISTORY rows, 기존 CHANGELOG entries, git commit messages, PR titles)은 변경 금지.
...
- 산문은 `AI.MINK` 인데 Go module 은 `github.com/modu-ai/goose`
- 산문은 `AI.MINK` 인데 README h1 은 `# 🪿 MINK`
- SPEC 본문은 `AI.MINK` 인데 SPEC ID 는 `SPEC-GOOSE-XXX-NNN`
...
8. **코드 내 user-facing 문자열 (Go)**: log message, error message, CLI help text, doc-comment 의 brand-position 토큰 정정. `AI.MINK` (27건, §research §2.12) → `MINK`. brand-position `Goose` 단어 (~5건 test fixture 포함) → `Mink`.
...
- **REQ-MINK-BR-012 [Event-Driven]** **When** the brand-lint script (`scripts/check-brand.sh`) runs against a user-facing markdown file outside exemption zones, **and** the file contains any of {`goose 프로젝트`, `Goose 프로젝트`, `MINK 프로젝트`, `goose project`, `Goose project`, `GOOSE-AGENT`, `AI.MINK`} outside backticks/code-block, the script **shall** flag the line as a brand violation and **shall** exit with code 1.
...
**When** 다음을 모두 검증하면:
1. `scripts/check-brand.sh` 의 violation patterns 가 `goose 프로젝트`, `Goose 프로젝트`, `GOOSE-AGENT`, `goose project`, `Goose project`, `AI.MINK` (산문) 6개로 retarget 됨
...
4. 의도적으로 brand 위반 (`MINK 프로젝트` 추가) 도입한 시험 PR 이 brand-lint check 실패로 merge 차단
...
3. `cmd/minkd/main.go` 가 존재하며 doc-comment 가 `MINK` brand 표기 사용 (`AI.MINK` / `MINK` 0건)
...
**When** 다음을 검증하면:
1. `grep -rn 'AI\.MINK' --include='*.go' | wc -l` = 0
2. `grep -rn '\bMINK\b' --include='*.go' | wc -l` ≥ 27 (baseline 27 이상)
3. `./mink --help` 출력 첫 줄에 `MINK` 등장, `AI.MINK` / `MINK` / `Goose` brand-position 0건
...
2. `grep -rEln '\bAI\.MINK\b' --include='*.md' --exclude-dir='.moai/specs/SPEC-GOOSE-*' --exclude-dir='.moai/brain/IDEA-*' --exclude-dir='.claude/agent-memory'` 출력 0 파일 (산문에서 `AI.MINK` 잔존 0건)
...
**Given** Phase 7 (문서 sweep) commit 적용 후
**When** 다음을 검증하면:
1. README.md 의 brand-position `goose` / `Goose` / `MINK` / `AI.MINK` 0건 (단, 백틱 인용 `` `mink CLI` `` 등 code identifier 는 제외; install snippet `go build ./cmd/mink` 는 정확히 그대로 노출)
...
**Given** Phase 7 (문서 sweep) commit 적용 후
**When** 다음을 검증하면:
1. `grep -rEln '\bAI\.MINK\b' .moai/project/` 출력 0 파일 (산문 brand 정정 완료, brand-style-guide-mink 신설 후 옛 표기 잔존 0)
...
3. 의도적으로 baseline release section 의 brand-position 토큰 (`AI.MINK`, `GOOSE-AGENT`, `goose 프로젝트` 등) 을 수정한 시험 PR 이 brand-lint check 실패로 merge 차단됨
...
- scripts/check-brand.sh 위반 패턴 retarget: AI.MINK, GOOSE-AGENT, Goose 프로젝트
...
3. `find cmd/mink cmd/minkd -name '*.go' -exec sed -i.bak 's|\bgoosed\b|minkd|g; s|MINK\b|MINK|g' {} +` (cmd 디렉토리 내 brand 토큰 정정, doc-comment 위주)
...
- cmd/goose/ → cmd/mink/ (git mv)
- cmd/goosed/ → cmd/minkd/ (git mv)
- cmd/{mink,minkd}/*.go: brand 토큰 정정 (goosed → minkd, MINK → MINK, doc-comment)
...
**도구**: Edit (manual per file, 도메인 용어 보존 위해 자동 sed 금지). 단, `AI.MINK` → `MINK` 같은 명확한 brand-position 토큰은 batch sed 가능: `grep -rEln '\bAI\.MINK\b' .moai/project/ | xargs sed -i.bak 's/\bAI\.MINK\b/MINK/g'`.

**검증**:
- `bash scripts/check-brand.sh` exit 0
- `grep -rEln '\bAI\.MINK\b' .moai/project/` 출력 0 파일
...
- .moai/project/*.md (9 파일): AI.MINK → MINK
...
**도구**:
1. `grep -rln 'AI\.MINK' --include='*.go' | xargs sed -i.bak 's/\bAI\.MINK\b/MINK/g'`
2. 도메인 용어 (예: `// Package command implements the slash command system for MINK.`)는 자동 결과 검토
3. `go vet ./...` + `go test ./...`

**검증**:
- `grep -rn 'AI\.MINK' --include='*.go' | wc -l` = 0
...
- 27 .go 파일: AI.MINK → MINK (doc-comment 위주)
...
3. **위반 검출 (코드 영역 외부)**: PCRE2 또는 Python regex 로 다음 패턴 검출:
   - `goose 프로젝트`, `Goose 프로젝트`, `MINK 프로젝트` (한국어 brand-position)
   - `goose project`, `Goose project` (영문 brand-position)
   - `GOOSE-AGENT` (옛 brand 약칭)
   - `\bAI\.MINK\b` (선행 SPEC 결과의 brand 표기 잔존)
...
- `SPEC-MINK-ENV-MIGRATE-001` (downstream): `MINK_*` 21개 env var → `MINK_*` deprecation alias loader. 본 SPEC §3.1 item 12 footnote 에서 분리 명시.
...
| R9 | env var (`MINK_*` 21개) 변경 → 개발자 환경 break | 중 | 중 | 새 `MINK_*` env var 신설 + 옛 `MINK_*` deprecated alias loader. 본 SPEC 은 정책만 (§3.1 item 12 footnote, REQ-MINK-BR-027). 실제 loader 는 별도 SPEC `SPEC-MINK-ENV-MIGRATE-001`. |
...
- **IDEA-002**: GooseBot 한국시장 단타 LLM-bot 분리 결정 + AI.MINK → MINK rename 결정 (별도 프로젝트 `/Users/goos/Projects/GooseBot/.moai/brain/IDEA-002/` 에 ideation/proposal/research 보존)
...
11. **`SPEC-MINK-ENV-MIGRATE-001`** (`MINK_*` 21개 env var → `MINK_*` deprecation alias loader) — 별도 SPEC. 본 SPEC 은 정책만 (§3.1 item 12 footnote, REQ-MINK-BR-027).
