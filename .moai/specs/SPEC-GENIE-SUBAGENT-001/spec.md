...
# SPEC-GENIE-SUBAGENT-001 — Sub-agent Runtime (Fork / Worktree / Background Isolation + 3 Memory Scope)
...
GENIE-AGENT의 **Sub-agent 런타임**을 정의한다. Claude Code의 `runAgent()` 생명주기(3단계) + 3종 isolation(fork / worktree / background) + 3-scope memory(user / project / local) + role profile override를 Go로 포팅하여, 하나의 부모 `QueryEngine`(QUERY-001)에서 여러 sub-agent를 순차·병렬로 spawn하고, 각각의 tool budget·권한 정책·메모리 디렉토리를 독립적으로 유지한다.
...
- 3-scope memory: `~/.genie/agent-memory/{agentType}/`(user), `./.genie/agent-memory/{agentType}/`(project), `./.genie/agent-memory-local/{agentType}/`(local, gitignored),
...
6. Memory 3-scope:
   - `user`: `$HOME/.genie/agent-memory/{agentType}/`,
   - `project`: `{projectRoot}/.genie/agent-memory/{agentType}/`,
   - `local`: `{projectRoot}/.genie/agent-memory-local/{agentType}/` (gitignored).
...
- **When** 완료 후 `~/.genie/agent-memory/researcher/transcript-{agentId}/` 조회
...
- **Given** `~/.genie/agent-memory/researcher/` 디렉토리 생성 시
...
```
~/.genie/agent-memory/researcher/           # user scope
...
./.genie/agent-memory/researcher/           # project scope (동일 구조)
./.genie/agent-memory-local/researcher/     # local scope (동일 구조, .gitignore)
```
...
func createWorktree(agentID string) (string, func(), error) {
    ...
    branch := fmt.Sprintf("genie/agent/%s", sanitize(agentID))
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-QUERY-001 | sub-agent를 감싸는 `QueryEngine` 재사용 |
| 선행 SPEC | SPEC-GENIE-SKILLS-001 | `AgentDefinition`의 frontmatter 파서 allowlist 공유 |
| 선행 SPEC | SPEC-GENIE-HOOK-001 | `SubagentStart`/`Stop`/`TeammateIdle`/`WorktreeCreate`/`Remove` dispatcher, `SwarmWorkerHandler` |
| 선행 SPEC | SPEC-GENIE-CORE-001 | zap 로거, context 루트, graceful shutdown |
| 후속 SPEC | SPEC-GENIE-MCP-001 | `def.MCPServers`의 `ConnectToServer` 호출 |
| 후속 SPEC | SPEC-GENIE-PLUGIN-001 | plugin manifest `agents:` 로딩 |
| 후속 SPEC | SPEC-GENIE-ROUTER-001 | `def.Model` alias 해석 |
| 후속 SPEC | SPEC-GENIE-MEMORY-001 (Phase 4) | memdir.jsonl의 구조화 저장 — 본 SPEC은 파일 기반만 |
...
| R8 | Worktree cleanup 실패로 디스크 누수 | 중 | 낮 | `SessionEnd` hook + startup scan(`git worktree prune` + orphan directory 제거). 수동 `genie worktree gc` 커맨드 제공 |
...
- `.moai/specs/SPEC-GENIE-QUERY-001/spec.md` — `TeammateIdentity`, `CoordinatorMode`, `CanUseTool`
- `.moai/specs/SPEC-GENIE-SKILLS-001/spec.md` — frontmatter allowlist 공유
- `.moai/specs/SPEC-GENIE-HOOK-001/spec.md` — subagent lifecycle hooks, permission flow
...
- `../SPEC-GENIE-QUERY-001/spec.md`
- `../SPEC-GENIE-HOOK-001/spec.md`
- `../SPEC-GENIE-MCP-001/spec.md`
...
**End of SPEC-GENIE-SUBAGENT-001**
