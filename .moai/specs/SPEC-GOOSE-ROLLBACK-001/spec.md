...
# SPEC-GOOSE-ROLLBACK-001 — Regression 자동 감지 및 30일 Staleness Rollback
...
- **learning-engine.md §6.3**:
  ...
  ```
  GOOSE는 warning 없이 0.10 이상 회귀 즉시 rollback (위 임계값 그대로).
- **SPEC-REFLECT-001 (MoAI)**: 동일 rollback 메커니즘. GOOSE는 `Restorer` hand-off를 LoRA까지 확장.
...
- `goosed` 데몬 부트스트랩 시 `watcher.Start(ctx)` 호출
...
- SPEC-GOOSE-REFLECT-001 (MarkRolledBack 수신자, RollbackDeadline 설정)
- SPEC-GOOSE-SAFETY-001 (CanaryCheck 사전 예방과의 역할 분리)
- SPEC-GOOSE-TRAJECTORY-001 (session score 데이터 소스)
- SPEC-GOOSE-INSIGHTS-001 (satisfaction score 계산)
- SPEC-GOOSE-LORA-001 (Restorer 주요 구현체)
- SPEC-GOOSE-HOOK-001 (HookEventRollbackManualIntervention)
