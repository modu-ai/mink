# Research — SPEC-GOOSE-ROLLBACK-001 (Regression 자동 감지 + Rollback)

> **작성일**: 2026-04-21
> **대상 SPEC**: SPEC-GOOSE-ROLLBACK-001
...
GOOSE는 "next project score" → "next session score"로 치환하지만 **0.10 임계값과 30일 staleness는 그대로 계승**한다.
...
**GOOSE 적용**: learning-engine은 `newAvg < 0.75` OR `regression > 0.10` 두 조건 중 하나를 warning/rollback으로 나눠 다룬다. **본 SPEC은 단순화하여 "regression > 0.10 즉시 rollback"만 구현**하고, warning 레벨은 초기 버전에서 제외한다. 이유:

1. Warning → 사용자 알림만 하는 중간 상태가 GOOSE에 아직 불필요 (사용자가 개입할 UI가 제한적)
...
- 분산이 매우 큰 경우: 본 SPEC은 mean만 사용. 향후 SPEC-GOOSE-ROLLBACK-002에서 t-test/Welch's test 등 통계적 유의성 검정 추가 고려.
...
1. **Warning 레벨 도입 시기**: learning-engine §6.3의 `StatusWarning`(0.05~0.10 회귀)은 본 SPEC 범위 외. SPEC-GOOSE-ROLLBACK-002에서 사용자 알림 채널 정의 시 도입.
...
- SPEC-GOOSE-REFLECT-001 (MarkRolledBack, RollbackDeadline 관리자)
- SPEC-GOOSE-SAFETY-001 (CanaryCheck — 사전 예방, ROLLBACK과 역할 분리)
- SPEC-GOOSE-TRAJECTORY-001 (session 데이터 소스)
- SPEC-GOOSE-INSIGHTS-001 (satisfaction score 산출)
- SPEC-GOOSE-LORA-001 (Restorer 주요 구현체)
- SPEC-GOOSE-HOOK-001 (HookEventRollbackManualIntervention 수신자)
