# SPEC-GENIE-LORA-001 research.md — LoRA Trainer 구현 자료 (Go 레이어)

> 본 문서는 Go 인터페이스 측의 실구현 참고 자료이다. Rust `genie-ml` crate 의 내부 구현은 본 문서에 포함하지 않는다 (→ ROADMAP-RUST.md).
...
└──────────────┬──────────────────────────────┘
               │ gRPC over unix:///tmp/genie-ml.sock
               ▼
┌─────────────────────────────────────────────┐
│ genie-ml (Rust process, spawned by genied)  │
│  crates/genie-ml/                           │
│    ├─ src/server.rs       (tonic)           │
...
- `exec.Command("genie-ml", "--socket", sockPath, "--home", geniehome)`
  ...
  "versions": [
    {
      ...
      "use_dora": true,
      "use_qlora": true,
      "file_path": "$GENIE_HOME/lora/user-abc123/v1.0.0.safetensors",
...
| 30-day cooldown 디스크 부족 | Medium | `genie lora gc --force` CLI + 사용자 확인 |
| EWC Fisher 벡터 bandwidth | Low | 압축(gzip) 옵션, default off |
| CGO 플랫폼 편차 | High | Phase 6 첫 릴리즈는 gRPC only, CGO 는 후속 |
| Go process 종료 시 genie-ml orphan | Medium | parent-death signal (Linux prctl, macOS kqueue), 아니면 heartbeat timeout self-exit |
...
- Prometheus metrics:
  - `genie_lora_train_total{result=success|failed}`
  - `genie_lora_train_duration_seconds`
  - `genie_lora_apply_drops_total`
  - `genie_lora_backend_degraded_seconds`
  - `genie_lora_rollback_total{reason=...}`
- Structured logs (zap):
...
$ genie lora train --replay-ratio 0.3 --use-ewc
[gate] conversations=1240 ✅ quality=0.81 ✅ frozen=OK ✅ rate=OK ✅ approval=granted ✅
[spawn] genie-ml PID 42311, socket /tmp/genie-ml.sock
...
$ genie lora rollback v1.2.0 --reason "new version misses formal tone"
...
- tech.md §2.1 (genie-ml Rust crate) → 본 SPEC 의 Rust 위임 경계.
...
| Rust 프로세스 독립 배포 | `genie-ml` 바이너리 별도 릴리즈, genied 가 exec 로만 호출 |
