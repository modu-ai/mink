...
# SPEC-GENIE-LORA-001 — User-specific QLoRA Trainer (Go 인터페이스 + Rust 위임 경계)

## HISTORY

| 버전 | 날짜 | 변경 사유 | 담당 |
|-----|------|---------|------|
| 0.1.0 | 2026-04-21 | 초안 작성 (learning-engine.md §5-6 + tech.md §2.1 genie-ml 기반) | manager-spec |

---

## 1. 개요 (Overview)

GENIE 사용자별 **QLoRA adapter** (10~200 MB) 를 생성·관리·핫스왑·롤백하는 **Go 인터페이스 레이어** 를 정의한다. 실제 tensor 수학·GPU 커널·ONNX 실행 같은 **성능 크리티컬 레이어는 Rust `genie-ml` crate 로 위임**하며, Go 는 훈련 오케스트레이션·데이터셋 빌드·안전장치 통합·버전관리·서빙에 집중한다.
...
본 SPEC 은 `internal/learning/lora/` 패키지의 인터페이스·불변식·에러를 규정한다. Rust crate `crates/genie-ml/` 은 **별도 로드맵(ROADMAP-RUST.md)** 에서 관리되며 본 SPEC 은 그 crate 의 존재를 가정하고 인터페이스만 정의한다.
...
- `tech.md` §2.1 은 `genie-ml` Rust crate 가 candle/burn/ort/safetensors 로 실제 훈련을 수행한다고 명시. Go 는 **오케스트레이션 만**.
...
| 항목 | Go (본 SPEC) | Rust (genie-ml, ROADMAP-RUST.md) |
|-----|-------------|--------------------------------|
| 훈련 오케스트레이션 | ✅ | — |
...
4. gRPC 프로토콜 `crates/genie-ml` 과 공유 (proto 파일 `proto/lora/v1/trainer.proto`).
...
8. `VersionRegistry` — `$GENIE_HOME/lora/versions.json` (SemVer + SHA-256 checksum + timestamp).
...
- **Tensor 연산 / gradient 계산**: Rust `genie-ml`.
...
All tensor-level computation (gradient, quantization, ONNX inference) shall be delegated to the external `genie-ml` Rust service via gRPC or CGO. The Go layer shall not implement tensor arithmetic.

### REQ-LORA-003 [State-Driven]
While the configuration value `lora.ipc` is `grpc` (default), the Go layer shall communicate with `genie-ml` over a gRPC connection at `lora.grpc_endpoint` (default `unix:///tmp/genie-ml.sock`).
...
When `Train` succeeds, the resulting adapter file shall be written to `$GENIE_HOME/lora/<userID>/<semver>.safetensors` and registered in `versions.json` atomically (temp file + `os.Rename`).
...
While the health-check to `genie-ml` Rust service fails for 10 consecutive seconds, the service shall mark itself `degraded` and reject `Train`/`Apply` calls with `ErrBackendDegraded`; `Evaluate` and read-only `ListVersions` shall remain available.
...
Rollback 가능성을 보장하기 위해 삭제 미루기. 디스크 용량 초과 시 `genie lora gc --force` 로 수동 삭제 (경고 메시지 + 사용자 확인).
...
type LoRAAdapter struct {
    ...
    FilePath       string           // $GENIE_HOME/lora/<userID>/<version>.safetensors
    SizeBytes      int64
...
type EvaluationScore struct {
    ...
    Confidence   float64
    Evaluator    string   // "genie-ml:<version>"
}
...
- Tensor arithmetic / gradient computation (→ Rust `genie-ml`).
...
Given `genie-ml` 서비스가 10 초간 health-check 실패할 때, When `Train` 을 호출하면, Then `ErrBackendDegraded` 가 반환되고 `ListVersions` 는 여전히 정상 응답한다.
...
- Mock gRPC server (Go): `genie-ml` 프로토콜 시뮬레이터. 각 `TrainProgress` 스트림 단계 검증.
- 통합: 실제 Rust `genie-ml` 바이너리 (빌드 artifact) + `-tags=integration`.
...
- **하위 소비자**: QueryEngine (SPEC-GENIE-QUERY-001)의 inference path 가 `Apply(version)` 결과의 adapter 를 사용 (이는 ADAPTER-001 의 책임).
- **Rust crate**: `crates/genie-ml/` — 본 SPEC 의 범위 외. ROADMAP-RUST.md 에서 관리.
- **CLI 영향**: `genie lora list`, `genie lora train`, `genie lora rollback`, `genie lora gc`, `genie lora apply <version>`.

---

## 14. 오픈 이슈 (Open Issues)

1. **Rust 서비스 배포 전략**: genie-ml 을 `genied` 내부에 embedded(port 공유)로 둘지, 별도 `genie-ml` 바이너리로 둘지. 현재 계획은 별도 프로세스 + unix socket.
