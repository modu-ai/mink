...
# SPEC-GENIE-CREDPOOL-001 — Credential Pool (OAuth/API 통합, 4 Strategy, Rotation)
...
GENIE-AGENT Phase 1의 **Multi-LLM Infrastructure 진입점**을 정의한다. Hermes Agent의 `credential_pool.py`(~1.3K LoC) 원형을 Go로 포팅하여, OAuth 토큰과 API key 자격을 하나의 풀에 통합하고, 선택 전략(4종)·자동 갱신·고갈 쿨다운·소프트 임대·외부 디스크 동기화를 제공하는 `internal/llm/credential` 패키지를 구현한다.
...
8. 영속 상태: `~/.genie/credentials/<provider>.json`에 `last_status`, `last_status_at`, `last_error_code`, `last_error_reset_at`, `request_count`, `expires_at_ms` 저장. `Load()` 시 복원, `MarkExhausted/Refresh` 시 atomic write(tmp + rename).
...
- **크로스 프로세스 락**: 단일 `genied` 프로세스 가정. 다중 프로세스 락(flock)은 후속.
...
**REQ-CREDPOOL-004 [Ubiquitous]** — Every mutation **shall** be immediately persisted to `~/.genie/credentials/<provider>.json` via atomic write (temp file + rename) before the public method returns.
...
- **Then** (1) `a.status == EXHAUSTED`, `a.last_error_reset_at == now + 60s`, (2) 반환값은 `b`이고 `b.status == OK`, (3) `~/.genie/credentials/<provider>.json`에 `a`의 상태가 persisted됨
...
`~/.genie/credentials/<provider>.json` 예시:
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-CORE-001 | zap logger, `GENIE_HOME` 의미 상속 |
| 선행 SPEC | SPEC-GENIE-CONFIG-001 | `LLMConfig.Providers[*]`로부터 초기 엔트리 구성 |
| 후속 SPEC | SPEC-GENIE-ROUTER-001 | 풀에서 provider 단위 자격 선택 |
| 후속 SPEC | SPEC-GENIE-RATELIMIT-001 | 429 status + Retry-After 헤더를 `MarkExhaustedAndRotate`로 전달 |
| 후속 SPEC | SPEC-GENIE-ADAPTER-001 | `Refresher` 인터페이스의 provider별 구현 주입 |
...
| R6 | 다중 `genied` 인스턴스가 동일 영속 파일에 쓰면 충돌 | 중 | 중 | Phase 1 범위 외(단일 인스턴스 가정). 후속 SPEC에서 flock 또는 SQLite로 이관 |
...
- `.moai/specs/SPEC-GENIE-CONFIG-001/spec.md` §6.2 `LLMConfig.Providers` 스키마
...
- 본 SPEC은 **크로스 프로세스 flock을 구현하지 않는다**. 단일 `genied` 프로세스 가정.
...
**End of SPEC-GENIE-CREDPOOL-001**
