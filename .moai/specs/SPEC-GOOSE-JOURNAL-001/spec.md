...
MINK v6.0 Daily Companion의 **저녁 리추얼**을 담당한다. 사용자 지시:
...
> **Milestone 분할 (v0.2.1 추가)** — plan.md §1 표 참조.
> - **M1 (P0)**: 항목 1~5, 9, 10, 11 (Writer + storage + Local emotion + Crisis + 저녁 프롬프트 + Config + 사용자 제어 API).
> - **M2 (P0)**: 항목 6, 7, 8 (TrendAggregator + MemoryRecall + AnniversaryDetector).
> - **M3 (P1)**: §6.3 의 LLM-assisted emotion analyzer (REQ-017) + §3.1 항목 6 의 weekly summary LLM 향상 (REQ-018).
> M1 → M2 → M3 순차 진입. M1 완료 시 일기 입력 + 로컬 감정 + crisis canned response 가 동작.
