...
# SPEC-GENIE-REFLECT-001 — 5단계 승격 파이프라인 (Observation → Graduated)
...
GENIE의 자기진화 루프에서 **INSIGHTS-001이 추출한 관찰 후보**와 **MEMORY-001에 저장된 학습 이력**을 입력으로 받아, 관찰(Observation)을 순차적으로 Heuristic → Rule → HighConfidence → Graduated로 승격하거나 Anti-Pattern/Rolled-back/Archived로 격리하는 **상태 머신(state machine)**을 정의한다.
...
본 SPEC은 상태 전이 규칙과 `LearningEntry` 영속화만 책임지며, 승격 결과를 실제 사용자 상태에 적용하는 작업(LoRA 교체, skill body 수정 등)은 `SPEC-GENIE-SAFETY-001`의 Human Oversight gate와 하위 소비자 SPEC에 위임한다.
...
- `SPEC-GENIE-INSIGHTS-001`이 Pattern/Preference/Error/Opportunity 4종 insight을 추출하지만, **관찰 한 번만으로 사용자 상태를 변경하면 위험하다** (오탐, 일시적 기분, 세션 특수 맥락). 이를 걸러내는 승격 시스템이 없으면 INSIGHTS의 출력이 실제 진화로 이어질 수 없다.
- `SPEC-GENIE-MEMORY-001`이 학습 결과를 영속화하지만 **"어떤 상태로 저장할지"에 대한 의미론을 MEMORY는 알지 못한다.** REFLECT가 상태 계약을 독점한다.
...
- **agency/constitution §6 Learnings Pipeline**: Observations/Heuristic/Rule/HighConfidence 명명법과 Anti-Pattern 즉시 FROZEN 규칙의 원형. 본 SPEC이 개인 학습(GENIE)에 적용하는 파생이다.
...
- **OUT**: 실제 LoRA 훈련/교체 (LORA-001), skill body 수정 (GENIE-SKILLS-001의 consumer), 사용자 승인 다이얼로그 (SAFETY-001의 Human Oversight), frozen path 차단 (SAFETY-001의 FrozenGuard), canary 재평가 (SAFETY-001의 CanaryCheck), rollback 실행 (ROLLBACK-001), insight 생성 알고리즘 (INSIGHTS-001), 저장 백엔드 선택 (MEMORY-001).
...
- LoRA adapter 교체/훈련 실행 → `SPEC-GENIE-LORA-001`
- 사용자 승인 다이얼로그, AskUserQuestion 연계 Hook 이벤트 발행 → `SPEC-GENIE-SAFETY-001` + `SPEC-GENIE-HOOK-001`
...
- **ZoneID**: `CLAUDE.md`의 `<!-- genie:evolvable-start id="..." -->` block 식별자 또는 LoRA adapter 영역 식별자.
...
- `SPEC-GENIE-INSIGHTS-001` (Ingest 입력 생산자)
- `SPEC-GENIE-MEMORY-001` (영속화 백엔드)
- `SPEC-GENIE-SAFETY-001` (HumanOversight, CanaryCheck, FrozenGuard, SafetyToken 발급)
- `SPEC-GENIE-ROLLBACK-001` (MarkRolledBack 트리거)
