...
# SPEC-GENIE-TRAJECTORY-001 — Trajectory 수집 + 익명화 (ShareGPT JSON-L)
...
GENIE-AGENT **자기진화 파이프라인의 Layer 1**을 정의한다. QueryEngine의 한 턴(turn)이 끝날 때마다 발생하는 대화 기록을 **ShareGPT 호환 JSON-L 포맷**으로 로컬 디스크에 유실 없이 적재하고, 저장 직전 **Redact 파이프라인**으로 이메일·API 키·신용카드·전화번호·경로상 PII를 제거하며, 성공(`completed=true`) vs 실패(`completed=false`) 궤적을 물리적으로 분리 저장하여 Layer 2(COMPRESSOR) · Layer 3(INSIGHTS)의 입력을 준비한다.
...
- `Writer`가 `~/.genie/trajectories/{success|failed}/YYYY-MM-DD.jsonl`에 append-only 기록하고, 일일 파일 크기가 10MB를 넘거나 날짜가 바뀌면 회전(rotation)하며,
...
- 로드맵 v2.0 §4 Phase 4 첫 번째 SPEC. SPEC-GENIE-COMPRESSOR-001 · INSIGHTS-001 · MEMORY-001이 모두 본 SPEC의 `Trajectory` 타입을 import한다.
...
7. Disk layout: `${GENIE_HOME}/trajectories/success/YYYY-MM-DD.jsonl`, `${GENIE_HOME}/trajectories/failed/YYYY-MM-DD.jsonl`.
...
- **다중 사용자 격리**: Phase 4 MVP는 단일 사용자 로컬 홈(`~/.genie/`). 멀티테넌시는 Phase 7+.
...
**REQ-TRAJECTORY-005 [Event-Driven]** — **When** the QueryEngine's Terminal event fires with `success: true`, the `TrajectoryCollector` **shall** flush the session buffer to `${GENIE_HOME}/trajectories/success/YYYY-MM-DD.jsonl` (date computed in UTC).

**REQ-TRAJECTORY-006 [Event-Driven]** — **When** the QueryEngine's Terminal event fires with `success: false`, the `TrajectoryCollector` **shall** flush the session buffer to `${GENIE_HOME}/trajectories/failed/YYYY-MM-DD.jsonl` with `TrajectoryMetadata.failure_reason` populated from the Terminal `error` field.
...
- **When** Terminal(success:true) 발화 후 `~/.genie/trajectories/success/2026-04-21.jsonl` 읽기
...
- **When** Writer flush
- **Then** `~/.genie/trajectories/failed/2026-04-21.jsonl`에 기록, `TrajectoryMetadata.failure_reason == "context_overflow"` 필드 존재, `success/` 디렉토리에는 동일 session_id 기록 없음
...
- **Then** `~/.genie/trajectories/` 디렉토리가 생성되지 않거나 빈 상태, Writer goroutine 미생성(runtime.NumGoroutine delta == 0)
...
- **Given** `{from:"system", value:"You are Genie. Email support@goose.ai for help."}`
...
- **When** 101번째 턴 도착
- **Then** 50턴이 `${GENIE_HOME}/trajectories/success/2026-04-21.jsonl`에 `{partial:true}` 플래그로 append, 메모리 버퍼는 50턴으로 축소
...
본 SPEC은 SPEC-GENIE-QUERY-001의 `PostSamplingHooks`, `StopFailureHooks`, Terminal 이벤트를 consumer로 사용한다. 주입 책임은 GENIE 부트스트랩(CORE-001 + 본 SPEC 등록자)에 있다.
...
| 타입 | 대상 | 설명 |
|-----|------|------|
| 선행 SPEC | SPEC-GENIE-CORE-001 | `GENIE_HOME` 해석, zap 로거, context 루트 |
| 선행 SPEC | SPEC-GENIE-QUERY-001 | `PostSamplingHooks`, `StopFailureHooks`, Terminal 이벤트, `SessionID` |
| 선행 SPEC | SPEC-GENIE-CONFIG-001 | `config.telemetry.trajectory.*` 로드 |
| 후속 SPEC | SPEC-GENIE-COMPRESSOR-001 | 본 SPEC의 `Trajectory` 타입을 입력으로 |
| 후속 SPEC | SPEC-GENIE-INSIGHTS-001 | `.jsonl` 파일을 입력으로 스캔 |
| 후속 SPEC | SPEC-GENIE-MEMORY-001 | `SessionID` 공유 |
...
- `.moai/specs/SPEC-GENIE-QUERY-001/spec.md` §4.5 REQ-QUERY-018 (PostSamplingHooks)
...
- `../SPEC-GENIE-COMPRESSOR-001/spec.md` — 본 SPEC의 `Trajectory` 소비자
- `../SPEC-GENIE-INSIGHTS-001/spec.md` — `.jsonl` 파일 스캐너
...
- 본 SPEC은 **다중 사용자 / 멀티테넌시 격리를 포함하지 않는다**(단일 사용자 `~/.genie/`).
- 본 SPEC은 **LoRA 훈련 데이터셋 변환을 포함하지 않는다**. LORA-001.

---

**End of SPEC-GENIE-TRAJECTORY-001**
