...
- 로드맵 v2.0 §4 Phase 4 #23. v1.0에서는 MEMORY SPEC이 없었기에 **신규 작성**(기존 SPEC-GOOSE-MEMORY-001이 레포에 존재하지 않음 확인 필요).
...
**REQ-MEMORY-019 [Optional]** — **Where** `config.memory.plugin.name` is a known Go import path (e.g. `github.com/goose/memory-honcho`), the `MemoryManager` **shall** attempt to instantiate the plugin via the registered factory function in `internal/memory/plugin/registry.go`.
...
- **Given** `~/.goose/memory/USER.md` 파일에 "I prefer async over sync" 기록
...
- **Given** Builtin.MEMORY.md 경로 `~/.goose/memory/MEMORY.md`
...
type BuiltinProvider struct {
    ...
    dbPath     string
    memoryMd   string       // ~/.goose/memory/MEMORY.md
    userMd     string       // ~/.goose/memory/USER.md
    db         *sql.DB
    mu         sync.Mutex
    logger     *zap.Logger
}
...
| 후속 SPEC | SPEC-GOOSE-TOOLS-001 | `GetAllToolSchemas()`를 Tool Registry가 수집 |
| 후속 SPEC | SPEC-GOOSE-MCP-001 | Memory tool을 MCP 서버로 외부 노출 가능 |
| 후속 SPEC | SPEC-GOOSE-IDENTITY-001 (Phase 6) | Identity Graph를 Builtin 확장으로 주입 |
| 후속 SPEC | SPEC-GOOSE-VECTOR-001 (Phase 6) | Vector search provider로 추가 등록 가능 |
| 후속 SPEC | SPEC-GOOSE-REFLECT-001 (Phase 5) | graduated learning을 `facts` 테이블에 영속 |
...
- `.moai/specs/SPEC-GOOSE-TRAJECTORY-001/spec.md` — session_id 공유 공급자
- `.moai/specs/SPEC-GOOSE-QUERY-001/spec.md` — lifecycle hook 진입점
...
- `../SPEC-GOOSE-TRAJECTORY-001/spec.md` — session_id 공급자
- `../SPEC-GOOSE-INSIGHTS-001/spec.md` — MEMORY 통계 소비자
