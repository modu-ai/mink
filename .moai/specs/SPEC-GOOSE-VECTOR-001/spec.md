...
- **OpenAI `text-embedding-3-small`** (1536→768 dim reducible): MINK 는 dimensions=768 고정 호출로 사용.
...
While the configuration value `vector.backend` is `hnsw`, the service shall use the pure-Go HNSW fallback with a periodic file flush to `$MINK_HOME/vector/hnsw.idx`.
...
프라이버시 관점에서 raw user ID를 join 키로 사용하지 않는다. salted SHA-256 (`salt` 는 `$MINK_HOME` 에 저장된 device-local 값). 사용자가 명시적 opt-in 한 경우에만 연산 자체가 실행됨.
...
Embedding model 이 바뀌면 dim 이 달라질 가능성 높음. 본 SPEC 은 dim=768 고정, migration 은 별도 CLI 명령 (`mink vector reindex --from=<old> --to=<new>`)으로만 제공.
...
- **CLI 영향**: `mink vector status`, `mink vector reindex`, `mink vector export <userID>`.
...
4. **Salt 배포 정책**: 기기별 salt 를 `$MINK_HOME/.salt` 에 저장. 기기 이전 시 FindSimilarUsers 결과가 불연속. 사용자에게 고지 필요.
