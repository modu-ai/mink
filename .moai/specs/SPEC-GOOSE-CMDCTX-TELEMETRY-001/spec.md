...
| 0.1.1 | 2026-05-01 | **R1 BLOCKER 해소 amendment**. 선행 SPEC `SPEC-GOOSE-OBS-METRICS-001` (`metrics.Sink` interface + Phase 1 expvar backend + noop fallback) 이 PR #60 (https://github.com/modu-ai/goose/pull/60) 으로 implemented v0.1.0 도달. §9 R1 (🔴 1급 BLOCKER) → 🟡 중 (해소). §3.1 #1 의 sink import 경로를 `type MetricsSink = metrics.Sink` (`internal/observability/metrics`) alias 패턴으로 명시. status frontmatter 는 planned 유지 (run phase 진입 자격만 갱신). | manager-spec |

---

> **Amendment v0.1.1 (2026-05-01) — R1 BLOCKER 해소**
>
> 선행 SPEC `SPEC-GOOSE-OBS-METRICS-001` (vendor-neutral `metrics.Sink` interface + stdlib `expvar` Phase 1 backend + zero-cost noop fallback) 이 PR #60 으로 implemented v0.1.0. 본 SPEC §9 R1 (🔴 1급 BLOCKER) 해소. run phase 진입 가능 (implementation 자체는 별도 단계).
>
> 주요 정정:
> - §1 / §2.2 / §11: "(TBD, 부재)" 표기는 "(implemented v0.1.0, PR #60)" 로 해석.
> - §3.1 #1: `type MetricsSink = metrics.Sink` alias 패턴 사용 — `import "github.com/modu-ai/goose/internal/observability/metrics"`. 별도 interface 재정의 불필요. `Counter` / `Histogram` / `Gauge` / `Labels` 도 동일 alias.
> - §9 R1: 🔴 높음 (blocker) → 🟡 중 (해소됨, contract 호환 검증 완료).
> - §9 R1 의사결정 트리: Q1=YES 경로 활성, normative REQ (REQ-CMDCTX-TEL-001 ~ REQ-CMDCTX-TEL-014) 모두 구현 가능. §4.5 REQ-CMDCTX-TEL-018 (Logger fallback) 은 informational 로 격하.
> - frontmatter `status: planned` 유지 — implementation 시작은 별도 run phase 진입.
>
> Contract 호환 검증: PR #60 의 `metrics.Sink` interface signature (`Counter(name, labels) Counter`, `Histogram(name, labels, buckets) Histogram`, `Gauge(name, labels) Gauge`) 가 본 SPEC §6.1 의 `MetricsSink` 정의와 100% 일치 (Histogram 의 추가 `buckets []float64` parameter 는 caller 가 nil 전달 시 default — backward compatible).
...
1. **`MetricsSink` interface** — `internal/command/adapter/metrics.go` (신규 파일) 가 선행 SPEC `SPEC-GOOSE-OBS-METRICS-001` 의 sink 를 alias 패턴으로 import (v0.1.1 amendment 로 확정):
   ```go
   // internal/command/adapter/metrics.go
   package adapter

   import "github.com/modu-ai/goose/internal/observability/metrics"

   // alias to upstream sink contract — single source of truth at OBS-METRICS-001
   ...
   별도 interface 재정의 없음. 본 SPEC body 의 §6.1 `MetricsSink` interface 정의는 alias 가 referencing 하는 contract 를 spec.md 차원에서 명시 보존하는 용도이며, run phase 시 신규 interface 선언으로 구현하지 않는다.
...
| ID | 위험 | 우선순위 | 완화 전략 |
|----|------|----------|-----------|
| **R1** | **선행 metrics 인프라 SPEC `SPEC-GOOSE-OBS-METRICS-001` 부재 — 본 SPEC implementation 불가** | **🟡 중 (v0.1.1 amendment 로 해소)** | **2026-05-01 v0.1.1 amendment 로 해소됨.** 선행 SPEC `SPEC-GOOSE-OBS-METRICS-001` 이 PR #60 으로 implemented v0.1.0. `metrics.Sink` interface + Phase 1 expvar backend + noop fallback 사용 가능. 본 SPEC §3.1 #1 의 alias 패턴 (`type MetricsSink = metrics.Sink`) 으로 import. (이전 plan phase 의 (a) "prerequisite SPEC 별도 신규 작성" 권장 경로가 충족됨. (b) Logger fallback 임시 대안 경로 비활성. (c) plan phase 본 SPEC 작성과 무관하게 진행된다는 항목은 historical.) |
