...
본 SPEC은 5 layer의 **구현 계약**과 **통과/거부 결정 로직**을 규정한다. 사용자와의 실제 상호작용(AskUserQuestion 표시)은 `SPEC-GOOSE-HOOK-001`의 Hook 이벤트 발행을 통해서만 수행하며, 직접 AskUserQuestion을 호출하지 않는다.
...
10. SafetyToken 발급 및 검증:
    - Ed25519 키쌍(`~/.goose/keys/safety-signer.ed25519`)으로 서명
...
import (
    ...
    "github.com/goose/internal/evolve/reflect"
)
...
var DefaultFrozenPaths = []string{
    ...
    ".moai/specs/SPEC-GOOSE-SAFETY-001/**",   // 자기 자신 보호
    ".moai/specs/SPEC-GOOSE-REFLECT-001/**",  // 의존 SPEC 보호
}
```
...
HookEmitter.EmitApprovalRequest
   → HOOK-001 dispatcher
   → 상위 orchestrator (goosed QueryEngine or CLI)
   → AskUserQuestion 표시
   → 사용자 선택
   ↓
...
| ContradictionDetector 오탐 | 유의미한 진화가 NeedsReview로 귀결되어 UX 저하 | 초기 구현은 보수적 규칙만 적용하고, 감지 로그를 축적하여 SPEC-GOOSE-REFLECT-002에서 임계값 튜닝 |
| SafetyToken 키 노출 | 무단 Graduated 승격 가능 | `~/.goose/keys/` 퍼미션 600, 키 로테이션 월 1회, 이전 키로 발급된 토큰은 rotation 즉시 invalidate |
...
- SPEC-GOOSE-INSIGHTS-001 (trajectory scoring API 제공자)
- SPEC-GOOSE-ROLLBACK-001 (canary regression 재활용)
