# GOOSE Runtime Architecture v0.2
...
- Scope: GOOSE end-user runtime (`./.goose/` + `~/.goose/`). NOT moai-adk build-time (`.moai/`, `.claude/`, `CLAUDE.md`는 불가침).
...
- 본 저장소 `.moai/` `.claude/` `CLAUDE.md` = **GOOSE 개발 도구**, 불가침
- 최종 사용자 런타임 = `./.goose/` (workspace) + `~/.goose/` (secrets only)
...
### 1.1 `~/.goose/` (user-global, credential/security 전용)
...
├── state/
│   └── projects.registry         # goose init 된 프로젝트 경로 목록
└── logs/
...
### 1.2 `./.goose/` (project-local, workspace)
...
│   ├── providers.yaml            # LLM 라우팅 규칙 (key는 ~/.goose/ 참조)
│   └── channels.yaml             # 활성 채널 목록
├── persona/
│   ├── soul.md                   # 이 프로젝트에서의 GOOSE 정체성
...
├── logs/
│   ├── goosed.log                # 구조화 JSON 로그
│   └── audit.local.log           # 프로젝트 단위 감사 로그
└── data/
    ├── goose.db                  # 단일 SQLite (WAL)
    ├── goose.db-wal / .shm
...
- `goose init` — 현재 디렉토리에 `./.goose/` 생성 + `~/.goose/state/projects.registry`에 등록
- `goose` — upward traversal로 가장 가까운 `./.goose/` 발견까지 상위 디렉토리 탐색 (git 방식)
- 발견 실패 시 → `goose init` 안내
...
│  Procedural  — ./.goose/skills/*.md + QMD 인덱스           │
...
## 4. 데이터베이스 스키마 (`goose.db`, SQLite WAL)
...
### 4.3 Migration 전략
Embedded golang-migrate + `internal/db/migrations/*.up.sql + *.down.sql` (Go `embed` 번들). `goose migrate up/down/status` 서브커맨드.
...
- `~/.goose/secrets/*` = credential 참조만 (키링 ID)
- `./.goose/**` = workspace, 쓰기 자유
- 물리적 분리로 정책 단순화
...
- `goose-proxy` 별도 프로세스 = 네트워크 경계에서 Authorization header 주입
...
Secret은 OS keyring, 이름은 `goose-provider-{name}` 규약.
...
- **인덱싱 대상**:
  - `./.goose/memory/MEMORY.md USER.md summaries/*.md`
  - `./.goose/context/*.md` (11개)
  - `./.goose/skills/**/*.md`
  - `./.goose/tasks/*/result.md reflection.md`
  - `./.goose/rituals/*/runs/*.md`
- **모델**: `bge-small-en-v1.5.gguf` (embedder, ~120MB) + `bge-reranker-base.gguf` (~280MB)
- **재인덱스**: `on_file_change` 자동, `goose qmd reindex` 수동
...
requires:
  ...
  fs_read: ["./.goose/context/goals.md"]
  fs_write: []
  exec: []
...
- 파일: `./.goose/tasks/{task-id}/plan.md` (사람 읽기용)
...
- Binaries:
  - `goosed` — 데몬 (long-running, channel gateway)
  - `goose` — 사용자 CLI (init, status, migrate, provider, qmd, forget, …)
  - `goose-proxy` — zero-knowledge credential proxy (별도 프로세스)
...
| `~/.goose/` 전체 워크스페이스 | project-local로 전환 (team 공유·격리) |
