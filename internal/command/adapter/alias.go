...
//  4. If len(parts) != 2 or provider/model is empty, return ErrInvalidCanonical.
//  5. Look up provider in registry; if not found, return ErrUnknownProviderInAlias.
//  6. Verify model is in meta.SuggestedModels; if not, return ErrUnknownModelInAlias.
...
func resolveAlias(
	...
	if len(parts) != 2 || parts[0] == "" || parts[1] == "" {
		return nil, command.ErrInvalidCanonical
	}
	...
	if !ok {
		return nil, command.ErrUnknownProviderInAlias
	}

	// Step 6: strict model verification.
	if !slices.Contains(meta.SuggestedModels, modelName) {
		return nil, command.ErrUnknownModelInAlias
	}
