...
// TestGroq_UsesCustomBaseURL는 AC-ADP2-004를 검증한다.
// Groq 어댑터가 https://api.groq.com/openai/v1를 기본으로 사용하고,
// 테스트에서 override 시 올바른 경로에 요청하는지 검증.
func TestGroq_UsesCustomBaseURL(t *testing.T) {
	t.Parallel()
	...
	secretStore := provider.NewMemorySecretStore(map[string]string{"kr-cred-a": "sk-groq-test"})

	adapter, err := groq.New(groq.Options{
		Pool:        pool,
	...
	// Name 검증
	assert.Equal(t, "groq", adapter.Name())

	// Capabilities 검증
	caps := adapter.Capabilities()
	assert.True(t, caps.Streaming)
	assert.True(t, caps.Tools)
	assert.False(t, caps.Vision) // Groq은 vision 미지원
	assert.False(t, caps.AdaptiveThinking)

	// 실제 요청 경로 검증
	ctx := context.Background()
	req := provider.CompletionRequest{
		Route:    router.Route{Provider: "groq", Model: "llama-3.3-70b-versatile"},
...
// TestGroq_DefaultName은 Groq 어댑터의 기본 이름을 검증한다.
func TestGroq_DefaultName(t *testing.T) {
	t.Parallel()
	pool := testhelper.FakePool(t, []string{"cred-a"})
	secretStore := provider.NewMemorySecretStore(map[string]string{"kr-cred-a": "sk-groq-test"})
